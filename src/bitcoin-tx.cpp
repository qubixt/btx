...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The btx-tx tool (legacy alias: bitcoin-tx) is used for creating and modifying bitcoin transactions.\n\n"
                "btx-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitcoin transaction, or with \"-create [commands]\" to create a hex-encoded bitcoin transaction.\n"
                "\n"
                "Usage: btx-tx [options] <hex-tx> [commands]\n"
                "or:    btx-tx [options] -create [commands]\n"
                "\n";
...
MAIN_FUNCTION
{
    WarnAboutLegacyBinaryInvocation(argv[0] ? argv[0] : "", "bitcoin-tx", "btx-tx");
    SetupEnvironment();
