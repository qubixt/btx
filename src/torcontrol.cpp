...
std::string TorController::LaunchTor()
{
    ...
    // Wait up to 60 seconds for Tor to create the control port file.
    constexpr int TOR_STARTUP_TIMEOUT_SECONDS{60};
    for (int elapsed = 0; !fs::exists(controlport_env_filepath); ++elapsed) {
        if (m_process->poll() != -1) {
            LogDebug(BCLog::TOR, "tor: Tor process died before making control port file\n");
            throw std::runtime_error("tor process died");
        }
        if (elapsed >= TOR_STARTUP_TIMEOUT_SECONDS) {
            LogDebug(BCLog::TOR, "tor: Timed out waiting for control port file after %d seconds\n", TOR_STARTUP_TIMEOUT_SECONDS);
            throw std::runtime_error("tor process timed out waiting for control port file");
        }
