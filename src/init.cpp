...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-retainshieldedcommitmentindex", "Keep the shielded commitment-position index in the local LevelDB store across restart and snapshot recovery. The default externalized-retention production profile leaves this off and rebuilds the index in memory as needed; enabling it is a fuller-retention dev/audit mode with higher persistent-state growth.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-miningminoutboundpeers=<n>", "Require at least <n> outbound peers before serving getblocktemplate work. This helps reduce orphan/stale risk from isolated mining topologies. Defaults to 2 on mainnet and 0 on other chains. Set to 0 to disable.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningminsyncedoutboundpeers=<n>", "Require at least <n> outbound peers to report sync height near our active tip before serving getblocktemplate work. This helps reduce stale/forked mining when connected peers lag. Defaults to 1 on mainnet and 0 on other chains. Set to 0 to disable.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningmaxpeersyncheightlag=<n>", "Maximum allowed sync-height lag (in blocks) for peers counted by -miningminsyncedoutboundpeers. Defaults to 2 on mainnet and 0 on other chains.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningmaxheaderlag=<n>", "Require validated tip to be within <n> blocks of best-known header before serving getblocktemplate work. This helps reduce stale-template orphan risk when headers outpace block validation. Defaults to 8 on mainnet and 0 on other chains. Set to 0 to disable.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguard", "Pause local mining work when outbound peers indicate the active tip diverged too far from the majority peer view (default: 1 on mainnet, 0 on test chains)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardminpeers=<n>", strprintf("Minimum outbound peers with usable tip heights required before local mining continues when -miningchainguard is enabled (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_MIN_PEERS), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardmaxmediangap=<n>", strprintf("Maximum allowed block gap between the local tip and the median outbound peer tip before local mining pauses when -miningchainguard is enabled (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_MAX_MEDIAN_GAP), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
