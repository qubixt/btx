...
namespace {
int AlignedPeriodParentHeight(const CBlockIndex* pindex_prev, int period)
{
    assert(pindex_prev != nullptr);
    const int64_t period_offset{(static_cast<int64_t>(pindex_prev->nHeight) + 1) % period};
    const int64_t aligned_height{static_cast<int64_t>(pindex_prev->nHeight) - period_offset};
    // -1 is valid here and represents the parent of genesis (nullptr ancestor).
    assert(aligned_height >= -1);
    assert(aligned_height <= std::numeric_limits<int>::max());
...
ThresholdState AbstractThresholdConditionChecker::GetStateFor(const CBlockIndex* pindexPrev, const Consensus::Params& params, ThresholdConditionCache& cache) const
{
    ...
    if (pindexPrev != nullptr) {
        pindexPrev = pindexPrev->GetAncestor(AlignedPeriodParentHeight(pindexPrev, nPeriod));
    }
    ...
    while (!vToCompute.empty()) {
        ...
        switch (state) {
            ...
            case ThresholdState::LOCKED_IN: {
                ...
                const int64_t next_height{static_cast<int64_t>(pindexPrev->nHeight) + 1};
                if (next_height >= min_activation_height) {
...
int AbstractThresholdConditionChecker::GetStateSinceHeightFor(const CBlockIndex* pindexPrev, const Consensus::Params& params, ThresholdConditionCache& cache) const
{
    ...
    pindexPrev = Assert(pindexPrev->GetAncestor(AlignedPeriodParentHeight(pindexPrev, nPeriod)));
