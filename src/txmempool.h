...
private:
    ...
    void AddShieldedNullifiers(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void RemoveShieldedNullifiers(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void AddShieldedSettlementAnchorRefs(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void RemoveShieldedSettlementAnchorRefs(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
...
public:
    ...
    /** Return true if the transaction reuses a nullifier already present in mempool. */
    bool HasShieldedNullifierConflict(const CTransaction& tx) const EXCLUSIVE_LOCKS_REQUIRED(cs);
    /** Return true if the transaction reuses a settlement anchor already referenced in mempool. */
    bool HasShieldedSettlementAnchorConflict(const CTransaction& tx) const EXCLUSIVE_LOCKS_REQUIRED(cs);
