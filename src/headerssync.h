...
struct CompressedHeader {
    ...
    CompressedHeader(const CBlockHeader& header)
    {
        ...
        matmul_digest = header.matmul_digest;
    ...
    CBlockHeader GetFullHeader(const uint256& hash_prev_block) const {
        ...
        ret.matmul_digest = matmul_digest;
        ret.matmul_dim = matmul_dim;
        ret.seed_a = seed_a;
        ret.seed_b = seed_b;
        return ret;
    };
...
private:
    ...
    /** Enforce schedule-valid MatMul nBits transitions and advance synthetic index state. */
    bool ValidateAndAdvanceMatMulDifficulty(const CBlockHeader& header, int64_t next_height, bool redownload_phase);

    /** Append a synthetic index entry, keeping a bounded window for retarget simulation. */
    void AdvanceSyntheticIndexWindow(
...
private:
    ...
    /** Synthetic index tail for MatMul nBits validation during PRESYNC. */
    CBlockIndex* m_presync_last_index{nullptr};
    std::list<CBlockIndex> m_presync_index_window;
    ...
    /** Synthetic index tail for MatMul nBits validation during REDOWNLOAD. */
    CBlockIndex* m_redownload_last_index{nullptr};
    std::list<CBlockIndex> m_redownload_index_window;
