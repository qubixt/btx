...
bool ParseP2MRPubkeyPush(Span<const unsigned char> script, size_t offset, Span<const unsigned char>& pubkey, size_t& consumed)
{
    if (offset >= script.size()) return false;

    const unsigned char op = script[offset];
    ...
    if (op <= 75) {
        ...
        if (offset + 1 >= script.size()) return false;
        header_size = 2;
        key_size = script[offset + 1];
    } else if (op == OP_PUSHDATA2) {
        if (offset + 2 >= script.size()) return false;
        header_size = 3;
        key_size = script[offset + 1] | (static_cast<size_t>(script[offset + 2]) << 8);
    } else if (op == OP_PUSHDATA4) {
        if (offset + 4 >= script.size()) return false;
        header_size = 5;
        key_size = script[offset + 1] |
                   (static_cast<size_t>(script[offset + 2]) << 8) |
                   (static_cast<size_t>(script[offset + 3]) << 16) |
                   (static_cast<size_t>(script[offset + 4]) << 24);
    } else {
        return false;
    }

    if (key_size != MLDSA44_PUBKEY_SIZE && key_size != SLHDSA128S_PUBKEY_SIZE) return false;
    if (offset + header_size + key_size > script.size()) return false;

    pubkey = script.subspan(offset + header_size, key_size);
    consumed = header_size + key_size;
    return true;
}

bool IsP2MRChecksigOpcode(opcodetype opcode)
{
    return opcode == OP_CHECKSIG_MLDSA || opcode == OP_CHECKSIG_SLHDSA;
}

bool IsP2MRChecksigAddOpcode(opcodetype opcode)
{
    return opcode == OP_CHECKSIGADD_MLDSA || opcode == OP_CHECKSIGADD_SLHDSA;
}
...
void ExtractP2MRLeafPQPubkeys(Span<const unsigned char> script, std::set<std::vector<unsigned char>, ShortestVectorFirstComparator>& out_pubkeys)
{
    ...
    if (script.size() > 35 && script[0] == 32 && script[33] == OP_CHECKTEMPLATEVERIFY && script[34] == OP_DROP) {
        ...
        if (script_obj.GetOp(pc, opcode, push_data)) {
            const bool is_locktime_push =
                opcode == OP_0 || opcode == OP_1NEGATE ||
                ((opcode < OP_1 || opcode > OP_16) ? !push_data.empty() : true);
            if (is_locktime_push &&
                script_obj.GetOp(pc, opcode, push_data) &&
                (opcode == OP_CHECKLOCKTIMEVERIFY || opcode == OP_CHECKSEQUENCEVERIFY) &&
                script_obj.GetOp(pc, opcode, push_data) &&
                opcode == OP_DROP) {
                offset = std::distance(script_obj.begin(), pc);
            }
        }
    }
    ...
    size_t consumed{0};
    if (!ParseP2MRPubkeyPush(script, offset, pubkey, consumed)) return;
    offset += consumed;
    if (offset >= script.size()) return;

    const opcodetype checksig_opcode = static_cast<opcodetype>(script[offset++]);
    if (!IsP2MRChecksigOpcode(checksig_opcode)) return;
    out_pubkeys.insert(std::vector<unsigned char>(pubkey.begin(), pubkey.end()));

    while (offset < script.size()) {
        if (offset + 2 == script.size() &&
            static_cast<opcodetype>(script[offset]) >= OP_1 &&
            static_cast<opcodetype>(script[offset]) <= OP_16 &&
            static_cast<opcodetype>(script[offset + 1]) == OP_NUMEQUAL) {
            return;
        }

        Span<const unsigned char> next_pubkey;
        size_t next_consumed{0};
        if (!ParseP2MRPubkeyPush(script, offset, next_pubkey, next_consumed)) return;
        offset += next_consumed;
        if (offset >= script.size()) return;

        const opcodetype checksigadd_opcode = static_cast<opcodetype>(script[offset++]);
        if (!IsP2MRChecksigAddOpcode(checksigadd_opcode)) return;
        out_pubkeys.insert(std::vector<unsigned char>(next_pubkey.begin(), next_pubkey.end()));
    }
}
...
SigningResult ScriptPubKeyMan::SignMessageBIP322(MessageSignatureFormat format, const SigningProvider* keystore, const std::string& message, const CTxDestination& address, std::string& str_sig) const
{
    ...
    if (!::SignTransaction(to_sign, keystore, coins, SIGHASH_ALL, errors)) {
        // NOTE: this may be a multisig which successfully signed but needed additional signatures
...
std::vector<CKeyPool> LegacyScriptPubKeyMan::MarkReserveKeysAsUsed(int64_t keypool_id)
{
    ...
    while (it != std::end(*setKeyPool)) {
        ...
        CKeyPool keypool;
        if (batch.ReadPool(index, keypool)) { //NOTE: This should be unnecessary
            m_pool_key_to_index.erase(keypool.vchPubKey.GetID());
        }
...
util::Result<CTxDestination> DescriptorScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    ...
    }
    {
        ...
        if (m_wallet_descriptor.range_end <= m_wallet_descriptor.next_index && !TopUp(1)) {
        ...
        if (scripts_temp.empty()) {
            WalletLogPrintf("%s: Descriptor expansion returned no scripts at index %d\n", __func__, m_wallet_descriptor.next_index);
            return util::Error{_("Error: Cannot extract destination from the generated scriptpubkey")};
        }
...
bool DescriptorScriptPubKeyMan::TopUpWithDB(WalletBatch& batch, unsigned int size)
{
    ...
    for (int32_t i = m_max_cached_index + 1; i < new_range_end; ++i) {
        ...
        bool expanded = m_wallet_descriptor.descriptor->ExpandFromCache(i, m_wallet_descriptor.cache, scripts_temp, out_keys);
        if (!expanded) {
            // Try direct expansion first. Even if it fails later (e.g. P2MR PQ pubkey derivation
            // without private keys), it can still populate xpub cache entries in temp_cache.
            expanded = m_wallet_descriptor.descriptor->Expand(i, provider, scripts_temp, out_keys, &temp_cache);

            DescriptorCache learned_cache = m_wallet_descriptor.cache.MergeAndDiff(temp_cache);
            if (!batch.WriteDescriptorCacheItems(id, learned_cache)) {
                throw std::runtime_error(std::string(__func__) + ": writing learned cache items failed");
            }

            if (!expanded) {
                DescriptorCache external_cache;
                std::string external_cache_error;
                if (PopulateDerivedCache(i, external_cache, external_cache_error)) {
                    DescriptorCache external_items = m_wallet_descriptor.cache.MergeAndDiff(external_cache);
                    if (!batch.WriteDescriptorCacheItems(id, external_items)) {
                        throw std::runtime_error(std::string(__func__) + ": writing external cache items failed");
                    }
                } else if (!external_cache_error.empty()) {
                    WalletLogPrintf("%s: descriptor cache population failed for index %d: %s\n", __func__, i, external_cache_error);
                }

                scripts_temp.clear();
                out_keys = FlatSigningProvider{};
                expanded = m_wallet_descriptor.descriptor->ExpandFromCache(i, m_wallet_descriptor.cache, scripts_temp, out_keys);
            }
        }
        ...
        // Build PQ pubkey → index reverse map from the expanded provider.
        // During TopUp the public expansion already contains PQ pubkeys from cache.
        for (const auto& [pq_pub, pq_key] : out_keys.pq_keys) {
            if (m_map_pq_pubkey_to_index.count(pq_pub) == 0) {
                m_map_pq_pubkey_to_index[pq_pub] = i;
            }
        }
...
bool DescriptorScriptPubKeyMan::SetupPQDescriptorGeneration(WalletBatch& batch, Span<const unsigned char> pq_seed, bool internal)
{
    LOCK(cs_desc_man);
    assert(m_storage.IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS));
...
bool DescriptorScriptPubKeyMan::HavePrivateKeys() const
{
    ...
    if (m_wallet_descriptor.descriptor) {
        ...
        return m_wallet_descriptor.descriptor->ToPrivateString(empty_provider, priv_desc);
    }
    return false;
}
...
std::unique_ptr<FlatSigningProvider> DescriptorScriptPubKeyMan::GetSigningProvider(Span<const unsigned char> pq_pubkey) const
{
    LOCK(cs_desc_man);

    const std::vector<unsigned char> lookup_key(pq_pubkey.begin(), pq_pubkey.end());

    // Fast O(1) lookup via the PQ pubkey reverse map (populated during TopUp/SetCache)
    auto it = m_map_pq_pubkey_to_index.find(lookup_key);
    if (it != m_map_pq_pubkey_to_index.end()) {
        std::unique_ptr<FlatSigningProvider> keys = GetSigningProvider(it->second, /*include_private=*/true);
        if (keys) {
            const CPQKey* key = keys->GetPQKey(lookup_key);
    ...
    // Fall back to linear scan for keys not in the map (e.g. derived after TopUp
    // without PQ pubkeys in the cache expansion).
    // This also covers the case where the input script itself is not recognized
    // by this descriptor manager.
    for (int32_t index = m_wallet_descriptor.range_start; index < m_wallet_descriptor.range_end; ++index) {
        std::unique_ptr<FlatSigningProvider> keys = GetSigningProvider(index, /*include_private=*/true);
        if (!keys) continue;
        const CPQKey* key = keys->GetPQKey(lookup_key);
        if (key != nullptr && key->IsValid()) {
            // Populate the reverse map for future lookups
            m_map_pq_pubkey_to_index[lookup_key] = index;
            return keys;
        }
    }
...
std::unique_ptr<FlatSigningProvider> DescriptorScriptPubKeyMan::GetSigningProvider(int32_t index, bool include_private) const
{
    ...
    if (it != m_map_signing_providers.end()) {
        ...
        if (!m_wallet_descriptor.descriptor->ExpandFromCache(index, m_wallet_descriptor.cache, scripts_temp, *out_keys)) {
            // Some imported descriptors (notably fixed-key non-ranged descriptors) may not have
            // enough cache material to expand from cache alone. Fall back to direct expansion.
            FlatSigningProvider empty_provider;
            if (!m_wallet_descriptor.descriptor->Expand(index, empty_provider, scripts_temp, *out_keys, /*write_cache=*/nullptr)) {
                return nullptr;
            }
        }
    ...
    if (include_private) {
        // Check the private-key-inclusive cache first to avoid expensive PQ key derivation
        auto it_priv = m_map_signing_providers_private.find(index);
        if (it_priv != m_map_signing_providers_private.end()) {
            return std::make_unique<FlatSigningProvider>(it_priv->second);
        }
        FlatSigningProvider master_provider;
        master_provider.keys = GetKeys();
        m_wallet_descriptor.descriptor->ExpandPrivate(index, master_provider, *out_keys);
        // Cache the fully-expanded provider (includes PQ private keys where available)
        m_map_signing_providers_private[index] = *out_keys;
    }
...
std::optional<PSBTError> DescriptorScriptPubKeyMan::FillPSBT(PartiallySignedTransaction& psbtx, const PrecomputedTransactionData& txdata, int sighash_type, bool sign, bool bip32derivs, int* n_signed, bool finalize) const
{
    ...
    // Phase 1: Collect signing providers for all inputs (sequential, under cs_desc_man lock).
    struct InputWork {
        unsigned int index;
        std::unique_ptr<FlatSigningProvider> keys;
    };
    std::vector<InputWork> work_items;

    {
        // The key-lookup phase needs cs_desc_man; collect all providers first.
        for (unsigned int i = 0; i < psbtx.tx->vin.size(); ++i) {
            const CTxIn& txin = psbtx.tx->vin[i];
            PSBTInput& input = psbtx.inputs.at(i);
            ...
            if (script_keys) {
                ...
                // P2MR leaf pubkeys (single-sig or multisig). This allows signing from pubkey metadata
                // even if this descriptor manager does not own the exact input script.
                std::set<std::vector<unsigned char>, ShortestVectorFirstComparator> pq_pubkeys;
                const auto maybe_add_pq_pubkey = [&pq_pubkeys](const std::vector<unsigned char>& pubkey) {
                    if (pubkey.size() != MLDSA44_PUBKEY_SIZE && pubkey.size() != SLHDSA128S_PUBKEY_SIZE) return;
                    pq_pubkeys.insert(pubkey);
                };
                for (const auto& [pubkey, _] : input.m_p2mr_bip32_paths) {
                    maybe_add_pq_pubkey(pubkey);
                }
                for (const auto& [leaf_pubkey, _] : input.m_p2mr_pq_sigs) {
                    maybe_add_pq_pubkey(leaf_pubkey.second);
                }
                for (const auto& [leaf_pubkey, _] : input.m_p2mr_csfs_msgs) {
                    maybe_add_pq_pubkey(leaf_pubkey.second);
                }
                for (const auto& [leaf_pubkey, _] : input.m_p2mr_csfs_sigs) {
                    maybe_add_pq_pubkey(leaf_pubkey.second);
                }
                ExtractP2MRLeafPQPubkeys(input.m_p2mr_leaf_script, pq_pubkeys);

                for (const auto& pq_pubkey : pq_pubkeys) {
                    std::unique_ptr<FlatSigningProvider> pq_keys = GetSigningProvider(Span<const unsigned char>{pq_pubkey});
                    if (pq_keys) {
                        keys->Merge(std::move(*pq_keys));
                    }
                }
            }

            work_items.push_back({i, std::move(keys)});
        }
    } // end Phase 1

    // Phase 2: Sign inputs in parallel (no lock needed).
    // Each thread operates on its own FlatSigningProvider and writes only to psbtx.inputs[i].
    // Thread safety: psbtx.tx is read-only, txdata is read-only (precomputed),
    // each FlatSigningProvider is thread-local, and libbitcoinpqc uses thread-local random state.
    if (sign && work_items.size() > 1) {
        // Cap concurrent threads at half hardware_concurrency to avoid oversubscription
        // when multiple wallets sign concurrently (signer1 + signer2 via tokio::join).
        size_t hw_threads = std::thread::hardware_concurrency();
        if (hw_threads == 0) hw_threads = 4; // safe fallback
        const size_t batch_size = std::clamp<size_t>(hw_threads / 2, 2, work_items.size());

        for (size_t batch_start = 0; batch_start < work_items.size(); batch_start += batch_size) {
            const size_t batch_end = std::min(batch_start + batch_size, work_items.size());
            ...
            for (size_t k = batch_start; k < batch_end; ++k) {
                bool signed_one = futures[k - batch_start].get();
                if (n_signed && signed_one) (*n_signed)++;

                // P2MR leaf pubkeys (single-sig or multisig). Merge PQ signing
                // metadata so hardware wallets can identify P2MR inputs.
                auto& psbt_input = psbtx.inputs.at(work_items[k].index);
                std::set<std::vector<unsigned char>, ShortestVectorFirstComparator> pq_pubkeys;
                for (const auto& [pubkey, _] : psbt_input.m_p2mr_bip32_paths) {
                    pq_pubkeys.insert(pubkey);
                }
                for (const auto& [leaf_pubkey, _] : psbt_input.m_p2mr_pq_sigs) {
                    pq_pubkeys.insert(leaf_pubkey.second);
                }
                for (const auto& [leaf_pubkey, _] : psbt_input.m_p2mr_csfs_msgs) {
                    pq_pubkeys.insert(leaf_pubkey.second);
                }
                for (const auto& [leaf_pubkey, _] : psbt_input.m_p2mr_csfs_sigs) {
                    pq_pubkeys.insert(leaf_pubkey.second);
                }
                ExtractP2MRLeafPQPubkeys(psbt_input.m_p2mr_leaf_script, pq_pubkeys);

                for (const auto& pq_pubkey : pq_pubkeys) {
                    std::unique_ptr<FlatSigningProvider> pq_keys = GetSigningProvider(Span<const unsigned char>{pq_pubkey});
                    if (pq_keys) {
                        work_items[k].keys->Merge(std::move(*pq_keys));
                    }
                }
            }
        }
...
{
    LOCK(cs_desc_man);
    // Invalidate caches that depend on the descriptor cache
    m_map_signing_providers.clear();
    m_map_signing_providers_private.clear();
    m_map_pq_pubkey_to_index.clear();
    std::set<CScript> new_spks;
    ...
    for (int32_t i = m_wallet_descriptor.range_start; i < m_wallet_descriptor.range_end; ++i) {
        ...
        // Build PQ pubkey → index reverse map from cache expansion
        for (const auto& [pq_pub, pq_key] : out_keys.pq_keys) {
            if (m_map_pq_pubkey_to_index.count(pq_pub) == 0) {
                m_map_pq_pubkey_to_index[pq_pub] = i;
            }
...
void DescriptorScriptPubKeyMan::UpgradeDescriptorCache()
{
    ...
    if (!m_wallet_descriptor.descriptor->Expand(0, provider, scripts_temp, out_keys, &temp_cache)) {
        // Descriptors that rely on externally provided PQ pubkeys may be valid and
        // fully usable from cache, yet not directly expandable with local keys.
        // Cache upgrade is best-effort; skip without failing wallet load.
        WalletLogPrintf("%s: skipping cache upgrade because descriptor expansion is unavailable with local keys\n", __func__);
        return;
    }
