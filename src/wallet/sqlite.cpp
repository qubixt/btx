...
static void ConfigureDatabaseSecurity(sqlite3* db)
{
#ifdef SQLITE_DBCONFIG_DEFENSIVE
    const int defensive_ret = sqlite3_db_config(db, SQLITE_DBCONFIG_DEFENSIVE, 1, nullptr);
    if (defensive_ret != SQLITE_OK) {
        throw std::runtime_error(strprintf("SQLiteDatabase: Failed to enable defensive mode: %s\n", sqlite3_errstr(defensive_ret)));
    }
#endif

#ifdef SQLITE_DBCONFIG_TRUSTED_SCHEMA
    const int trusted_schema_ret = sqlite3_db_config(db, SQLITE_DBCONFIG_TRUSTED_SCHEMA, 0, nullptr);
    if (trusted_schema_ret != SQLITE_OK) {
        throw std::runtime_error(strprintf("SQLiteDatabase: Failed to disable trusted schema mode: %s\n", sqlite3_errstr(trusted_schema_ret)));
    }
#endif
...
void SQLiteDatabase::Open()
{
    ...
    if (m_db == nullptr) {
        ...
        ConfigureDatabaseSecurity(m_db);
