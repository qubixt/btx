...
extern const std::string PQMASTERSEEDCRYPT;
extern const std::string SHIELDEDSTATECRYPT;
...
public:
    ...
    bool WriteDescriptorPQPubKeyCache(PQAlgorithm algo, Span<const unsigned char> pubkey, const uint256& desc_id, uint32_t key_exp_index, uint32_t der_index);
    bool WriteDescriptorCacheItems(const uint256& desc_id, const DescriptorCache& cache);

    //! Write the global PQ master seed (encrypted if wallet is encrypted)
    bool WritePQMasterSeed(const std::vector<unsigned char>& seed);
    //! Read the global PQ master seed
    bool ReadPQMasterSeed(std::vector<unsigned char>& seed);
    //! Erase plaintext global PQ master seed.
    bool ErasePQMasterSeed();
    //! Write encrypted global PQ master seed.
    bool WriteCryptedPQMasterSeed(const uint256& iv, const std::vector<unsigned char>& seed);
    //! Read encrypted global PQ master seed.
    ...
    bool ReadShieldedState(std::vector<unsigned char>& state);
    //! Erase plaintext shielded wallet persisted state blob.
    bool EraseShieldedState();
    //! Write encrypted shielded wallet persisted state blob.
    bool WriteCryptedShieldedState(const uint256& iv, const std::vector<unsigned char>& state);
    //! Read encrypted shielded wallet persisted state blob.
    bool ReadCryptedShieldedState(uint256& iv, std::vector<unsigned char>& state);
    //! Erase encrypted shielded wallet persisted state blob.
    bool EraseCryptedShieldedState();

    //! Write the shielded commitment index (plaintext, public on-chain data).
    bool WriteShieldedCommitments(const std::vector<uint256>& commitments);
    //! Read the shielded commitment index.
    bool ReadShieldedCommitments(std::vector<uint256>& commitments);
