...
//! Default for -keypool
// BTX P2MR outputs require deriving large PQ keys; keep the default keypool
// small to avoid expensive wallet creation and address generation. Users may
// override with `-keypool=<n>` if they want a larger look-ahead window.
static const unsigned int DEFAULT_KEYPOOL_SIZE = 20;
...
public:
    ...
    virtual bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, std::optional<CAmount>* inputs_amount_sum = nullptr, std::optional<PQAlgorithm> preferred_pq_signing_algo = std::nullopt) const { return false; }
...
public:
    ...
    // NOTE: Remove IsMine when deleting LegacyScriptPubKeyMan
...
public:
    ...
    bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, std::optional<CAmount>* inputs_amount_sum = nullptr, std::optional<PQAlgorithm> preferred_pq_signing_algo = std::nullopt) const override;
...
private:
    ...
    mutable std::map<int32_t, FlatSigningProvider> m_map_signing_providers GUARDED_BY(cs_desc_man);
    // Cached FlatSigningProviders that include private keys (PQ key derivation is expensive).
    mutable std::map<int32_t, FlatSigningProvider> m_map_signing_providers_private GUARDED_BY(cs_desc_man);
    // Reverse map from PQ pubkey to descriptor index for O(1) lookup.
    mutable std::map<std::vector<unsigned char>, int32_t, ShortestVectorFirstComparator> m_map_pq_pubkey_to_index GUARDED_BY(cs_desc_man);
...
protected:
    ...
    //! Optional hook for subclasses that can provide descriptor cache items without private keys.
    virtual bool PopulateDerivedCache(uint32_t index, DescriptorCache& cache, std::string& error) const
    {
        return false;
    }
