...
{
    argsman.AddArg("-addresstype", strprintf("What type of addresses to use (supported: \"p2mr\", default: \"%s\")", FormatOutputType(DEFAULT_ADDRESS_TYPE)), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    argsman.AddArg("-changetype",
                   "What type of change to use (supported: \"p2mr\").",
...
#endif
    argsman.AddArg("-autoshieldcoinbase", "Automatically shield mature coinbase outputs into the shielded pool when a new block arrives (default: true)", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    argsman.AddArg("-shieldedringsize=<n>",
                   strprintf("Default shielded send/ingress ring size to build in the wallet (supported: %u..%u, default: %u). Larger supported ring sizes remain available without a wire-format change.",
                             shielded::lattice::MIN_RING_SIZE,
                             shielded::lattice::MAX_RING_SIZE,
                             shielded::lattice::DEFAULT_RING_SIZE),
...
bool WalletInit::ParameterInteraction() const
{
    ...
    if (gArgs.IsArgSet("-shieldedringsize")) {
        const int64_t ring_size = gArgs.GetIntArg("-shieldedringsize",
                                                  static_cast<int64_t>(shielded::lattice::DEFAULT_RING_SIZE));
        if (ring_size < 0 ||
            !shielded::lattice::IsSupportedRingSize(static_cast<size_t>(ring_size))) {
            return InitError(Untranslated(strprintf("Unsupported -shieldedringsize=%d (supported: %u..%u)",
                                                    ring_size,
                                                    shielded::lattice::MIN_RING_SIZE,
                                                    shielded::lattice::MAX_RING_SIZE)));
        }
