...
int NextWalletHeightOrLimit(const CWallet& wallet) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    const int last_block_height = wallet.GetLastBlockHeight();
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    static constexpr int MAX_LISTTRANSACTIONS_RESULTS{1'000'000};
    if (nCount > MAX_LISTTRANSACTIONS_RESULTS) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Count must be <= %d", MAX_LISTTRANSACTIONS_RESULTS));
    }
    if (nFrom > MAX_LISTTRANSACTIONS_RESULTS) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("From must be <= %d", MAX_LISTTRANSACTIONS_RESULTS));
    }
    ...
    std::vector<UniValue> ret;
    {
        ...
        const size_t requested_entries = static_cast<size_t>(nCount) + static_cast<size_t>(nFrom);

        // Iterate backwards until we have enough entries to satisfy [from, from+count).
        for (CWallet::TxItems::const_reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it)
        {
            CWalletTx *const pwtx = (*it).second;
            ListTransactions(*pwallet, *pwtx, 0, true, ret, filter, filter_label);
            if (ret.size() >= requested_entries) break;
        }
    }

    // ret is newest to oldest

    const size_t from = std::min(ret.size(), static_cast<size_t>(nFrom));
    const size_t count = std::min(ret.size() - from, static_cast<size_t>(nCount));
    UniValue result{UniValue::VARR};
    for (size_t i = from + count; i > from; --i) {
        result.push_back(std::move(ret[i - 1])); // Return oldest to newest
    }
    return result;
},
    };
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const int next_wallet_height = NextWalletHeightOrLimit(wallet);
    int depth = -1;
    if (height) {
        depth = SaturatingIntFromInt64(static_cast<int64_t>(next_wallet_height) - static_cast<int64_t>(*height));
    }
    ...
    for (const std::pair<const uint256, CWalletTx>& pairWtx : wallet.mapWallet) {
        ...
        const int tx_depth = wallet.GetTxDepthInMainChain(tx);
        const int tx_depth_abs = tx_depth == std::numeric_limits<int>::min() ? std::numeric_limits<int>::max() : std::abs(tx_depth);

        if (depth == -1 || tx_depth_abs < depth) {
    ...
    uint256 lastblock;
    target_confirms = std::min(target_confirms, next_wallet_height);
    const int ancestor_height = SaturatingIntFromInt64(static_cast<int64_t>(next_wallet_height) - static_cast<int64_t>(target_confirms));
    CHECK_NONFATAL(wallet.chain().findAncestorByHeight(wallet.GetLastBlockHash(), ancestor_height, FoundBlock().hash(lastblock)));
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", /*optional=*/true, "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address", /*optional=*/true, "The BTX address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Ensure shielded keys are loaded before rescanning.  When the node
    // started with a locked wallet, LoadPersistedState could only read the
    // plaintext tree-only fallback, leaving m_key_sets empty.  Rehydrating
    // here (now that the wallet is unlocked) restores the KEM keys from the
    // encrypted blob so ScanBlock can trial-decrypt notes.
    const bool has_shielded = pwallet->m_shielded_wallet != nullptr;
    if (has_shielded) {
        LOCK(pwallet->m_shielded_wallet->cs_shielded);
        pwallet->m_shielded_wallet->MaybeRehydrateSpendingKeys();
        // Reset the shielded Merkle tree so ScanBlock rebuilds it from scratch.
        pwallet->m_shielded_wallet->Rescan(start_height);
    }

    CWallet::ScanResult result =
        pwallet->ScanForWalletTransactions(start_block, start_height, stop_height, reserver, /*fUpdate=*/true, /*save_progress=*/false, /*scan_shielded=*/has_shielded);
