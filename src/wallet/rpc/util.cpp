...
bool WalletNeedsLockedShieldedAccountingRefresh(const CWallet& wallet)
{
    if (!wallet.IsCrypted() || !wallet.IsLocked() || !wallet.m_shielded_wallet) {
        return false;
    }

    LOCK2(wallet.cs_wallet, wallet.m_shielded_wallet->cs_shielded);
    return wallet.m_shielded_wallet->IsLockedStateIncomplete();
}
...
void AddLockedShieldedAccountingWarningIfNeeded(const CWallet& wallet,
                                                std::vector<bilingual_str>& warnings,
                                                std::string_view operation)
{
    if (!WalletNeedsLockedShieldedAccountingRefresh(wallet)) {
        return;
    }

    std::string message{"This encrypted wallet contains shielded state. Unlock it after "};
    if (operation.empty()) {
        message += "use";
    } else {
        message += operation;
    }
    message += " to refresh shielded balances, note ownership, and integrity accounting.";
    warnings.push_back(Untranslated(message));
}

std::string ValidateWalletBundleArchiveDirectoryName(const std::string& directory_name)
{
    const fs::path path = fs::u8path(directory_name);
    if (path.empty() || path.is_absolute()) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Wallet bundle archive contains an invalid absolute bundle name");
    }

    auto it = path.begin();
    if (it == path.end()) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Wallet bundle archive contains an invalid empty bundle name");
    }
    const fs::path component = *it;
    ++it;
    if (it != path.end() || component == "." || component == "..") {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Wallet bundle archive contains an invalid bundle name");
    }
    return fs::PathToString(component);
}

std::string ValidateWalletBundleArchiveRelativePath(const std::string& relative_path)
{
    fs::path path = fs::u8path(relative_path).lexically_normal();
    if (path.empty() || path.is_absolute()) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Wallet bundle archive contains an invalid absolute path");
    }
    for (const auto& part : path) {
        if (part == "." || part == "..") {
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Wallet bundle archive contains a parent-directory traversal path");
        }
    }
...
void EnsureUniqueWalletBundleArchiveRelativePaths(const std::vector<std::string>& relative_paths)
{
    ...
    for (const auto& relative_path : relative_paths) {
        const std::string normalized = ValidateWalletBundleArchiveRelativePath(relative_path);
        if (!seen.insert(normalized).second) {
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR,
                               strprintf("Wallet bundle archive contains duplicate file path %s", normalized));
        }
    }
}
...
{
    if (start_height <= 0 || !wallet.m_shielded_wallet) {
        return;
    }

    LOCK(wallet.m_shielded_wallet->cs_shielded);
    if (wallet.m_shielded_wallet->LastScannedHeight() >= 0) {
        throw JSONRPCError(
            RPC_INVALID_PARAMETER,
            "Partial shielded rescans are only supported before a wallet has existing shielded scan state. Use start_height=0 to rebuild from genesis.");
    }
}
