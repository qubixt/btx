...
static std::unique_ptr<SigningProvider> GetSolvingProvider(const CWallet* wallet, const CCoinControl* coin_control, const CScript script_pubkey)
{
...
{
    const std::unique_ptr<SigningProvider> provider = GetSolvingProvider(wallet, coin_control, script_pubkey);
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (fee_needed > current_fee && change_pos) {
        const CAmount fee_shortfall = fee_needed - current_fee;
        CTxOut& change = txNew.vout.at(*change_pos);
        const CAmount remaining_change = change.nValue - fee_shortfall;

        // Coin selection can leave a small fee shortfall once the full signed
        // transaction size is known. Prefer shrinking or dropping change over
        // aborting the transaction after input selection has already succeeded.
        if (remaining_change >= coin_selection_params.min_viable_change) {
            change.nValue = remaining_change;
        } else {
            txNew.vout.erase(txNew.vout.begin() + *change_pos);
            change_pos = std::nullopt;
        }
    ...
    if (sign && !wallet.SignTransaction(txNew, coin_control.m_preferred_pq_signing_algo)) {
...
util::Result<CreatedTransactionResult> FundTransaction(CWallet& wallet, const CMutableTransaction& tx, const std::vector<CRecipient>& vecSend, std::optional<unsigned int> change_pos, bool lockUnspents, CCoinControl coinControl)
{
    ...
    // The caller must hold cs_wallet to prevent races to the new locked unspents between the
