...
// avoid timeouts while still exercising the randomized paths.
#ifdef DEBUG
#define RUN_TESTS 25
#define BNB_SAME_VALUE_UTXOS 5000
#else
#define RUN_TESTS 100
#define BNB_SAME_VALUE_UTXOS 50000
#endif
...
static void add_coin(CoinsResult& available_coins, CWallet& wallet, const CAmount& nValue, CFeeRate feerate = CFeeRate(0), int nAge = 6*24, bool fIsFromMe = false, int nInput =0, bool spendable = false, int custom_size = 0)
{
    ...
    if (spendable) {
        static std::map<const CWallet*, CScript> cached_spendable_script;
        auto it = cached_spendable_script.find(&wallet);
        if (it == cached_spendable_script.end()) {
            const CTxDestination dest = *Assert(wallet.GetNewDestination(OutputType::P2MR, ""));
            it = cached_spendable_script.emplace(&wallet, GetScriptForDestination(dest)).first;
        }
        tx.vout[nInput].scriptPubKey = it->second;
    }
    auto ret = wallet.mapWallet.emplace(std::piecewise_construct, std::forward_as_tuple(txid), std::forward_as_tuple(MakeTransactionRef(std::move(tx)), TxStateInactive{}));
    assert(ret.second);
    CWalletTx& wtx = (*ret.first).second;
    const auto& txout = wtx.tx->vout.at(nInput);
    const int input_bytes = custom_size == 0 ? (spendable ? 68 : CalculateMaximumSignedInputSize(txout, &wallet, /*coin_control=*/nullptr)) : custom_size;
    available_coins.Add(OutputType::BECH32, {COutPoint(wtx.GetHash(), nInput), txout, nAge, input_bytes, /*spendable=*/ true, /*solvable=*/ true, /*safe=*/ true, wtx.GetTxTime(), fIsFromMe, feerate});
}
...
BOOST_AUTO_TEST_CASE(bnb_search_test)
{
    ...
    for (int i = 0; i < BNB_SAME_VALUE_UTXOS; ++i) {
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                /* 10 × 1 BTC + 10 × 2 BTC = 30 BTC. Reaching the target requires all 20 inputs. */
...
static util::Result<SelectionResult> select_coins(const CAmount& target, const CoinSelectionParams& cs_params, const CCoinControl& cc, std::function<CoinsResult(CWallet&)> coin_setup, const node::NodeContext& m_node)
{
    ...
    if (result) {
        BOOST_CHECK_LE(result->GetWeight(), MAX_STANDARD_TX_WEIGHT);
        BOOST_CHECK_GE(result->GetSelectedValue(), target);
    }
    return result;
}
...
BOOST_AUTO_TEST_CASE(check_max_selection_weight)
{
    ...
    CCoinControl cc;
    constexpr int tiny_inputs_required = 4950;
    const int oversized_tiny_input_size = static_cast<int>(MAX_STANDARD_TX_WEIGHT / tiny_inputs_required) + 10;
    BOOST_REQUIRE_GT(tiny_inputs_required * oversized_tiny_input_size, MAX_STANDARD_TX_WEIGHT);
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 BTC and 5000x 0.01 BTC (100.0 BTC total) unspent outputs.
        // On BTX the full serialized input size counts against the standard-weight limit.
        ...
        const auto result = select_coins(
            target, cs_params, cc, [&](CWallet& wallet) {
                ...
                for (int j = 0; j < 5000; ++j) {
                    add_coin(available_coins, wallet, CAmount(0.01 * COIN), CFeeRate(0), 144, false, 0, true, oversized_tiny_input_size);
                }
...

    {
        ...
        // The actor starts with 5000x 0.01 BTC (50.0 BTC total) unspent outputs
        ...
        // No results
        // Reaching 49.5 BTC requires 4950 inputs. Oversized tiny inputs should force that path above MAX_STANDARD_TX_WEIGHT.
        BOOST_CHECK(!result);
    }
}
