...
BOOST_FIXTURE_TEST_CASE(SubtractFee, TestChain100Setup)
{
    ...
    const CAmount coinbase_value{m_coinbase_txns.at(0)->vout.at(0).nValue};
    auto check_tx = [&wallet, coinbase_value](CAmount leftover_input_amount) {
        ...
        // Use P2MR change type on this chain; high witness cost still discourages dust change.
        coin_control.m_change_type = OutputType::P2MR;
...
BOOST_FIXTURE_TEST_CASE(wallet_duplicated_preset_inputs_test, TestChain100Setup)
{
    ...
    std::vector<CRecipient> recipients{{*Assert(wallet->GetNewDestination(OutputType::P2MR, "dummy")),
...
BOOST_FIXTURE_TEST_CASE(availablecoins_buckets_p2mr_outputs, TestChain100Setup)
{
    auto wallet = CreateSyncedWallet(*m_node.chain, WITH_LOCK(Assert(m_node.chainman)->GetMutex(), return m_node.chainman->ActiveChain()), coinbaseKey);
    ...
    const CBlock block = CreateAndProcessBlock({}, script_pubkey);
    BOOST_REQUIRE(!block.vtx.empty());
    const COutPoint outpoint{block.vtx.at(0)->GetHash(), 0};

    // CreateAndProcessBlock() advances the chain tip, but this wallet isn't registered for
    // block-connected notifications in this unit test. Update the wallet's last processed
    // height/hash so ScanForWalletTransactions() will scan past old_tip into the new block.
    ...
    WITH_LOCK(wallet->cs_wallet, wallet->SetLastBlockProcessed(new_tip->nHeight, new_tip->GetBlockHash()));

    WalletRescanReserver reserver(*wallet);
    BOOST_REQUIRE(reserver.reserve());
    const CWallet::ScanResult scan = wallet->ScanForWalletTransactions(old_tip->GetBlockHash(), old_tip->nHeight, /*max_height=*/{}, reserver, /*fUpdate=*/false, /*save_progress=*/false);
    ...
    auto found_outpoint = [&](OutputType type) -> bool {
        const auto it = coins.coins.find(type);
        if (it == coins.coins.end()) return false;
        for (const auto& output : it->second) {
            if (output.outpoint == outpoint) return true;
        }
        return false;
    };

    BOOST_CHECK(found_outpoint(OutputType::P2MR));
    BOOST_CHECK(!found_outpoint(OutputType::UNKNOWN));
}
