...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    const CAmount immature_old_and_new{GetBlockSubsidy(oldTip->nHeight, consensus) + GetBlockSubsidy(newTip->nHeight, consensus)};
    const CAmount immature_new_only{GetBlockSubsidy(newTip->nHeight, consensus)};
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, immature_old_and_new);
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(result.last_scanned_block, newTip->GetBlockHash());
        BOOST_CHECK_EQUAL(*result.last_scanned_height, newTip->nHeight);
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, immature_new_only);
    }
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see btxd log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    wtx.MarkDirty();
    AddKey(wallet, coinbaseKey);
    BOOST_CHECK_EQUAL(CachedTxGetImmatureCredit(wallet, wtx, ISMINE_SPENDABLE), m_coinbase_txns.back()->vout.at(0).nValue);
}
...
{
    if (OUTPUT_TYPES.size() == 1 && OUTPUT_TYPES[0] == OutputType::P2MR) return;
    ...
    // bucket dynamically so this test remains valid across policy/consensus script
    // constraints (e.g. P2PK vs P2PKH coinbase outputs).
    OutputType coinbase_bucket = OutputType::UNKNOWN;
    for (const auto& out_type : OUTPUT_TYPES) {
        if (available_coins.coins[out_type].empty()) continue;
        coinbase_bucket = out_type;
        break;
    }
    expected_coins_sizes[coinbase_bucket] = 1U;
    BOOST_CHECK_EQUAL(available_coins.coins[coinbase_bucket].size(), expected_coins_sizes[coinbase_bucket]);
    ...
    for (const auto& out_type : OUTPUT_TYPES) {
        ...
        expected_coins_sizes[out_type] = (out_type == coinbase_bucket) ? 3U : 2U;
...
{
    const size_t low_r_size = CalculateNestedKeyhashInputSize(false);
    const size_t high_r_size = CalculateNestedKeyhashInputSize(true);

    if (WITNESS_SCALE_FACTOR == 1) {
        BOOST_CHECK_EQUAL(low_r_size + 1, high_r_size);
        BOOST_CHECK_EQUAL(high_r_size, DUMMY_NESTED_P2WPKH_INPUT_SIZE);
    } else {
        BOOST_CHECK_EQUAL(low_r_size, DUMMY_NESTED_P2WPKH_INPUT_SIZE);
        BOOST_CHECK_EQUAL(high_r_size, DUMMY_NESTED_P2WPKH_INPUT_SIZE);
    }
}
...
BOOST_FIXTURE_TEST_CASE(CreateWallet, TestChain100Setup)
{
    ...
    // NOTE: this test fails for some reason if there's a flush
...
BOOST_FIXTURE_TEST_CASE(wallet_sync_tx_invalid_state_test, TestingSetup)
{
    ...
    // Add tx to wallet
    const auto op_dest{*Assert(wallet.GetNewDestination(OutputType::P2MR, ""))};
...
BOOST_FIXTURE_TEST_CASE(tx_depth_height_overflow_guards, BasicTestingSetup)
{
    ...
    LOCK(wallet.cs_wallet);
    wallet.SetLastBlockProcessed(std::numeric_limits<int>::max(), uint256::ONE);

    const CWalletTx confirmed{tx, TxStateConfirmed{uint256::ONE, 0, 0}};
    const CWalletTx conflicted{tx, TxStateBlockConflicted{uint256::ONE, 0}};

    BOOST_CHECK_EQUAL(wallet.GetTxDepthInMainChain(confirmed), std::numeric_limits<int>::max());
    BOOST_CHECK_EQUAL(wallet.GetTxDepthInMainChain(conflicted), std::numeric_limits<int>::min());
}

BOOST_FIXTURE_TEST_CASE(tx_depth_unset_last_processed_height_returns_zero, BasicTestingSetup)
{
    CWallet wallet(m_node.chain.get(), "", CreateMockableWalletDatabase());
    const auto tx = MakeTransactionRef(CMutableTransaction{});
    ...
    const CWalletTx confirmed{tx, TxStateConfirmed{uint256::ONE, 0, 0}};
    const CWalletTx conflicted{tx, TxStateBlockConflicted{uint256::ONE, 0}};
    const CWalletTx inactive{tx, TxStateInactive{/*abandoned=*/false}};
