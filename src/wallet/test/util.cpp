...
{
    constexpr auto output_type = OutputType::P2MR;
    return EncodeDestination(getNewDestination(w, output_type));
}

CTxDestination getNewDestination(CWallet& w, OutputType output_type)
{
    if (auto dest = w.GetNewDestination(output_type, "")) return *dest;
    if (output_type != OutputType::P2MR) {
        if (auto p2mr_dest = w.GetNewDestination(OutputType::P2MR, "")) return *p2mr_dest;
    }
    return *Assert(w.GetNewDestination(OutputType::P2MR, ""));
}
