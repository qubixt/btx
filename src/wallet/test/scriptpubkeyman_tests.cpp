...
BOOST_AUTO_TEST_CASE(Descriptor_IsKeyActive)
{
    ...
    DescriptorScriptPubKeyMan* spkm = dynamic_cast<DescriptorScriptPubKeyMan*>(wallet.GetScriptPubKeyMan(OutputType::P2MR, /*internal=*/false));
    ...
    auto dest1 = spkm->GetNewDestination(OutputType::P2MR);
    ...
    // When user encrypts wallet for the first time, descriptor setup is
    // re-run. With a stable PQ master seed, existing descriptor managers are
    // reused and previously derived keys remain active.
    {
        LOCK(wallet.cs_wallet);
        // called by EncryptWallet()
        wallet.SetupDescriptorScriptPubKeyMans();
    ...
    DescriptorScriptPubKeyMan* active_spkm = dynamic_cast<DescriptorScriptPubKeyMan*>(wallet.GetScriptPubKeyMan(OutputType::P2MR, /*internal=*/false));
    BOOST_REQUIRE(active_spkm != nullptr);
    BOOST_CHECK(active_spkm == spkm);

    // The original keys are still active at both manager and wallet levels.
    int num_script_keys_not_found = 0;
    for (const CScript& script : scripts4) {
        BOOST_CHECK(active_spkm->IsKeyActive(script));
        CTxDestination dest;
        if (!ExtractDestination(script, dest) || !wallet.IsDestinationActive(dest)) {
            ++num_script_keys_not_found;
        }
    }
    BOOST_CHECK_EQUAL(num_script_keys_not_found, 0);
}
