                        ...
                        const CBlockIndex* stop_index, std::vector<DBVal>& results)
{
    ...
    const int64_t results_size_64{
        static_cast<int64_t>(stop_index->nHeight) - static_cast<int64_t>(start_height) + 1};
    if (results_size_64 <= 0) {
        LogError("%s: invalid range size (%lld) in %s\n",
                 __func__, static_cast<long long>(results_size_64), index_name);
        return false;
    }
    size_t results_size = static_cast<size_t>(results_size_64);
