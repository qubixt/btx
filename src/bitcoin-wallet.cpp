...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "btx-wallet (legacy alias: bitcoin-wallet) is an offline tool for creating and interacting with " CLIENT_NAME " wallet files.\n\n"
                "By default btx-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n\n"
                "To change the target wallet, use the -datadir, -wallet and (test)chain selection arguments.\n"
                "\n"
                "Usage: btx-wallet [options] <command>\n"
                "\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
MAIN_FUNCTION
{
...
#endif
    WarnAboutLegacyBinaryInvocation(argv[0] ? argv[0] : "", "bitcoin-wallet", "btx-wallet");
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `btx-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
