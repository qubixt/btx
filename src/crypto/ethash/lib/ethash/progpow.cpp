...
namespace
{
/// Apply the Keccak-f[800] permutation used by ProgPoW.
///
/// @param st  Pointer to 25 32-bit state words updated in-place.
    ...
    uint64_t nonce) noexcept
{
    static const auto lazy_lookup = [](const epoch_context& ctx, uint32_t index) noexcept
    {
        ...
        if (item.word64s[0] == 0)
        {
            // TO-DO: Copy elision here makes it thread-safe?
