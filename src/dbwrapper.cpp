    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
    ...
    if (!key_exists && params.obfuscate && IsEmpty()) {
        ...
        LogInfo("Wrote new obfuscation key for %s", fs::PathToString(params.path));
    }
    LogInfo("Using obfuscation key for %s\n", fs::PathToString(params.path));
}
...
std::optional<std::string> CDBWrapper::GetProperty(std::string_view property) const
{
    std::string value;
    if (!DBContext().pdb->GetProperty(std::string{property}, &value)) {
        return std::nullopt;
    }
    return value;
}
