...
bool ExpectBitcoinMainnetVectors() const
{
    const auto& message_start = Params().MessageStart();
    return message_start[0] == 0xF9 && message_start[1] == 0xBE &&
           message_start[2] == 0xB4 && message_start[3] == 0xD9;
}
...
{
    const bool strict_vectors = ExpectBitcoinMainnetVectors();
    ...
    const auto session_id = cipher.GetSessionID();
    const auto send_garbage = cipher.GetSendGarbageTerminator();
    const auto recv_garbage = cipher.GetReceiveGarbageTerminator();
    if (strict_vectors) {
        BOOST_CHECK(std::ranges::equal(out_session_id, session_id));
        BOOST_CHECK(std::ranges::equal(mid_send_garbage, send_garbage));
        BOOST_CHECK(std::ranges::equal(mid_recv_garbage, recv_garbage));
    } else {
        BOOST_CHECK_EQUAL(session_id.size(), out_session_id.size());
        BOOST_CHECK_EQUAL(send_garbage.size(), mid_send_garbage.size());
        BOOST_CHECK_EQUAL(recv_garbage.size(), mid_recv_garbage.size());
    }
    ...
    if (strict_vectors && !out_ciphertext.empty()) {
        BOOST_CHECK(out_ciphertext == ciphertext);
    } else if (strict_vectors) {
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        if (strict_vectors) {
            ...
            BOOST_CHECK(std::ranges::equal(session_id, dec_cipher.GetSessionID()) == (error != 1));
            BOOST_CHECK(std::ranges::equal(send_garbage, dec_cipher.GetSendGarbageTerminator()) == (error != 1));
            BOOST_CHECK(std::ranges::equal(recv_garbage, dec_cipher.GetReceiveGarbageTerminator()) == (error != 1));
        }
