...
static std::string ConvertLegacyWif(std::string_view legacy_wif)
{
    std::vector<unsigned char> data;
    if (!DecodeBase58Check(std::string{legacy_wif}, data, 34)) return {};
    if (data.size() != 33 && data.size() != 34) return {};
    const bool compressed = data.size() == 34 && data.back() == 1;
    if (data.size() == 34 && data.back() != 1) return {};
...
BOOST_AUTO_TEST_CASE(rpc_next_block_height_or_limit)
{
    BOOST_CHECK_EQUAL(NextBlockHeightOrLimit(0), 1);
    BOOST_CHECK_EQUAL(NextBlockHeightOrLimit(std::numeric_limits<int>::max() - 1), std::numeric_limits<int>::max());
    BOOST_CHECK_EQUAL(NextBlockHeightOrLimit(std::numeric_limits<int>::max()), std::numeric_limits<int>::max());
}
...
BOOST_AUTO_TEST_CASE(rpc_rawsign)
{
    ...
    const std::string output_address = EncodeDestination(PKHash(dest_key.GetPubKey()));
    r = CallRPC(std::string("createrawtransaction ")+prevout+" "+
      "{\"" + output_address + "\":11}");
    std::string notsigned = r.get_str();
    const std::string converted_privkey1 = ConvertLegacyWif("KzsXybp9jX64P5ekX1KUxRQ79Jht9uzW7LorgwE65i5rWACL6LQe");
    const std::string converted_privkey2 = ConvertLegacyWif("Kyhdf5LuKTRx4ge69ybABsiUAWjVRK4XGxAKk2FQLp2HjGMy87Z4");
    BOOST_REQUIRE(!converted_privkey1.empty());
...
BOOST_AUTO_TEST_CASE(rpc_convert_values_pq_wallet_methods)
{
    UniValue result;

    BOOST_CHECK_NO_THROW(result = RPCConvertValues("exportpqkey", {"btx1pqaddress", "slh-dsa-shake-128s"}));
    BOOST_CHECK_EQUAL(result[0].get_str(), "btx1pqaddress");
    BOOST_CHECK_EQUAL(result[1].get_str(), "slh-dsa-shake-128s");

    const std::string pq_keys{"[\"aaaaaaaa\",\"pk_slh(bbbbbbbb)\"]"};
    BOOST_CHECK_NO_THROW(result = RPCConvertValues("addpqmultisigaddress", {"2", pq_keys, "pq-msig", "true"}));
    BOOST_CHECK_EQUAL(result[0].getInt<int>(), 2);
    BOOST_CHECK(result[1].isArray());
    BOOST_CHECK_EQUAL(result[1].size(), 2U);
    BOOST_CHECK_EQUAL(result[1][0].get_str(), "aaaaaaaa");
...
BOOST_AUTO_TEST_CASE(help_example)
{
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", args), "> btx-cli -named test foo=bar b=true n=1\n");
    BOOST_CHECK_EQUAL(HelpExampleRpcNamed("test", args), "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", \"method\": \"test\", \"params\": {\"foo\":\"bar\",\"b\":true,\"n\":1}}' -H 'content-type: application/json' http://127.0.0.1:19334/\n");

    // test shell escape
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b'ar"}}), "> btx-cli -named test foo='b'''ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b\"ar"}}), "> btx-cli -named test foo='b\"ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b ar"}}), "> btx-cli -named test foo='b ar'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", obj_value}}), "> btx-cli -named test name='{\"foo\":\"bar\",\"b\":false,\"n\":1}'\n");
    BOOST_CHECK_EQUAL(HelpExampleRpcNamed("test", {{"name", obj_value}}), "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", \"method\": \"test\", \"params\": {\"name\":{\"foo\":\"bar\",\"b\":false,\"n\":1}}}' -H 'content-type: application/json' http://127.0.0.1:19334/\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", arr_value}}), "> btx-cli -named test name='[\"bar\",false,1]'\n");
    BOOST_CHECK_EQUAL(HelpExampleRpcNamed("test", {{"name", arr_value}}), "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", \"method\": \"test\", \"params\": {\"name\":[\"bar\",false,1]}}' -H 'content-type: application/json' http://127.0.0.1:19334/\n");
