...
static CKey DecodeWIFAnyPrefix(const std::string& wif)
{
    std::vector<unsigned char> data;
    if (!DecodeBase58Check(wif, data, 64)) return {};
    if (data.size() != 33 && data.size() != 34) return {};

    bool compressed{false};
    if (data.size() == 34) {
        if (data.back() != 0x01) return {};
        data.pop_back();
        compressed = true;
    }
...
BOOST_AUTO_TEST_CASE(bloom_create_insert_key)
{
    ...
    if (!key.IsValid()) {
        key = DecodeWIFAnyPrefix(strSecret);
    }
    BOOST_REQUIRE_MESSAGE(key.IsValid(), "Failed to decode test private key");
...
BOOST_AUTO_TEST_CASE(merkle_block_2)
{
    ...
    BOOST_REQUIRE(DecodeHexBlkCompat(block, HexStr(stream)));
...
BOOST_AUTO_TEST_CASE(merkle_block_3_and_serialize)
{
    ...
    // BTX serializes headers with KAWPOW fields; verify stable roundtrip instead of
    // comparing to legacy Bitcoin wire bytes.
    CMerkleBlock decoded_merkle_block;
    merkleStream >> decoded_merkle_block;
    BOOST_CHECK(decoded_merkle_block.header.GetHash() == merkleBlock.header.GetHash());
    BOOST_CHECK(decoded_merkle_block.txn.GetNumTransactions() == merkleBlock.txn.GetNumTransactions());
}
