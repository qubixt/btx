    ...
    for (int decisions = 0; decisions < (1 << 4); ++decisions) {
        ...
        if (parent_in_mempool) {
            ...
            BOOST_CHECK_MESSAGE(mempool_result.m_result_type == MempoolAcceptResult::ResultType::VALID,
                                strprintf("ProcessTransaction failed: result=%d reason=%s",
                                          int(mempool_result.m_result_type),
