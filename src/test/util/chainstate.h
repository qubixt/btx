    ...
    bool in_memory_chainstate = false)
{
    ...
    if (reset_chainstate) {
        {
            ...
            // NOTE: perhaps we could improve this by using pruning to delete
    ...
    //
    // NOTE this is a unittest-specific hack, and we should probably rethink how to
    ...
    if (!res) {
        LogPrintf("CreateAndActivateUTXOSnapshot: ActivateSnapshot failed: %s\n",
                  util::ErrorString(res).original);
        std::fprintf(stderr,
                     "CreateAndActivateUTXOSnapshot: ActivateSnapshot failed: %s\n",
