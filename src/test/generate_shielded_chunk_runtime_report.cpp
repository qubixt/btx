...
using btx::test::shieldedv2chunk::RuntimeReportConfig;
...
RuntimeReportConfig ParseArgs(int argc, char** argv, fs::path& output_path)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help") {
            std::cout << "Usage: gen_shielded_chunk_runtime_report [--warmup=N] [--samples=N] [--outputs=N] [--outputs-per-chunk=N] [--output=/path/report.json]\n";
            std::exit(0);
        }
        ...
        if (arg.starts_with("--outputs=")) {
            config.output_count = ParsePositiveSize(arg.substr(10), "--outputs", /*allow_zero=*/false);
            continue;
        }
        if (arg.starts_with("--outputs-per-chunk=")) {
            config.outputs_per_chunk =
                ParsePositiveSize(arg.substr(20), "--outputs-per-chunk", /*allow_zero=*/false);
            continue;
        }
...
int main(int argc, char** argv)
{
    try {
        ...
        const std::string json = btx::test::shieldedv2chunk::BuildRuntimeReport(config).write(2) + '\n';
        ...
        std::cerr << "gen_shielded_chunk_runtime_report: " << e.what() << '\n';
        return 1;
    }
}
