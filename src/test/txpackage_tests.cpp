...
BOOST_AUTO_TEST_CASE(package_sanitization_tests)
{
    ...
    // Pick a placeholder transaction shape that is heavy enough for this
    // build's weight rules so the package crosses the weight limit before it
    // crosses MAX_PACKAGE_COUNT.
    size_t io_count{600};
    CTransactionRef large_ptx = create_placeholder_tx(io_count, io_count);
    auto size_large = GetTransactionWeight(*large_ptx);
    while (size_large * MAX_PACKAGE_COUNT <= MAX_PACKAGE_WEIGHT) {
        io_count *= 2;
        large_ptx = create_placeholder_tx(io_count, io_count);
        size_large = GetTransactionWeight(*large_ptx);
    }
...
BOOST_AUTO_TEST_CASE(package_validation_tests)
{
    ...
    // With DEFAULT_ANCESTOR_SIZE_LIMIT_KVB=1024, need a very large tx to exceed the limit.
    CTransactionRef giant_ptx = create_placeholder_tx(6000, 6000);
    ...
    if (auto err_single_large{CheckPackageMempoolAcceptResult(package_single_giant, result_single_large, /*expect_valid=*/false, nullptr)}) {
        ...
        const auto& reject_reason{it_giant_tx->second.m_state.GetRejectReason()};
...
BOOST_AUTO_TEST_CASE(noncontextual_package_tests)
{
    ...
    // 24 Parents and 1 Child
    {
        ...
        for (int i{0}; i < 24; ++i) {
            const CAmount parent_value{m_coinbase_txns[i + 1]->vout[0].nValue - COIN};
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, parent_value, false));
...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
    ...
    for (size_t i{0}; i < 10; ++i) {
        const CAmount output_amount{m_coinbase_txns[i + 25]->vout[0].nValue - COIN};
        auto mtx = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[i + 25], /*input_vout=*/0,
                                                 /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                 /*output_destination=*/parent_locking_script,
                                                 /*output_amount=*/output_amount, /*submit=*/false);
    ...
    const CAmount parent_value{m_coinbase_txns[0]->vout[0].nValue - COIN};
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/parent_locking_script,
                                                    /*output_amount=*/parent_value, /*submit=*/false);
    ...
    // missing inputs, so the package validation isn't expected to happen.
    {
        ...
        if (auto err_parent_invalid{CheckPackageMempoolAcceptResult(package_invalid_parent, result_quit_early, /*expect_valid=*/false, m_node.mempool.get())}) {
            ...
            const auto parent_result{it_parent->second.m_state.GetResult()};
            const auto& parent_reject{it_parent->second.m_state.GetRejectReason()};
            BOOST_CHECK(parent_result == TxValidationResult::TX_WITNESS_MUTATED ||
                        parent_result == TxValidationResult::TX_CONSENSUS ||
                        parent_result == TxValidationResult::TX_NOT_STANDARD);
            BOOST_CHECK(parent_reject == "bad-witness-nonwitness-input" ||
                        parent_reject.find("Witness provided for non-witness script") != std::string::npos);
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    const CAmount coinbase_value_single{m_coinbase_txns[0]->vout[0].nValue};
    const CAmount single_value{coinbase_value_single - COIN};
    ...
    auto mtx_single = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    ...
                                                    /*output_amount=*/single_value, /*submit=*/false);
    ...
    const CAmount coinbase_value_parent{m_coinbase_txns[1]->vout[0].nValue};
    const CAmount parent_value{coinbase_value_parent - high_fee};
    ...
    const CAmount child_value{parent_value - high_fee};
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   /*input_height=*/101, /*input_signing_key=*/parent_key,
                                                   /*output_destination=*/child_locking_script,
                                                   /*output_amount=*/child_value, /*submit=*/false);
    ...
    auto mtx_single_low_fee = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/single_locking_script,
                                                    /*output_amount=*/single_value - 1, /*submit=*/false);
...
BOOST_AUTO_TEST_CASE(package_witness_swap_tests)
{
    ...
    const CAmount parent1_value{m_coinbase_txns[1]->vout[0].nValue - COIN};
    ...
    const CAmount grandparent2_value{m_coinbase_txns[2]->vout[0].nValue - COIN};
    auto mtx_grandparent2 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                          ...
                                                          /*output_amount=*/grandparent2_value, /*submit=*/true);
    ...
    const CAmount parent2_value{grandparent2_value - COIN};
    ...
    const CAmount parent3_value{m_coinbase_txns[3]->vout[0].nValue - low_fee_amt};
    ...
    mtx_mixed_child.vout.emplace_back(parent1_value + parent2_value + parent3_value - COIN + low_fee_amt, mixed_child_spk);
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    // Pick child fees that are high enough for the child transaction by itself, but still
    // leave the parent+child package just below the current mempool minimum feerate.
    ...
    auto mtx_child_probe = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent_cheap, /*input_vout=*/0,
                                                         /*input_height=*/101, /*input_signing_key=*/child_key,
                                                         /*output_destination=*/child_spk,
                                                         /*output_amount=*/coinbase_value - parent_fee - 1, /*submit=*/false);
    const CTransactionRef tx_child_probe = MakeTransactionRef(mtx_child_probe);
    const CAmount child_min_fee =
        m_node.mempool->GetMinFee().GetFee(GetVirtualTransactionSize(*tx_child_probe));
    const CAmount package_min_fee =
        m_node.mempool->GetMinFee().GetFee(GetVirtualTransactionSize(*tx_parent_cheap) +
                                           GetVirtualTransactionSize(*tx_child_probe));
    BOOST_REQUIRE_GT(package_min_fee - parent_fee, child_min_fee);
    const CAmount child_fee{package_min_fee - parent_fee - 1};
    BOOST_REQUIRE_GE(child_fee, child_min_fee);
...
BOOST_AUTO_TEST_CASE(package_rbf_tests)
{
    ...
    // Test package rbf.
    {
        ...
        CTransactionRef tx_child_3 = MakeTransactionRef(CreateValidMempoolTransaction(
            ...
            child_key, child_spk, coinbase_value - 199 - 2600, /*submit=*/false));
        ...
        // 1 parent paying 199sat, 1 child paying 2600sat.
        ...
        BOOST_CHECK_EQUAL(it_parent_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 199 + 2600);
        BOOST_CHECK_EQUAL(it_child_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 199 + 2600);
        ...
        // Keep a cushion above incremental relay requirements to avoid flaky
        // anti-DoS fee threshold changes.
        m_node.mempool->PrioritiseTransaction(tx_child_1->GetHash(), 3000);
