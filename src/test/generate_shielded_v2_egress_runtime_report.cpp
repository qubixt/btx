...
using btx::test::shieldedv2egress::RuntimeReportConfig;
using btx::test::shieldedv2egress::RuntimeScenarioConfig;
...
std::vector<RuntimeScenarioConfig> ParseScenarioList(std::string_view value)
{
    ...
    while (!value.empty()) {
        ...
        if (separator == std::string_view::npos || separator == 0 || separator + 1 >= token.size()) {
            throw std::runtime_error("scenario must be formatted as <outputs>x<outputs_per_chunk>");
        }
...
RuntimeReportConfig ParseArgs(int argc, char** argv, fs::path& output_path)
{
    ...
    config.scenarios = {
        RuntimeScenarioConfig{32, 32},
        RuntimeScenarioConfig{1300, 32},
        RuntimeScenarioConfig{5000, 32},
    };

    for (int i = 1; i < argc; ++i) {
        const std::string_view arg{argv[i]};
        if (arg == "--help") {
            std::cout << "Usage: gen_shielded_v2_egress_runtime_report "
                         "[--warmup=N] [--samples=N] "
                         "[--scenarios=32x32,1300x32,5000x32] "
...
int main(int argc, char** argv)
{
    try {
        ...
        const std::string json = btx::test::shieldedv2egress::BuildRuntimeReport(config).write(2) + '\n';
        ...
        std::cerr << "gen_shielded_v2_egress_runtime_report: " << e.what() << '\n';
        return 1;
    }
}
