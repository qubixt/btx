...
BOOST_FIXTURE_TEST_CASE(miniminer_1p1c, TestChain100Setup)
{
    ...
    const auto manual_sequences = miniminer_manual.Linearize();
    const auto pool_sequences = miniminer_pool.Linearize();
    BOOST_CHECK(manual_sequences == pool_sequences);

    // tx6 has the strongest standalone feerate and should still lead.
    BOOST_CHECK_EQUAL(Find(pool_sequences, tx6->GetHash()), 0);

    // tx2 + tx3 and tx4 + tx5 remain package-selected together.
    BOOST_CHECK_EQUAL(Find(pool_sequences, tx2->GetHash()), Find(pool_sequences, tx3->GetHash()));
    BOOST_CHECK_EQUAL(Find(pool_sequences, tx4->GetHash()), Find(pool_sequences, tx5->GetHash()));
    BOOST_CHECK_LT(Find(pool_sequences, tx2->GetHash()), Find(pool_sequences, tx4->GetHash()));

    // The tx0 -> tx1 chain now mines in two steps; preserve that ordering.
    BOOST_CHECK_LT(Find(pool_sequences, tx4->GetHash()), Find(pool_sequences, tx0->GetHash()));
    BOOST_CHECK_LT(Find(pool_sequences, tx0->GetHash()), Find(pool_sequences, tx1->GetHash()));
    BOOST_CHECK_LT(Find(pool_sequences, tx1->GetHash()), Find(pool_sequences, tx7->GetHash()));
}
 ...
 *  -> tx7_ASFR = (high+low+high) / (tx4_vsize + tx5_vsize + tx7_vsize) */
    ...
    for (const auto& tx : all_transactions) {
        tx_vsizes.push_back(Assert(pool.GetEntry(tx->GetHash()))->GetTxSize());
    }
    ...
    // Feerate just below tx4: tx6 and tx7 have different bump fees.
    {
        ...
        BOOST_CHECK(!mini_miner_tx6_tx7.IsReadyToCalculate());
        BOOST_CHECK(tx6_tx7_bumpfee.has_value());
        BOOST_CHECK(std::llabs(tx6_tx7_bumpfee.value() -
                               (just_below_tx4.GetFee(tx_vsizes[5] + tx_vsizes[6]) - (low_fee + med_fee))) <= 1);
    }
    ...
    for (const auto& sequences : {miniminer_manual.Linearize(), miniminer_pool.Linearize()}) {
        ...
        // (tie broken deterministically; either ordering is acceptable).
        const auto seq_tx4{Find(sequences, tx4->GetHash())};
        const auto seq_tx2{Find(sequences, tx2->GetHash())};
        BOOST_CHECK_MESSAGE((seq_tx4 == 0 && seq_tx2 == 1) || (seq_tx4 == 1 && seq_tx2 == 0),
                            strprintf("Unexpected order for top-scoring txs: tx4=%u tx2=%u", seq_tx4, seq_tx2));
...
BOOST_FIXTURE_TEST_CASE(calculate_cluster, TestChain100Setup)
{
    ...
    // NOTE this can be removed once the mempool interface uses Txid, Wtxid
