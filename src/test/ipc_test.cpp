...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_btx", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_btx", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_btx", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_btx", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_Bitcoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_btx.sock"' exceeded maximum socket path length
    const std::string mkstemp_suffix(6, 'X');
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath(std::string("bitcoin_sock0_") + mkstemp_suffix)),
        strprintf("unix:%s", TempPath(std::string("bitcoin_sock1_") + mkstemp_suffix)),
    };
