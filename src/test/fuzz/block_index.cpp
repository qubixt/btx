...
// Hardcoded PoW-related fields to make sure the blocks we store pass the PoW check.
// BTX uses MatMul PoW, so the MatMul phase 1 checks must also be satisfied.
...
CBlockHeader ConsumeBlockHeader(FuzzedDataProvider& provider)
{
    ...
    // MatMul PoW requires non-null seeds, a fixed dimension, and a digest below target.
    // Keep the digest at 0 to satisfy the target check while still exercising DB parsing.
    const auto& consensus{Params().GetConsensus()};
    header.matmul_dim = consensus.nMatMulDimension;
    header.seed_a = g_block_hash;
    header.seed_b = g_block_hash;
