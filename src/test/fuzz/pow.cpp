...
FUZZ_TARGET(pow, .init = initialize_pow)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.remaining_bytes() > 0, 10000) {
        CBlockIndex* previous_block = nullptr;
        if (!blocks.empty()) {
            previous_block = PickValue(fuzzed_data_provider, blocks).get();
        }
        ...
        auto current_block_ptr = std::make_unique<CBlockIndex>(*block_header);
        CBlockIndex& current_block{*current_block_ptr};
        {
            ...
            const bool link_to_previous = previous_block != nullptr && fuzzed_data_provider.ConsumeBool();
            if (link_to_previous) {
            ...
            if (fuzzed_data_provider.ConsumeBool()) {
                current_block.nChainWork = link_to_previous ? previous_block->nChainWork + GetBlockProof(*previous_block) : arith_uint256{0};
            } else {
                current_block.nChainWork = ConsumeArithUInt256(fuzzed_data_provider);
            }
        }
        blocks.emplace_back(std::move(current_block_ptr));
        {
...
FUZZ_TARGET(pow_transition, .init = initialize_pow)
{
    ...
    // Keep the generated compact target in the same valid domain used by
    // PermittedDifficultyTransition() to avoid asserting on malformed nBits.
    if (!DeriveTarget(nbits, consensus_params.powLimit).has_value()) {
