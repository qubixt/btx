                           ...
                           const Consensus::Params& consensus)
{
    CBlockHeader header;
    header.nNonce = 0;
    // BTX fuzz builds run with MatMul PoW. Reusing previous nBits keeps header
    // mining bounded and deterministic for this harness.
    if (consensus.fMatMulPOW || fuzzed_data_provider.ConsumeBool()) {
        header.nBits = prev_nbits;
    } else {
        // Non-MatMul fallback: fuzz a realistic target range from historical
        // Bitcoin mainnet difficulties.
...
CBlock ConsumeBlock(FuzzedDataProvider& fuzzed_data_provider,
                    ...
                    const Consensus::Params& consensus)
{
    auto header = ConsumeHeader(fuzzed_data_provider, prev_hash, prev_nbits, consensus);
...
void FinalizeHeader(CBlockHeader& header, uint32_t block_height, const ChainstateManager& chainman)
{
    assert(MineHeaderForConsensus(header, block_height, chainman.GetParams().GetConsensus()));
}
...
void initialize()
{
    static auto setup = MakeNoLogFileContext<HeadersSyncSetup>(
        ChainType::REGTEST,
        {.extra_args = {
             "-checkpoints=0",
             // If accepted by runtime args, keep the anti-DoS work threshold
             // above any fuzzer-generated chain work.
             "-minimumchainwork=0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
         }});
...
FUZZ_TARGET(p2p_headers_presync, .init = initialize)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 100)
    {
        auto finalized_block = [&]() {
            CBlock block = ConsumeBlock(fuzzed_data_provider, base.GetHash(), base.nBits, chainman.GetParams().GetConsensus());
            FinalizeHeader(block, base_height + 1, chainman);
            return block;
        };
        ...
        CallOneOf(
            ...
            [&]() NO_THREAD_SAFETY_ANALYSIS {
                ...
                for (CBlock& header : headers) {
                    header = ConsumeHeader(fuzzed_data_provider, base.GetHash(), base.nBits, chainman.GetParams().GetConsensus());
                    FinalizeHeader(header, base_height + 1, chainman);
                    base = header;
                    ++base_height;
                }
    ...
    // Forks under active development may set MinimumChainWork to zero until launch.
    // In that case, this anti-DoS threshold does not apply and the presync-specific
    // assumptions below are intentionally skipped.
    if (chainman.MinimumChainWork() > 0) {
