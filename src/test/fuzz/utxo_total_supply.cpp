...
FUZZ_TARGET(utxo_total_supply)
{
    ...
    constexpr int64_t REGTEST_GENESIS_TIME{1296688602};
    // Keep fuzzed mock-time below software expiry to avoid irrelevant
    // startup-expiry errors when exercising UTXO supply logic.
    const int64_t max_mock_time{std::max<int64_t>(REGTEST_GENESIS_TIME, DEFAULT_SOFTWARE_EXPIRY - 1)};
    const auto mock_time{ConsumeTime(fuzzed_data_provider, REGTEST_GENESIS_TIME, max_mock_time)};
    ...
    /** Maximum allowed UTXO supply: initial chainstate supply + mined subsidies. */
    CAmount supply_cap{0};
    bool supply_cap_initialized{false};
    ...
    const auto UpdateUtxoStats = [&]() {
        ...
        const CAmount total_amount{*utxo_stats.total_amount};
        // Miner output should never exceed the expected max supply.
        // Equality is too strict here: a valid block may burn fees by underclaiming in coinbase.
        if (supply_cap_initialized && total_amount > supply_cap) {
            std::cerr << "utxo_total_supply mismatch: height=" << ActiveHeight()
    ...
    // Keep the duplicate-coinbase height within the practical mutation budget so
    // this check is exercised regularly in corpus-driven runs.
    constexpr int64_t MAX_DUPLICATE_COINBASE_HEIGHT{32};
    int64_t duplicate_coinbase_height = fuzzed_data_provider.ConsumeIntegralInRange<int64_t>(0, MAX_DUPLICATE_COINBASE_HEIGHT);
    ...
    // Bound per-input work so large corpora do not cause timeout-prone tails.
    // This target mines and flushes chainstate repeatedly, so keep the step
    // budget conservative for deterministic CI runtime.
    constexpr int MAX_MUTATION_STEPS{16};
    LIMITED_WHILE(fuzzed_data_provider.remaining_bytes(), MAX_MUTATION_STEPS)
    {
        CallOneOf(
