...
FUZZ_TARGET(script, .init = initialize_script)
{
...

    {
        ...
        if (!std::get_if<PubKeyDestination>(&tx_destination_1)) {
            ...
            // Some destinations normalize across type boundaries during decode
            // (e.g. witness unknown v2/32-byte program -> WitnessV2P2MR).
            // Only assert encode/decode roundtrip when decode yields a valid destination.
            const CTxDestination decoded_dest{DecodeDestination(encoded_dest)};
            if (IsValidDestination(decoded_dest)) {
                Assert(encoded_dest == EncodeDestination(decoded_dest));
            }
