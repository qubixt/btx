...
void Finish(FuzzedDataProvider& fuzzed_data_provider, MockedTxPool& tx_pool, Chainstate& chainstate)
{
    WITH_LOCK(::cs_main, tx_pool.check(chainstate.CoinsTip(), chainstate.m_chain.Height() + 1));
    {
        ...
        try {
            ...
            // Fuzzed mempool entries can violate BTX consensus constraints
            // (e.g. scriptpubkey-size), which CreateNewBlock surfaces as
            // runtime_error. Treat this as expected fuzz input rejection.
        }
    }
...
FUZZ_TARGET(tx_pool_standard, .init = initialize_tx_pool)
{
    ...
    // Track supply using the actual matured coinbase set for the active chain
    // parameters instead of assuming a fixed subsidy.
    ...
    {
        {
            // Total supply is the mempool fee + all tracked spendable outpoints.
            CAmount check_supply{WITH_LOCK(tx_pool.cs, return tx_pool.GetTotalFee())};
            size_t missing_in_rbf{0};
            size_t missing_in_view{0};
            for (const auto& op : outpoints_supply) {
                if (!outpoints_rbf.contains(op)) {
                    ++missing_in_rbf;
                }
                const auto coin{amount_view.GetCoin(op)};
                if (!coin.has_value()) {
                    ++missing_in_view;
                    continue;
                }
                check_supply += coin->out.nValue;
            }
            if (check_supply != supply_total) {
                // Keep this explicit mismatch branch for easier fuzz debugging.
                Assert(false);
            }
            Assert(missing_in_rbf == 0);
            Assert(missing_in_view == 0);
        }
        ...
        // Add created outpoints, remove spent outpoints
        {
            ...
            // Outpoints restored because mempool txs were removed
            std::set<COutPoint> restored_supply;
            for (const auto& removed_tx : removed) {
                insert_tx(/*created_by_tx=*/{consumed_erased}, /*consumed_by_tx=*/{restored_supply}, /*tx=*/*removed_tx);
            }
            for (const auto& added_tx : added) {
                insert_tx(/*created_by_tx=*/{outpoints_supply, outpoints_rbf}, /*consumed_by_tx=*/{consumed_supply}, /*tx=*/*added_tx);
            }
            for (const auto& p : restored_supply) {
                Assert(outpoints_supply.insert(p).second);
            }
            ...
            for (const auto& p : outpoints_supply) {
                Assert(outpoints_rbf.contains(p));
            }
        }
    }
