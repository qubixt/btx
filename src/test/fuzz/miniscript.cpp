...
struct SmartInfo
{
    ...
    void Init(std::map<Type, std::vector<recipe>>& table, MsCtx script_ctx)
    {
        ...
        for (int fragidx = 0; fragidx <= int(Fragment::MULTI_A); ++fragidx) {
            ...
            const bool p2mr_only_fragment{
                frag == Fragment::PK_MLDSA ||
                frag == Fragment::PK_SLHDSA ||
                frag == Fragment::MULTI_MLDSA ||
                frag == Fragment::MULTI_SLHDSA
            };
            if ((p2mr_only_fragment && !miniscript::IsP2MR(script_ctx)) ||
                (!miniscript::IsTapscript(script_ctx) && frag == Fragment::MULTI_A) ||
