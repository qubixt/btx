...
void AssertTrackedOutpointsHaveValues(const std::set<COutPoint>& mempool_outpoints, const std::map<COutPoint, CAmount>& outpoints_value)
{
    for (const auto& outpoint : mempool_outpoints) {
        Assert(outpoints_value.find(outpoint) != outpoints_value.end());
    }
}
...
FUZZ_TARGET(ephemeral_package_eval, .init = initialize_tx_pool)
{
    ...
    for (const auto& outpoint : g_outpoints_coinbase_init_mature) {
        ...
        const auto coin{amount_view.GetCoin(outpoint)};
        Assert(coin.has_value());
        outpoints_value[outpoint] = coin->out.nValue;
    }
    AssertTrackedOutpointsHaveValues(mempool_outpoints, outpoints_value);
            ...
            txs.emplace_back([&] {
                ...
                for (int i = 0; i < num_in; ++i) {
                    ...
                    if (i == 0 && outpoint_to_rbf) {
                        ...
                        Assert(outpoints.erase(outpoint) == 1);
                    } else {
                        outpoints.erase(pop);
                    }
                    const auto value_it{outpoints_value.find(outpoint)};
                    Assert(value_it != outpoints_value.end());
                    amount_in += value_it->second;
