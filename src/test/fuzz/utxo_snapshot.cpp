...
void utxo_snapshot_fuzz(FuzzBufferType buffer)
{
    ...
    // Keep fuzzed mock-time below the software-expiry guard so corpus replay
    // cannot spam expiry errors unrelated to snapshot logic.
    const int64_t max_mock_time{std::max<int64_t>(REGTEST_GENESIS_TIME, DEFAULT_SOFTWARE_EXPIRY - 1)};
    SetMockTime(ConsumeTime(fuzzed_data_provider, REGTEST_GENESIS_TIME, max_mock_time));
    auto& setup{*g_setup};
    ...
    if (fuzzed_data_provider.ConsumeBool()) {
        ...
        if constexpr (!INVALID) {
            for (const auto& block : *g_chain) {
                ...
                bool processed{chainman.ProcessNewBlockHeaders({{block->GetBlockHeader()}}, true, dummy)};
                if (!processed) {
                    // Header admission can fail for non-standard test chainparams.
                    // Mark chainman dirty and stop mutating this run.
                    dirty_chainman = true;
                    break;
                }
                const auto* index{WITH_LOCK(::cs_main, return chainman.m_blockman.LookupBlockIndex(block->GetHash()))};
