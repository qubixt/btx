...
struct FuzzedWallet {
    ...
    {
        static constexpr std::array<OutputType, 5> DESTINATION_TYPES{
            OutputType::P2MR,
            OutputType::BECH32M,
            OutputType::BECH32,
        ...
        const OutputType preferred_type{fuzzed_data_provider.PickValueInArray(DESTINATION_TYPES)};
        auto try_get_destination = [&](OutputType type) {
            return external ? wallet->GetNewDestination(type, "") : wallet->GetNewChangeDestination(type);
        };

        if (auto destination{try_get_destination(preferred_type)}) {
            return *destination;
        }
        for (OutputType fallback_type : DESTINATION_TYPES) {
            if (auto destination{try_get_destination(fallback_type)}) {
                return *destination;
            }
        }
    ...
    void FundTx(FuzzedDataProvider& fuzzed_data_provider, CMutableTransaction tx)
    {
        ...
        CallOneOf(
            ...
            [&] {
                static constexpr std::array<OutputType, 5> CHANGE_TYPES{
                    OutputType::P2MR,
                    OutputType::BECH32M,
                    OutputType::BECH32,
                    OutputType::P2SH_SEGWIT,
                ...
                coin_control.m_change_type.emplace(fuzzed_data_provider.PickValueInArray(CHANGE_TYPES));
            },
