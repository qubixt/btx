...
using btx::test::shieldedv2scan::RuntimeReportConfig;
...
RuntimeReportConfig ParseArgs(int argc, char** argv, fs::path& output_path)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help") {
            std::cout << "Usage: gen_shielded_scan_hint_runtime_report [--warmup=N] [--samples=N] [--candidates=N] [--output=/path/report.json]\n";
            std::exit(0);
        }
        ...
        if (arg.starts_with("--candidates=")) {
            config.minimum_candidate_keys = ParsePositiveSize(arg.substr(13), "--candidates", /*allow_zero=*/false);
            continue;
        }
...
int main(int argc, char** argv)
{
    try {
        ...
        const std::string json = btx::test::shieldedv2scan::BuildRuntimeReport(config).write(2) + '\n';
        ...
        std::cerr << "gen_shielded_scan_hint_runtime_report: " << e.what() << '\n';
        return 1;
    }
}
