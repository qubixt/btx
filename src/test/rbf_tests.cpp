...
static CTxMemPool::ChangeSet::TxHandle RBFTestStageAddition(CTxMemPool::ChangeSet& changeset,
                                                            const CTransactionRef& tx,
                                                            const CAmount fee,
                                                            const unsigned int sigops_cost = 4) {
    return changeset.StageAddition(tx, fee, 0, 1, 0, COIN_AGE_CACHE_ZERO, false, /*extra_weight=*/0, sigops_cost, LockPoints());
}
...
BOOST_FIXTURE_TEST_CASE(rbf_helper_functions, TestChain100Setup)
{
    ...
    BOOST_CHECK(!violates_policy);
    // NOTE: Add tests for policy-only conflicts
    ...
    {
        const auto topo_err{pool.CheckConflictTopology(set_34_cpfp)};
        BOOST_REQUIRE(topo_err.has_value());
        BOOST_CHECK_MESSAGE(topo_err.value() == strprintf("%s has 23 descendants, max 1 allowed", entry4_high->GetSharedTx()->GetHash().ToString()) ||
                                topo_err.value() == strprintf("%s has 24 descendants, max 1 allowed", entry3_low->GetSharedTx()->GetHash().ToString()),
                            topo_err.value());
    }
...
BOOST_FIXTURE_TEST_CASE(improves_feerate, TestChain100Setup)
{
    ...
    // With fewer sigops-adjusted vbytes it does
    CMutableTransaction tx4{entry3.GetTx()};
    tx4.vin[0].scriptWitness = CScriptWitness();
    const auto tx4_ref = MakeTransactionRef(tx4);
    changeset = pool.GetChangeSet();
    changeset->StageRemoval(entry1);
    changeset->StageRemoval(entry2);
    RBFTestStageAddition(*changeset, tx1_conflict, tx1_fee);
    RBFTestStageAddition(*changeset, tx4_ref, tx2_fee, /*sigops_cost=*/0);
    BOOST_CHECK(ImprovesFeerateDiagram(*changeset) == std::nullopt);
    changeset.reset();
    ...
    RBFTestStageAddition(*changeset, tx1_conflict, tx1_fee);
    RBFTestStageAddition(*changeset, tx4_ref, tx2_fee + entry5->GetModifiedFee() + 1, /*sigops_cost=*/0);
    ...
    BOOST_CHECK(res3.value().second.find("max 1 allowed") != std::string::npos ||
                res3.value().second.find("exceeding cluster limit") != std::string::npos);
}
...
BOOST_FIXTURE_TEST_CASE(calc_feerate_diagram_rbf, TestChain100Setup)
{
...

    {
        ...
        {
            const auto err{util::ErrorString(replace_too_large).original};
            BOOST_CHECK_MESSAGE(err.find("max 1 allowed") != std::string::npos ||
                                    err.find("exceeding cluster limit") != std::string::npos,
                                err);
        }
    }
...

    {
        ...
        const auto err_msg{util::ErrorString(replace_cluster_size_3).original};
        BOOST_CHECK(
            err_msg.find("exceeding cluster limit of 2") != std::string::npos ||
            err_msg.find("max 1 allowed") != std::string::npos
        );
    }
}
