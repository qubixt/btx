...
BOOST_AUTO_TEST_CASE(util_GetChainTypeString)
{
    ...
    const auto shieldedv2dev = std::make_pair("-shieldedv2dev", ArgsManager::ALLOW_ANY);
    test_args.SetupArgs({testnet, testnet4, regtest, shieldedv2dev});

    const char* argv_testnet[] = {"cmd", "-testnet"};
    const char* argv_testnet4[] = {"cmd", "-testnet4"};
    const char* argv_regtest[] = {"cmd", "-regtest"};
    const char* argv_shieldedv2dev[] = {"cmd", "-shieldedv2dev"};
    const char* argv_test_no_reg[] = {"cmd", "-testnet", "-noregtest"};
    const char* argv_both[] = {"cmd", "-testnet", "-regtest"};
    const char* argv_reg_and_v2dev[] = {"cmd", "-regtest", "-shieldedv2dev"};
    ...
    BOOST_CHECK(test_args.ParseParameters(2, argv_shieldedv2dev, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "shieldedv2dev");
    ...
    BOOST_CHECK(test_args.ParseParameters(3, argv_reg_and_v2dev, error));
    BOOST_CHECK_THROW(test_args.GetChainTypeString(), std::runtime_error);
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_btx --run_test=argsman_tests/util_ArgsMerge
    //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_btx --run_test=argsman_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
    //
    //   <input> || <output>
    BOOST_CHECK_EQUAL(out_sha_hex, "d41220d0dc252c1aade0d3e7d636bb33ab1e705d0a396e7d1f5ec1712e29979f");
}
