...
/** Raw-byte key converter for P2MR PQ miniscript fragment testing. */
struct PQKeyConverter {
    using Key = std::vector<unsigned char>;

    explicit constexpr PQKeyConverter(miniscript::MiniscriptContext ctx) noexcept : m_script_ctx{ctx} {}

    bool KeyCompare(const Key& a, const Key& b) const { return a < b; }

    std::vector<unsigned char> ToPKBytes(const Key& key) const { return key; }

    std::vector<unsigned char> ToPKHBytes(const Key&) const { return {}; }

    template<typename I>
    std::optional<Key> FromString(I first, I last) const
    {
        std::vector<unsigned char> bytes = ParseHex(std::string(first, last));
    ...
    template<typename I>
    std::optional<Key> FromPKHBytes(I, I) const
    {
        return {};
    }
    ...
    miniscript::MiniscriptContext MsContext() const { return m_script_ctx; }

private:
    miniscript::MiniscriptContext m_script_ctx;
};

/** Satisfier for PQ miniscript fragments backed by raw-byte keys. */
struct PQSatisfier : public PQKeyConverter {
    explicit PQSatisfier(miniscript::MiniscriptContext ctx) noexcept : PQKeyConverter{ctx} {}

    std::set<std::vector<unsigned char>> signable_keys;

    bool CheckAfter(uint32_t) const { return false; }
    bool CheckOlder(uint32_t) const { return false; }

    miniscript::Availability SatSHA256(const std::vector<unsigned char>&, std::vector<unsigned char>&) const
    {
        return miniscript::Availability::NO;
    }

    miniscript::Availability SatRIPEMD160(const std::vector<unsigned char>&, std::vector<unsigned char>&) const
    {
        return miniscript::Availability::NO;
    }

    miniscript::Availability SatHASH256(const std::vector<unsigned char>&, std::vector<unsigned char>&) const
    {
        return miniscript::Availability::NO;
    }

    miniscript::Availability SatHASH160(const std::vector<unsigned char>&, std::vector<unsigned char>&) const
    {
        return miniscript::Availability::NO;
    }

    miniscript::Availability Sign(const std::vector<unsigned char>& key, std::vector<unsigned char>& sig) const
    {
        if (!signable_keys.count(key)) return miniscript::Availability::NO;
        if (key.size() == MLDSA44_PUBKEY_SIZE) {
            sig.assign(MLDSA44_SIGNATURE_SIZE + 1, 0x99);
            return miniscript::Availability::YES;
        }
        if (key.size() == SLHDSA128S_PUBKEY_SIZE) {
            sig.assign(SLHDSA128S_SIGNATURE_SIZE + 1, 0x77);
            return miniscript::Availability::YES;
        }
        return miniscript::Availability::NO;
    }
...
BOOST_AUTO_TEST_CASE(p2mr_context_properties)
{
    BOOST_CHECK(miniscript::IsP2MR(miniscript::MiniscriptContext::P2MR));
    BOOST_CHECK(!miniscript::IsTapscript(miniscript::MiniscriptContext::P2MR));
    BOOST_CHECK_EQUAL(miniscript::internal::MaxScriptSize(miniscript::MiniscriptContext::P2MR), MAX_P2MR_SCRIPT_SIZE);
    BOOST_CHECK_GT(miniscript::internal::MaxScriptSize(miniscript::MiniscriptContext::TAPSCRIPT), MAX_P2MR_SCRIPT_SIZE);
}

BOOST_AUTO_TEST_CASE(parse_pk_mldsa_p2mr_fragment)
{
    const PQKeyConverter p2mr_converter{miniscript::MiniscriptContext::P2MR};
    std::vector<unsigned char> mldsa_pubkey(MLDSA44_PUBKEY_SIZE, 0x42);
    const std::string miniscript_expr = "pk_mldsa(" + HexStr(mldsa_pubkey) + ")";

    const auto node = miniscript::FromString(miniscript_expr, p2mr_converter);
    BOOST_REQUIRE(node);
    BOOST_CHECK(node->IsValidTopLevel());

    const CScript expected = CScript{} << mldsa_pubkey << OP_CHECKSIG_MLDSA;
    const CScript script = node->ToScript(p2mr_converter);
    BOOST_CHECK(script == expected);
    const auto rendered = node->ToString(p2mr_converter);
    BOOST_REQUIRE(rendered);
    BOOST_CHECK_EQUAL(*rendered, miniscript_expr);

    const auto decoded = miniscript::FromScript(script, p2mr_converter);
    BOOST_REQUIRE(decoded);
    BOOST_CHECK(decoded->ToScript(p2mr_converter) == script);
}

BOOST_AUTO_TEST_CASE(parse_pk_slhdsa_p2mr_fragment)
{
    const PQKeyConverter p2mr_converter{miniscript::MiniscriptContext::P2MR};
    std::vector<unsigned char> slhdsa_pubkey(SLHDSA128S_PUBKEY_SIZE, 0x24);
    const std::string miniscript_expr = "pk_slhdsa(" + HexStr(slhdsa_pubkey) + ")";

    const auto node = miniscript::FromString(miniscript_expr, p2mr_converter);
    BOOST_REQUIRE(node);
    BOOST_CHECK(node->IsValidTopLevel());

    const CScript expected = CScript{} << slhdsa_pubkey << OP_CHECKSIG_SLHDSA;
    const CScript script = node->ToScript(p2mr_converter);
    BOOST_CHECK(script == expected);
    const auto rendered = node->ToString(p2mr_converter);
    BOOST_REQUIRE(rendered);
...
BOOST_AUTO_TEST_CASE(parse_pk_pq_fragments_require_p2mr_context_and_correct_sizes)
{
    const PQKeyConverter p2mr_converter{miniscript::MiniscriptContext::P2MR};
    const PQKeyConverter tapscript_converter{miniscript::MiniscriptContext::TAPSCRIPT};
    const PQKeyConverter p2wsh_converter{miniscript::MiniscriptContext::P2WSH};

    const std::string mldsa_expr = "pk_mldsa(" + HexStr(std::vector<unsigned char>(MLDSA44_PUBKEY_SIZE, 0x11)) + ")";
    const std::string slhdsa_expr = "pk_slhdsa(" + HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x22)) + ")";
    const std::string wrong_mldsa_expr = "pk_mldsa(" + HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x33)) + ")";
    const std::string wrong_slhdsa_expr = "pk_slhdsa(" + HexStr(std::vector<unsigned char>(MLDSA44_PUBKEY_SIZE, 0x44)) + ")";

    BOOST_CHECK(miniscript::FromString(mldsa_expr, p2mr_converter));
    BOOST_CHECK(miniscript::FromString(slhdsa_expr, p2mr_converter));
    BOOST_CHECK(!miniscript::FromString(mldsa_expr, tapscript_converter));
    BOOST_CHECK(!miniscript::FromString(mldsa_expr, p2wsh_converter));
    BOOST_CHECK(!miniscript::FromString(slhdsa_expr, tapscript_converter));
    BOOST_CHECK(!miniscript::FromString(slhdsa_expr, p2wsh_converter));
    BOOST_CHECK(!miniscript::FromString(wrong_mldsa_expr, p2mr_converter));
    BOOST_CHECK(!miniscript::FromString(wrong_slhdsa_expr, p2mr_converter));
}

BOOST_AUTO_TEST_CASE(parse_multi_mldsa_p2mr_fragment)
{
    const PQKeyConverter p2mr_converter{miniscript::MiniscriptContext::P2MR};
    const std::vector<unsigned char> key1(MLDSA44_PUBKEY_SIZE, 0x01);
    const std::vector<unsigned char> key2(MLDSA44_PUBKEY_SIZE, 0x02);
    const std::vector<unsigned char> key3(MLDSA44_PUBKEY_SIZE, 0x03);
    const std::string miniscript_expr =
        "multi_mldsa(2," + HexStr(key1) + "," + HexStr(key2) + "," + HexStr(key3) + ")";

    const auto node = miniscript::FromString(miniscript_expr, p2mr_converter);
    BOOST_REQUIRE(node);
    BOOST_CHECK(node->IsValidTopLevel());
    BOOST_CHECK_EQUAL(*node->GetWitnessSize(), 2 * (1 + MLDSA44_SIGNATURE_SIZE) + 1);

    const std::vector<unsigned char> expected_vec = BuildP2MRMultisigScript(2, {
        {PQAlgorithm::ML_DSA_44, key1},
        {PQAlgorithm::ML_DSA_44, key2},
        {PQAlgorithm::ML_DSA_44, key3},
    });
    BOOST_REQUIRE(!expected_vec.empty());
    const CScript expected{expected_vec.begin(), expected_vec.end()};
    const CScript script = node->ToScript(p2mr_converter);
    BOOST_CHECK(script == expected);
...
BOOST_AUTO_TEST_CASE(parse_multi_slhdsa_p2mr_fragment)
{
    const PQKeyConverter p2mr_converter{miniscript::MiniscriptContext::P2MR};
    const std::vector<unsigned char> key1(SLHDSA128S_PUBKEY_SIZE, 0x0a);
    const std::vector<unsigned char> key2(SLHDSA128S_PUBKEY_SIZE, 0x0b);
    const std::vector<unsigned char> key3(SLHDSA128S_PUBKEY_SIZE, 0x0c);
    const std::string miniscript_expr =
        "multi_slhdsa(1," + HexStr(key1) + "," + HexStr(key2) + "," + HexStr(key3) + ")";

    const auto node = miniscript::FromString(miniscript_expr, p2mr_converter);
    BOOST_REQUIRE(node);
    BOOST_CHECK(node->IsValidTopLevel());
    BOOST_CHECK_EQUAL(*node->GetWitnessSize(), (1 + SLHDSA128S_SIGNATURE_SIZE) + 2);

    const std::vector<unsigned char> expected_vec = BuildP2MRMultisigScript(1, {
        {PQAlgorithm::SLH_DSA_128S, key1},
        {PQAlgorithm::SLH_DSA_128S, key2},
        {PQAlgorithm::SLH_DSA_128S, key3},
    });
    BOOST_REQUIRE(!expected_vec.empty());
    const CScript expected{expected_vec.begin(), expected_vec.end()};
    const CScript script = node->ToScript(p2mr_converter);
...
BOOST_AUTO_TEST_CASE(parse_multi_pq_fragments_require_p2mr_context_and_valid_sizes)
{
    const PQKeyConverter p2mr_converter{miniscript::MiniscriptContext::P2MR};
    const PQKeyConverter tapscript_converter{miniscript::MiniscriptContext::TAPSCRIPT};
    const PQKeyConverter p2wsh_converter{miniscript::MiniscriptContext::P2WSH};

    const std::string multi_mldsa_expr =
        "multi_mldsa(2," +
        HexStr(std::vector<unsigned char>(MLDSA44_PUBKEY_SIZE, 0x11)) + "," +
        HexStr(std::vector<unsigned char>(MLDSA44_PUBKEY_SIZE, 0x12)) + "," +
        HexStr(std::vector<unsigned char>(MLDSA44_PUBKEY_SIZE, 0x13)) + ")";
    const std::string multi_slhdsa_expr =
        "multi_slhdsa(1," +
        HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x21)) + "," +
        HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x22)) + ")";
    const std::string wrong_size_expr =
        "multi_mldsa(2," +
        HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x31)) + "," +
        HexStr(std::vector<unsigned char>(MLDSA44_PUBKEY_SIZE, 0x32)) + ")";
    const std::string slhdsa_too_many_required_expr =
        "multi_slhdsa(2," +
        HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x41)) + "," +
        HexStr(std::vector<unsigned char>(SLHDSA128S_PUBKEY_SIZE, 0x42)) + ")";

    BOOST_CHECK(miniscript::FromString(multi_mldsa_expr, p2mr_converter));
    BOOST_CHECK(miniscript::FromString(multi_slhdsa_expr, p2mr_converter));
    BOOST_CHECK(!miniscript::FromString(multi_mldsa_expr, tapscript_converter));
    BOOST_CHECK(!miniscript::FromString(multi_mldsa_expr, p2wsh_converter));
    BOOST_CHECK(!miniscript::FromString(multi_slhdsa_expr, tapscript_converter));
    BOOST_CHECK(!miniscript::FromString(multi_slhdsa_expr, p2wsh_converter));
    BOOST_CHECK(!miniscript::FromString(wrong_size_expr, p2mr_converter));
    BOOST_CHECK(!miniscript::FromString(slhdsa_too_many_required_expr, p2mr_converter));
}

BOOST_AUTO_TEST_CASE(parse_script_regression_handles_malformed_fuzz_vector_without_throw)
{
    const std::vector<unsigned char> serialized_script =
        ParseHex("0bff00fff50400fc4cff9cadc95c0421ff");
    DataStream stream{serialized_script};
    CScript script;
    stream >> script;

    const KeyConverter tap_converter{miniscript::MiniscriptContext::TAPSCRIPT};
    const KeyConverter wsh_converter{miniscript::MiniscriptContext::P2WSH};

    BOOST_CHECK_NO_THROW((void)miniscript::FromScript(script, tap_converter));
    BOOST_CHECK_NO_THROW((void)miniscript::FromScript(script, wsh_converter));
}

BOOST_AUTO_TEST_CASE(multi_mldsa_p2mr_satisfaction)
{
    const PQSatisfier satisfier{miniscript::MiniscriptContext::P2MR};
    const std::vector<unsigned char> key1(MLDSA44_PUBKEY_SIZE, 0x61);
    const std::vector<unsigned char> key2(MLDSA44_PUBKEY_SIZE, 0x62);
    const std::vector<unsigned char> key3(MLDSA44_PUBKEY_SIZE, 0x63);
    const std::string miniscript_expr =
        "multi_mldsa(2," + HexStr(key1) + "," + HexStr(key2) + "," + HexStr(key3) + ")";

    auto node = miniscript::FromString(miniscript_expr, satisfier);
    BOOST_REQUIRE(node);

    PQSatisfier with_keys{miniscript::MiniscriptContext::P2MR};
    with_keys.signable_keys.insert(key1);
    with_keys.signable_keys.insert(key3);
    std::vector<std::vector<unsigned char>> witness;
    BOOST_CHECK(node->Satisfy(with_keys, witness, /*nonmalleable=*/true) == miniscript::Availability::YES);
    BOOST_CHECK_EQUAL(witness.size(), 3);
    const auto non_empty = std::count_if(witness.begin(), witness.end(), [](const auto& item) { return !item.empty(); });
