...
static void CheckAncestorSortAllowTie(CTxMemPool& pool,
                                      const std::vector<std::string>& expected_order,
                                      const std::string& tie_a,
                                      const std::string& tie_b) EXCLUSIVE_LOCKS_REQUIRED(pool.cs)
{
    BOOST_CHECK_EQUAL(pool.size(), expected_order.size());
    std::vector<std::string> actual_order;
    actual_order.reserve(pool.size());
    for (const auto& entry : pool.mapTx.get<ancestor_score>()) {
        actual_order.push_back(entry.GetTx().GetHash().ToString());
    }

    BOOST_TEST_CONTEXT("expected=" << boost::join(expected_order, ",") <<
                       " actual=" << boost::join(actual_order, ",")) {
    size_t i{0};
    while (i < expected_order.size()) {
        if (actual_order[i] == expected_order[i]) {
            ++i;
            continue;
        }
        const bool expected_tie_pair =
            i + 1 < expected_order.size() &&
            ((expected_order[i] == tie_a && expected_order[i + 1] == tie_b) ||
             (expected_order[i] == tie_b && expected_order[i + 1] == tie_a));
        const bool actual_tie_pair =
            i + 1 < actual_order.size() &&
            ((actual_order[i] == tie_a && actual_order[i + 1] == tie_b) ||
             (actual_order[i] == tie_b && actual_order[i + 1] == tie_a));
        if (expected_tie_pair && actual_tie_pair) {
            i += 2;
            continue;
        }
        BOOST_CHECK_EQUAL(actual_order[i], expected_order[i]);
        ++i;
    }
    }
}
...
BOOST_AUTO_TEST_CASE(MempoolAncestorIndexingTest)
{
    ...
    // tx1 and tx5 are both at the same ancestor score; use the active
    // comparator directly so the test stays aligned with index semantics.
    const auto& ancestor_index = pool.mapTx.get<ancestor_score>();
    const auto tx1_position = std::distance(
        ancestor_index.begin(),
        std::find_if(ancestor_index.begin(), ancestor_index.end(), [&](const auto& entry) {
            return entry.GetTx().GetHash() == tx1.GetHash();
        }));
    const auto tx5_position = std::distance(
        ancestor_index.begin(),
        std::find_if(ancestor_index.begin(), ancestor_index.end(), [&](const auto& entry) {
            return entry.GetTx().GetHash() == tx5.GetHash();
        }));
    if (tx1_position < tx5_position) {
    ...
    CheckAncestorSortAllowTie(pool, sortedOrder, tx1.GetHash().ToString(), tx5.GetHash().ToString());
...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest)
{
    ...
    const auto* removed_package_entry = Assert(pool.GetEntry(tx2.GetHash()));
    CFeeRate rolling_floor(removed_package_entry->GetModFeesWithDescendants(),
                           removed_package_entry->GetSizeWithDescendants());
    rolling_floor += pool.m_opts.incremental_relay_feerate;

    pool.TrimToSize(GetVirtualTransactionSize(CTransaction(tx1))); // mempool is limited to tx1's size in memory usage, so nothing fits
    BOOST_CHECK(!pool.exists(GenTxid::Txid(tx1.GetHash())));
    BOOST_CHECK(!pool.exists(GenTxid::Txid(tx2.GetHash())));
    BOOST_CHECK(!pool.exists(GenTxid::Txid(tx3.GetHash())));
    BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(), rolling_floor.GetFeePerK());
    ...
    BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(), llround(rolling_floor.GetFeePerK()/2.0));
    // ... then feerate should drop 1/2 each halflife

    SetMockTime(42 + 2*CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE/2);
    BOOST_CHECK_EQUAL(pool.GetMinFee(pool.DynamicMemoryUsage() * 5 / 2).GetFeePerK(), llround(rolling_floor.GetFeePerK()/4.0));
    // ... with a 1/2 halflife when mempool is < 1/2 its target size

    SetMockTime(42 + 2*CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE/2 + CTxMemPool::ROLLING_FEE_HALFLIFE/4);
    BOOST_CHECK_EQUAL(pool.GetMinFee(pool.DynamicMemoryUsage() * 9 / 2).GetFeePerK(), llround(rolling_floor.GetFeePerK()/8.0));
