...
class TestEdgeHeightConditionChecker : public AbstractThresholdConditionChecker
{
private:
...
public:
    ...
    int MinActivationHeight(const Consensus::Params& params) const override { return std::numeric_limits<int>::max(); }
    int Period(const Consensus::Params& params) const override { return 1; }
    int Threshold(const Consensus::Params& params) const override { return 1; }
    bool Condition(const CBlockIndex* pindex, const Consensus::Params& params) const override { return true; }

    void Seed(const CBlockIndex* pindex_prev, ThresholdState state) const { m_cache[pindex_prev] = state; }
    ThresholdState GetStateFor(const CBlockIndex* pindex_prev) const { return AbstractThresholdConditionChecker::GetStateFor(pindex_prev, paramsDummy, m_cache); }
    int GetStateSinceHeightFor(const CBlockIndex* pindex_prev) const { return AbstractThresholdConditionChecker::GetStateSinceHeightFor(pindex_prev, paramsDummy, m_cache); }
};
...
BOOST_AUTO_TEST_CASE(versionbits_height_overflow_guards)
{
    TestEdgeHeightConditionChecker checker;

    CBlockIndex parent{};
    parent.nHeight = std::numeric_limits<int>::max() - 1;
    parent.nTime = TestTime(1);
    parent.nVersion = VERSIONBITS_TOP_BITS;

    CBlockIndex tip{};
    tip.nHeight = std::numeric_limits<int>::max();
    tip.nTime = TestTime(2);
    tip.nVersion = VERSIONBITS_TOP_BITS;
    tip.pprev = &parent;

    checker.Seed(&parent, ThresholdState::LOCKED_IN);
    BOOST_CHECK_EQUAL(checker.GetStateFor(&tip), ThresholdState::ACTIVE);
    BOOST_CHECK_EQUAL(checker.GetStateSinceHeightFor(&tip), std::numeric_limits<int>::max());
}

BOOST_AUTO_TEST_CASE(versionbits_period_alignment_handles_genesis_parent)
{
    TestConditionChecker checker;

    CBlockIndex genesis{};
    genesis.nHeight = 0;
    genesis.nTime = TestTime(0);
    genesis.nVersion = VERSIONBITS_TOP_BITS;

    BOOST_CHECK_EQUAL(checker.GetStateFor(&genesis), ThresholdState::DEFINED);
    BOOST_CHECK_EQUAL(checker.GetStateSinceHeightFor(&genesis), 0);
}
