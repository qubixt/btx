...
BOOST_AUTO_TEST_CASE(chain_contains_next_null_safety)
{
    ...
    BOOST_CHECK(!chain.Contains(nullptr));
    BOOST_CHECK(chain.Next(nullptr) == nullptr);

    CBlockIndex foreign;
    foreign.nHeight = 1;
    foreign.pprev = &blocks[0];
    foreign.BuildSkip();
    BOOST_CHECK(!chain.Contains(&foreign));
    BOOST_CHECK(chain.Next(&foreign) == nullptr);

    BOOST_CHECK(chain.Contains(&blocks[0]));
    BOOST_CHECK(chain.Next(&blocks[0]) == &blocks[1]);
    BOOST_CHECK(chain.Next(&blocks[2]) == nullptr);
}
