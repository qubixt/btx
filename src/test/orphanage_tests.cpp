...
static void AttachOrphanFeeCarrier(CMutableTransaction& tx,
                                   const COutPoint& prevout,
                                   CAmount output_value,
                                   unsigned char witness_seed)
{
    tx.vin = {CTxIn{prevout}};
    tx.vin[0].scriptWitness.stack = {{witness_seed}};
    tx.vout = {CTxOut{output_value, CScript() << OP_TRUE}};
}
...
BOOST_AUTO_TEST_CASE(DoS_mapOrphans)
{
    ...
    // This really-big orphan should be ignored. Size the transaction from the
    // current policy limit instead of a stale fixed input count.
    constexpr unsigned int BIG_ORPHAN_INPUT_FLOOR{2777};
    for (int i = 0; i < 10; i++)
    {
        CTransactionRef txPrev = orphanage.RandomOrphan();
        ...
        tx.vout[0].scriptPubKey = GetScriptForDestination(PKHash(key.GetPubKey()));
        tx.vin.resize(BIG_ORPHAN_INPUT_FLOOR);
        ...
        while (GetTransactionWeight(CTransaction(tx)) <= MAX_STANDARD_TX_WEIGHT) {
            const uint32_t next_index = static_cast<uint32_t>(tx.vin.size());
            tx.vin.emplace_back(COutPoint{txPrev->GetHash(), next_index}, CScript{});
            tx.vin.back().scriptSig = tx.vin[0].scriptSig;
        }
...
BOOST_AUTO_TEST_CASE(oversized_shielded_proof_orphan_tx)
{
    TxOrphanage orphanage;
    auto fixture = test::shielded::BuildV2EgressReceiptFixture(/*output_count=*/2);
    AttachOrphanFeeCarrier(fixture.tx,
                           COutPoint{Txid::FromUint256(uint256{0x42}), 0},
                           /*output_value=*/5 * CENT,
                           /*witness_seed=*/0x53);
    auto& bundle = *Assert(fixture.tx.shielded_bundle.v2_bundle);
    bundle.proof_payload.resize((256 * 1024) + 1, 0x7a);
    BOOST_REQUIRE_EQUAL(bundle.proof_shards.size(), 1U);
    bundle.proof_shards.front().proof_payload_size = bundle.proof_payload.size();

    const auto tx_ref = MakeTransactionRef(fixture.tx);
    BOOST_CHECK_LE(GetTransactionWeight(*tx_ref), MAX_STANDARD_TX_WEIGHT);
    BOOST_CHECK_GT(tx_ref->GetShieldedBundle().GetProofSize(), 256U * 1024U);
    BOOST_CHECK(!orphanage.AddTx(tx_ref, 0));
}
...
BOOST_AUTO_TEST_CASE(shielded_v2_manifest_and_settlement_anchor_orphans)
{
    const NodeId node0{0};
    FastRandomContext det_rand{true};
    TxOrphanageTest orphanage{det_rand};
    ...
    BOOST_REQUIRE(rebalance_fixture.tx.shielded_bundle.v2_bundle.has_value());
    BOOST_CHECK(shielded::v2::BundleHasSemanticFamily(*rebalance_fixture.tx.shielded_bundle.v2_bundle,
                                                      shielded::v2::TransactionFamily::V2_REBALANCE));
    AttachOrphanFeeCarrier(rebalance_fixture.tx,
                           COutPoint{Txid::FromUint256(det_rand.rand256()), 0},
                           /*output_value=*/5 * CENT,
                           /*witness_seed=*/0x51);
    ...
    BOOST_REQUIRE(settlement_fixture.tx.shielded_bundle.v2_bundle.has_value());
    BOOST_CHECK(shielded::v2::BundleHasSemanticFamily(*settlement_fixture.tx.shielded_bundle.v2_bundle,
                                                      shielded::v2::TransactionFamily::V2_SETTLEMENT_ANCHOR));
    AttachOrphanFeeCarrier(settlement_fixture.tx,
                           COutPoint{rebalance_tx->GetHash(), 0},
                           /*output_value=*/4 * CENT,
                           /*witness_seed=*/0x52);
    const auto settlement_tx = MakeTransactionRef(settlement_fixture.tx);

    BOOST_CHECK(orphanage.AddTx(rebalance_tx, node0));
    BOOST_CHECK(orphanage.AddTx(settlement_tx, node0));
    orphanage.SanityCheck();

    BOOST_CHECK_EQUAL(orphanage.Size(), 2U);
    BOOST_CHECK(orphanage.HaveTx(rebalance_tx->GetWitnessHash()));
    BOOST_CHECK(orphanage.HaveTx(settlement_tx->GetWitnessHash()));
    BOOST_CHECK(orphanage.HaveTxFromPeer(rebalance_tx->GetWitnessHash(), node0));
    BOOST_CHECK(orphanage.HaveTxFromPeer(settlement_tx->GetWitnessHash(), node0));
    BOOST_CHECK_EQUAL(orphanage.TotalOrphanUsage(),
                      GetTransactionWeight(*rebalance_tx) + GetTransactionWeight(*settlement_tx));
    BOOST_CHECK_EQUAL(orphanage.UsageByPeer(node0), orphanage.TotalOrphanUsage());

    std::set<CTransactionRef> expected_children{settlement_tx};
    BOOST_CHECK(EqualTxns(expected_children, orphanage.GetChildrenFromSamePeer(rebalance_tx, node0)));

    orphanage.AddChildrenToWorkSet(*rebalance_tx, det_rand);
    BOOST_CHECK(orphanage.HaveTxToReconsider(node0));
    BOOST_CHECK_EQUAL(orphanage.GetTxToReconsider(node0), settlement_tx);
    BOOST_CHECK(!orphanage.HaveTxToReconsider(node0));

    CBlock block;
    block.vtx.emplace_back(MakeTransactionSpending({rebalance_tx->vin[0].prevout}, det_rand));
    block.vtx.emplace_back(MakeTransactionSpending({settlement_tx->vin[0].prevout}, det_rand));
    orphanage.EraseForBlock(block);

    BOOST_CHECK_EQUAL(orphanage.Size(), 0U);
    BOOST_CHECK(!orphanage.HaveTx(rebalance_tx->GetWitnessHash()));
    BOOST_CHECK(!orphanage.HaveTx(settlement_tx->GetWitnessHash()));
}
