...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, Dersig100Setup)
{
    ...
    const CScript spend_script = m_coinbase_txns[0]->vout[0].scriptPubKey;
    const CScript block_script = GetScriptForDestination(PKHash(coinbaseKey.GetPubKey()));
    ...
    const auto ProcessBlock = [this, &block_script](const std::vector<CMutableTransaction>& txns) {
        CBlock new_block;
        {
            LOCK(cs_main);
            new_block = CreateBlock(txns, block_script, m_node.chainman->ActiveChainstate(), /*use_mempool=*/false);
        }
        auto shared_pblock = std::make_shared<const CBlock>(new_block);
        m_node.chainman->ProcessNewBlock(shared_pblock, true, true, nullptr);
        return new_block;
    };
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uint256 hash = SignatureHash(spend_script, spends[i], 0, SIGHASH_ALL, 0, SigVersion::BASE);
    ...
    {
        LOCK(cs_main);
        block = CreateBlock(oneSpend, block_script, m_node.chainman->ActiveChainstate(), /*use_mempool=*/false);
    }
    auto shared_pblock = std::make_shared<const CBlock>(block);
    bool new_block = false;
    BOOST_CHECK(m_node.chainman->ProcessNewBlock(shared_pblock, true, true, &new_block));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    const std::vector<unsigned char> coinbase_pubkey = ToByteVector(coinbaseKey.GetPubKey());
    const std::vector<unsigned char> coinbase_pubkey_hash = ToByteVector(PKHash(coinbaseKey.GetPubKey()));
    ...
    CScript cltv_p2pkh_scriptPubKey = CScript() << OP_CHECKLOCKTIMEVERIFY << OP_DROP << OP_DUP << OP_HASH160 << coinbase_pubkey_hash << OP_EQUALVERIFY << OP_CHECKSIG;
    CScript csv_p2pkh_scriptPubKey = CScript() << OP_CHECKSEQUENCEVERIFY << OP_DROP << OP_DUP << OP_HASH160 << coinbase_pubkey_hash << OP_EQUALVERIFY << OP_CHECKSIG;
    BOOST_REQUIRE_LE(cltv_p2pkh_scriptPubKey.size(), 34U);
    BOOST_REQUIRE_LE(csv_p2pkh_scriptPubKey.size(), 34U);
    ...
    spend_tx.vout[2].scriptPubKey = cltv_p2pkh_scriptPubKey;
    spend_tx.vout[3].nValue = 11*CENT;
    spend_tx.vout[3].scriptPubKey = csv_p2pkh_scriptPubKey;
    ...
    // Sign, with a non-DER signature
    {
        ...
        vchSig.push_back((unsigned char)SIGHASH_ALL);
        spend_tx.vin[0].scriptSig << vchSig << coinbase_pubkey;
    }
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL);
        invalid_with_cltv_tx.vin[0].scriptSig = CScript() << vchSig << coinbase_pubkey << 101;
        ...
        invalid_with_cltv_tx.vin[0].scriptSig = CScript() << vchSig << coinbase_pubkey << 100;
        TxValidationState state;
        PrecomputedTransactionData txdata;
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL);
        invalid_with_csv_tx.vin[0].scriptSig = CScript() << vchSig << coinbase_pubkey << 101;
        ...
        // Make it valid, and check again
        invalid_with_csv_tx.vin[0].scriptSig = CScript() << vchSig << coinbase_pubkey << 100;
        TxValidationState state;
        PrecomputedTransactionData txdata;
    ...
    // NOTE: add tests for remaining script flags
