...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    TxoutType legacy_type{};
    if (!::IsStandard(t.vout[0].scriptPubKey, g_mempool_opts.max_datacarrier_bytes, legacy_type)) {
        BOOST_TEST_MESSAGE("Skipping legacy IsStandardTx matrix under P2MR-only policy");
        return;
    }
    ...
    t.vin.clear();
    t.vin.resize(7316); // size per input (empty scriptSig): 41 bytes
    ...
    // 7316 inputs: 7316*41 = 299956 bytes => 1199824 weight units
    //    1 output:              30 bytes =>    120 weight units
    //                      ======================================
    //                             total: 1199992 weight units
    BOOST_CHECK_EQUAL(GetTransactionWeight(CTransaction(t)), 1199992);
    CheckIsStandard(t);

    // increase output script by 3 bytes, so we end up with 1200004 weight units
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(22, 0); // output size: 33 bytes
    BOOST_CHECK_EQUAL(GetTransactionWeight(CTransaction(t)), 1200004);
...
BOOST_AUTO_TEST_CASE(spends_witness_prog)
{
    ...
    for (int i{2}; i <= 16; ++i) {
        ...
        const TxoutType expected_type = (i == 2) ? TxoutType::WITNESS_V2_P2MR : TxoutType::WITNESS_UNKNOWN;
...
BOOST_AUTO_TEST_CASE(p2mr_multisig_witness_sigop_count)
{
    const std::vector<unsigned char> pk1(MLDSA44_PUBKEY_SIZE, 0x11);
    const std::vector<unsigned char> pk2(MLDSA44_PUBKEY_SIZE, 0x22);
    const std::vector<unsigned char> pk3(SLHDSA128S_PUBKEY_SIZE, 0x33);
    ...
    script_pub_key << OP_2 << ToByteVector(merkle_root);
    ...
    const size_t sigops = CountWitnessSigOps(
        /*scriptSig=*/CScript{},
        script_pub_key,
        &witness,
        SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS);
    BOOST_CHECK_EQUAL(sigops,
