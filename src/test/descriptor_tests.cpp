...
bool IsBase58Char(char c)
{
    ...
    return BASE58_ALPHABET.find(c) != std::string_view::npos;
}

std::optional<CKey> DecodeWIFAnyPrefix(const std::string& token)
{
    std::vector<unsigned char> data;
    if (!DecodeBase58Check(token, data, 64)) return std::nullopt;

    // WIF payload is 1-byte prefix + 32-byte key [+ optional 0x01 compression marker].
    if (data.size() != 33 && data.size() != 34) return std::nullopt;

    bool compressed{false};
    if (data.size() == 34) {
        if (data.back() != 0x01) return std::nullopt;
        data.pop_back();
        compressed = true;
    }

    CKey key;
    key.Set(data.begin() + 1, data.end(), compressed);
    if (!key.IsValid()) return std::nullopt;
    return key;
}

std::string NormalizeLegacyWIFLiterals(const std::string& desc)
{
    std::string out;
    ...
    auto flush = [&] {
        ...
        if (const auto key{DecodeWIFAnyPrefix(token)}) {
            out += EncodeSecret(*key);
        } else {
            out += token;
        }
        token.clear();
...
{
    const std::string normalized_prv{NormalizeLegacyWIFLiterals(prv)};
    const std::string normalized_pub{NormalizeLegacyWIFLiterals(pub)};

    FlatSigningProvider keys_priv, keys_pub;
    std::string error;
    auto parse_priv = Parse(normalized_prv, keys_priv, error);
    auto parse_pub = Parse(normalized_pub, keys_pub, error);
    BOOST_CHECK_MESSAGE(parse_priv.empty(), normalized_prv);
    BOOST_CHECK_MESSAGE(parse_pub.empty(), normalized_pub);
...
{
    prv = NormalizeLegacyWIFLiterals(prv);
    pub = NormalizeLegacyWIFLiterals(pub);
    const std::string normalized_norm_pub{NormalizeLegacyWIFLiterals(norm_pub)};
    if (expected_prv) expected_prv = NormalizeLegacyWIFLiterals(*expected_prv);
    if (expected_pub) expected_pub = NormalizeLegacyWIFLiterals(*expected_pub);
    ...
    BOOST_CHECK_MESSAGE(EqualDescriptor(norm1, normalized_norm_pub), "priv->ToNormalizedString(): " + norm1 + " Norm. desc: " + normalized_norm_pub);
...
BOOST_AUTO_TEST_CASE(descriptor_test)
{
    ...
    CheckInferDescriptor("001422e363a523947a110d9a9eb114820de183aca313", "addr(btx1qyt3k8ffrj3apzrv6n6c3fqsduxp6egcn7lqv4d)", {}, {{"049228de6902abb4f541791f6d7f925b10e2078ccb1298856e5ea5cc5fd667f930eac37a00cc07f9a91ef3c2d17bf7a17db04552ff90ac312a5b8b4caca6c97aa4", ""}});
...
BOOST_AUTO_TEST_CASE(descriptor_parses_pq_multisig_leaf)
{
    const std::vector<unsigned char> ml1(MLDSA44_PUBKEY_SIZE, 0x11);
    const std::vector<unsigned char> ml2(MLDSA44_PUBKEY_SIZE, 0x22);
    const std::vector<unsigned char> slh(SLHDSA128S_PUBKEY_SIZE, 0x33);
    ...
    BOOST_REQUIRE(parsed[0]->Expand(/*pos=*/0, DUMMY_SIGNING_PROVIDER, scripts, out));
    ...
    CScript expected_spk;
    expected_spk << OP_2 << ToByteVector(merkle_root);
    BOOST_CHECK(scripts[0] == expected_spk);
}

BOOST_AUTO_TEST_CASE(tr_descriptor_scriptpath_estimation_not_keypath_only)
{
    FlatSigningProvider provider;
    std::string error;
    auto parsed = Parse(
        AddChecksum("tr(03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5,{pk(03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5),pk(03272c0c1ae2c07528283b91ca57b45d2cc84e7960e1f17f58815372285f35e99a)})"),
        provider,
        error,
    ...
    const auto max_weight = parsed[0]->MaxSatisfactionWeight(/*use_max_sig=*/false);
    const auto max_elems = parsed[0]->MaxSatisfactionElems();
    BOOST_REQUIRE(max_weight.has_value());
    BOOST_REQUIRE(max_elems.has_value());
    BOOST_CHECK_GT(*max_weight, 1 + 65);
    BOOST_CHECK_GT(*max_elems, 1);
}
