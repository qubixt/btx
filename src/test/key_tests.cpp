...
static const std::string strSecret1Legacy = "5HxWvvfubhXpYYpS3tJkw6fq9jE9j18THftkZjHHfmFiWtmAbrj";
static const std::string strSecret2Legacy = "5KC4ejrDjv152FGwP386VD1i2NYc5KkfSMyv1nGy1VGDxGHqVY3";
static const std::string strSecret1CLegacy = "Kwr371tjA9u2rFSMZjTNun2PXXP3WPZu2afRHTcta6KxEUdm1vEw";
static const std::string strSecret2CLegacy = "L3Hq7a8FEQwJkW1M2GNKDW28546Vp5miewcCzSqUD9kCAXrJdS3g";

static CKey DecodeLegacySecret(const std::string& legacy_wif)
{
    CKey key;
    std::vector<unsigned char> data;
    if (!DecodeBase58Check(legacy_wif, data, 34)) return key;
    if (data.size() != 33 && data.size() != 34) return key;
    const bool compressed = data.size() == 34 && data.back() == 1;
    if (data.size() == 34 && data.back() != 1) return key;
...
{
    CKey key1  = DecodeLegacySecret(strSecret1Legacy);
    BOOST_CHECK(key1.IsValid() && !key1.IsCompressed());
    CKey key2  = DecodeLegacySecret(strSecret2Legacy);
    BOOST_CHECK(key2.IsValid() && !key2.IsCompressed());
    CKey key1C = DecodeLegacySecret(strSecret1CLegacy);
    BOOST_CHECK(key1C.IsValid() && key1C.IsCompressed());
    CKey key2C = DecodeLegacySecret(strSecret2CLegacy);
    BOOST_CHECK(key2C.IsValid() && key2C.IsCompressed());
    const std::string strAddressBad = EncodeDestination(PKHash(key1.GetPubKey()));
...
BOOST_AUTO_TEST_CASE(key_signature_tests)
{
    ...
    CKey key = DecodeLegacySecret(strSecret1Legacy);
...
{
    for (const auto& secret : {strSecret1Legacy, strSecret2Legacy, strSecret1CLegacy, strSecret2CLegacy}) {
        CKey key = DecodeLegacySecret(secret);
        BOOST_CHECK(key.IsValid());
