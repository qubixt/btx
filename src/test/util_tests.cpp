...
std::optional<CTxDestination> DecodeBitcoinMainnetAddress(std::string_view address)
{
    std::vector<unsigned char> data;
    if (DecodeBase58Check(std::string{address}, data, 21)) {
        uint160 hash;
        if (data.size() == 21 && data[0] == 0x00) {
            std::copy(data.begin() + 1, data.end(), hash.begin());
            return PKHash(hash);
        }
        if (data.size() == 21 && data[0] == 0x05) {
            std::copy(data.begin() + 1, data.end(), hash.begin());
    ...
    const auto dec = bech32::Decode(std::string{address});
    if (dec.encoding == bech32::Encoding::INVALID || dec.hrp != "bc" || dec.data.empty()) {
        return std::nullopt;
    }
    ...
    if (version == 0) {
        ...
        if (program.size() == key_hash.size()) {
            std::copy(program.begin(), program.end(), key_hash.begin());
            return key_hash;
        }
        WitnessV0ScriptHash script_hash;
        if (program.size() == script_hash.size()) {
            std::copy(program.begin(), program.end(), script_hash.begin());
            return script_hash;
        }
        return std::nullopt;
    }
    ...
    if (version == 1 && program.size() == 32) {
        WitnessV1Taproot taproot;
        std::copy(program.begin(), program.end(), taproot.begin());
        return taproot;
    }

    if (version >= 1 && version <= 16 && program.size() >= 2 && program.size() <= 40) {
        return WitnessUnknown{version, program};
    }
    return std::nullopt;
}

std::string ReencodeBitcoinMainnetAddress(std::string_view address)
{
    const auto destination = DecodeBitcoinMainnetAddress(address);
    return destination ? EncodeDestination(*destination) : std::string{address};
}
} // namespace
...
BOOST_AUTO_TEST_CASE(legacy_binary_invocation_detection)
{
    BOOST_CHECK(IsLegacyBinaryInvocation("bitcoind", "bitcoind"));
    BOOST_CHECK(!IsLegacyBinaryInvocation("btxd", "bitcoind"));
    BOOST_CHECK(IsLegacyBinaryInvocation("/tmp/bitcoin-cli", "bitcoin-cli"));
    BOOST_CHECK(!IsLegacyBinaryInvocation("/tmp/btx-cli", "bitcoin-cli"));
#ifdef WIN32
    BOOST_CHECK(IsLegacyBinaryInvocation("C:\\tmp\\bitcoin-wallet.exe", "bitcoin-wallet"));
#else
    BOOST_CHECK(!IsLegacyBinaryInvocation("/tmp/bitcoin-wallet.exe", "bitcoin-wallet"));
#endif
}
...
BOOST_AUTO_TEST_CASE(test_LockDirectory)
{
    ...
    pid_t pid = fork();
    if (!pid) {
        if (close(fd[1]) != 0) _exit(16); // Child: close parent end
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    ...
    BOOST_CHECK(IsValidDestination(DecodeDestination(EncodeDestination(dest_legacy))));
    ...
    BOOST_CHECK(IsValidDestination(DecodeDestination(EncodeDestination(dest_p2sh_segwit))));
    ...
    BOOST_CHECK(IsValidDestination(DecodeDestination(EncodeDestination(dest_bech32))));
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("3B5fQsEXEaV8v6U3ejYc8XaKXAkyQj2MjV"),
            "AA==",
            "message too"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("1KqbBpLy5FARmTPD4VZnDDpYjkUvkr82Pm"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("15CRxFdyRpGZLW9w8HnHvVduizdL5jKNbs"),
            "IPojfrX2dfPnH26UegfbGQQLrdK844DlHq5157/P6h57WyuS/Qsl+h/WSVGDF4MUi4rWSswW38oimDYfNNUBUOk=",
            "I never signed this"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("11canuhp9X2NocwCq7xNrQYTmUgZAnLK3"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("bc1q9vza2e8x573nczrlzms0wvx3gsqjx7vavgkx0l"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("3LnYoUkFrhyYP3V7rq3mhpwALz1XbCY9Uq"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("bc1ppv609nr0vr25u07u95waq5lucwfm6tde4nydujnu8npg4q75mr5sxq8lt3"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            ReencodeBitcoinMainnetAddress("bc1qkecg9ly2xwxqgdy9egpuy87qc9x26smpts562s"),
            "AkcwRAIgM2gBAQqvZX15ZiysmKmQpDrG83avLIT492QBzLnQIxYCIBaTpOaD20qRlEylyxFSeEA2ba9YOixpX8z46TSDtS40ASECx/EgAxlkQpQ9hYjgGu6EBCPMVPwVIVJqO4XCsMvViHI=",
            ""),
        MessageVerificationResult::ERR_INVALID);
