...
BOOST_AUTO_TEST_CASE(coinstats_height_encoding_handles_max_height)
{
    const COutPoint outpoint{Txid::FromUint256(uint256::ONE), 0};
    const Coin coin_non_coinbase{CTxOut{1, CScript{}}, std::numeric_limits<int>::max(), /*fCoinBaseIn=*/false};
    const Coin coin_coinbase{CTxOut{1, CScript{}}, std::numeric_limits<int>::max(), /*fCoinBaseIn=*/true};

    MuHash3072 hash_a{};
    MuHash3072 hash_b{};
    MuHash3072 hash_c{};
    kernel::ApplyCoinHash(hash_a, outpoint, coin_non_coinbase);
    kernel::ApplyCoinHash(hash_b, outpoint, coin_non_coinbase);
    kernel::ApplyCoinHash(hash_c, outpoint, coin_coinbase);

    uint256 digest_a;
    uint256 digest_b;
    uint256 digest_c;
    hash_a.Finalize(digest_a);
    hash_b.Finalize(digest_b);
    hash_c.Finalize(digest_c);

    BOOST_CHECK_EQUAL(digest_a, digest_b);
    BOOST_CHECK(digest_a != digest_c);
}
