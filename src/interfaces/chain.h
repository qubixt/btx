...
//!
//! TO-DO: Current chain methods are too low level, exposing too much of the
...
public:
    ...
    //! Return the consensus shielded Merkle tree root and size.
    //! Returns nullopt if shielded state is not initialized.
    virtual std::optional<std::pair<uint256, uint64_t>> getShieldedTreeInfo() = 0;

    //! Return the current account-registry root plus an inclusion proof for an
    //! unspent account-leaf commitment. Returns nullopt if unavailable.
    virtual std::optional<std::pair<uint256, shielded::registry::ShieldedAccountRegistrySpendWitness>>
    getShieldedAccountRegistryWitness(const uint256& account_leaf_commitment) = 0;
...
public:
    ...
    virtual void assignContext(std::any&) {};
