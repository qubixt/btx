...
const char * const BITCOIN_RW_CONF_FILENAME = "btx_rw.conf";
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ...
        ChainTypeToString(ChainType::SHIELDEDV2DEV),
...
const std::vector<std::string> TEST_OPTIONS_DOC{
    ...
    "matmulstrict (enable full MatMul PoW validation on regtest)",
    "kawpowstrict (deprecated alias for matmulstrict)",
    "matmuldgw (enable MatMul DGW retargeting with a finite fast-mining window on regtest)",
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fShieldedV2Dev = get_net("-shieldedv2dev");
    const bool fTestNet = get_net("-testnet");
    const bool fTestNet4 = get_net("-testnet4");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fShieldedV2Dev + (int)fTestNet + (int)fTestNet4 > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -shieldedv2dev, -signet, -testnet, -testnet4 and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fShieldedV2Dev) return ChainType::SHIELDEDV2DEV;
