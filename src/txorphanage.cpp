...
namespace {
constexpr size_t MAX_STANDARD_ORPHAN_SHIELDED_PROOF_BYTES{256 * 1024};
}
...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    if (tx->HasShieldedBundle() &&
        tx->GetShieldedBundle().GetProofSize() > MAX_STANDARD_ORPHAN_SHIELDED_PROOF_BYTES) {
        LogDebug(BCLog::TXPACKAGES,
                 "ignoring shielded orphan tx with oversized proof payload (proof_bytes: %u, txid: %s, wtxid: %s)\n",
                 static_cast<unsigned int>(tx->GetShieldedBundle().GetProofSize()),
                 hash.ToString(),
                 wtxid.ToString());
        return false;
    }
