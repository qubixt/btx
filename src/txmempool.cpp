...
void CTxMemPool::addNewTransaction(CTxMemPool::txiter newit, CTxMemPool::setEntries& setAncestors)
{
    ...
    AddShieldedNullifiers(tx);
    AddShieldedSettlementAnchorRefs(tx);
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    RemoveShieldedNullifiers(tx);
    RemoveShieldedSettlementAnchorRefs(tx);
...
void CTxMemPool::AddShieldedNullifiers(const CTransaction& tx)
{
    ...
    for (const auto& nullifier : CollectShieldedNullifiers(tx.GetShieldedBundle())) {
        if (!nullifier.IsNull()) {
...
void CTxMemPool::RemoveShieldedNullifiers(const CTransaction& tx)
{
    if (!tx.HasShieldedBundle()) return;
    const Txid txid = tx.GetHash();
    for (const auto& nullifier : CollectShieldedNullifiers(tx.GetShieldedBundle())) {
        if (!nullifier.IsNull()) {
            const auto it = m_shielded_nullifiers.find(nullifier);
...
void CTxMemPool::AddShieldedSettlementAnchorRefs(const CTransaction& tx)
{
    if (!tx.HasShieldedBundle()) return;
    for (const auto& settlement_anchor : CollectShieldedSettlementAnchorRefs(tx.GetShieldedBundle())) {
        if (!settlement_anchor.IsNull()) {
            m_shielded_settlement_anchor_refs.insert(settlement_anchor);
        }
    }
}

void CTxMemPool::RemoveShieldedSettlementAnchorRefs(const CTransaction& tx)
{
    if (!tx.HasShieldedBundle()) return;
    for (const auto& settlement_anchor : CollectShieldedSettlementAnchorRefs(tx.GetShieldedBundle())) {
        if (!settlement_anchor.IsNull()) {
            m_shielded_settlement_anchor_refs.erase(settlement_anchor);
        }
    }
}

bool CTxMemPool::HasShieldedNullifierConflict(const CTransaction& tx) const
{
    AssertLockHeld(cs);
    const auto conflicts = GetShieldedNullifierConflicts(tx);
    return conflicts.invalid_in_tx || !conflicts.txids.empty();
}

CTxMemPool::ShieldedNullifierConflictInfo CTxMemPool::GetShieldedNullifierConflicts(const CTransaction& tx) const
{
    AssertLockHeld(cs);
    ShieldedNullifierConflictInfo result;
    if (!tx.HasShieldedBundle()) return result;

    std::unordered_set<Nullifier, NullifierHasher> seen_in_tx;
    const Txid txid = tx.GetHash();
    for (const auto& nullifier : CollectShieldedNullifiers(tx.GetShieldedBundle())) {
        if (nullifier.IsNull()) {
            result.invalid_in_tx = true;
            return result;
        }
        if (!seen_in_tx.insert(nullifier).second) {
            result.invalid_in_tx = true;
            return result;
        }
        const auto it = m_shielded_nullifiers.find(nullifier);
...
bool CTxMemPool::HasShieldedSettlementAnchorConflict(const CTransaction& tx) const
{
    ...
    std::set<uint256> seen_in_tx;
    for (const auto& settlement_anchor : CollectShieldedSettlementAnchorRefs(tx.GetShieldedBundle())) {
        if (settlement_anchor.IsNull()) return true;
        if (!seen_in_tx.insert(settlement_anchor).second) return true;
        if (m_shielded_settlement_anchor_refs.count(settlement_anchor) != 0) return true;
    }
    return false;
}
