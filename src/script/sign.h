...
struct SignatureData {
    ...
    P2MRSpendData p2mr_spenddata; ///< P2MR leaf scripts and control blocks.
    std::map<std::pair<uint256, std::vector<unsigned char>>, std::vector<unsigned char>> p2mr_script_sigs; ///< P2MR CHECKSIG signatures indexed by (leaf_hash, pubkey).
    std::map<std::pair<uint256, std::vector<unsigned char>>, std::vector<unsigned char>> p2mr_csfs_sigs; ///< CSFS signatures indexed by (leaf_hash, pubkey).
    std::map<std::pair<uint256, std::vector<unsigned char>>, std::vector<unsigned char>> p2mr_csfs_msgs; ///< CSFS messages indexed by (leaf_hash, pubkey).
    std::vector<unsigned char> p2mr_leaf_script; ///< Selected P2MR leaf script for single-leaf PSBT flow.
    std::vector<unsigned char> p2mr_control_block; ///< Selected P2MR control block for single-leaf PSBT flow.
    std::optional<PQAlgorithm> preferred_pq_signing_algo; ///< Preferred PQ algorithm for P2MR leaf selection.
