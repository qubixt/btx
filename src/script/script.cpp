...
std::string GetOpName(opcodetype opcode)
{
    ...
    case OP_CHECKSIG_MLDSA         : return "OP_CHECKSIG_MLDSA";
    case OP_CHECKSIG_SLHDSA        : return "OP_CHECKSIG_SLHDSA";
    case OP_CHECKSIGFROMSTACK      : return "OP_CHECKSIGFROMSTACK";
    case OP_CHECKSIGADD_MLDSA      : return "OP_CHECKSIGADD_MLDSA";
    case OP_CHECKSIGADD_SLHDSA     : return "OP_CHECKSIGADD_SLHDSA";
    case OP_CHECKSIG_FALCON        : return "OP_CHECKSIG_FALCON";
    case OP_CHECKSIGADD_FALCON     : return "OP_CHECKSIGADD_FALCON";
    case OP_CHECKSIGFROMSTACK_FALCON: return "OP_CHECKSIGFROMSTACK_FALCON";
...
unsigned int CScript::GetSigOpCount(bool fAccurate) const
{
    ...
    while (pc < end())
    {
        ...
        if (!GetOp(pc, opcode))
            break;
        if (opcode == OP_CHECKSIG || opcode == OP_CHECKSIGVERIFY || opcode == OP_CHECKSIG_MLDSA || opcode == OP_CHECKSIG_SLHDSA || opcode == OP_CHECKSIGFROMSTACK || opcode == OP_CHECKSIGADD_MLDSA || opcode == OP_CHECKSIGADD_SLHDSA)
            n++;
...
bool IsOpSuccessP2MR(const opcodetype& opcode)
{
    if (!IsOpSuccess(opcode)) return false;
    // Active P2MR opcodes are excluded here. Reserved Falcon slots intentionally
    // stay in the OP_SUCCESSx set until a later soft fork assigns semantics.
