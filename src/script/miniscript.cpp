...
Type ComputeType(Fragment fragment, Type x, Type y, Type z, const std::vector<Type>& sub_types, uint32_t k,
    ...
    if (fragment == Fragment::OLDER || fragment == Fragment::AFTER) {
        CHECK_NONFATAL(k >= 1 && k < 0x80000000UL);
               ...
               fragment == Fragment::MULTI_MLDSA || fragment == Fragment::MULTI_SLHDSA) {
    ...
    if (fragment == Fragment::PK_K || fragment == Fragment::PK_H ||
        fragment == Fragment::PK_MLDSA || fragment == Fragment::PK_SLHDSA) {
        CHECK_NONFATAL(n_keys == 1);
        if (fragment == Fragment::PK_MLDSA || fragment == Fragment::PK_SLHDSA) {
            CHECK_NONFATAL(IsP2MR(ms_ctx));
        }
        ...
        CHECK_NONFATAL(IsTapscript(ms_ctx));
    } else if (fragment == Fragment::MULTI_MLDSA || fragment == Fragment::MULTI_SLHDSA) {
        CHECK_NONFATAL(n_keys >= 1 && n_keys <= MAX_PQ_PUBKEYS_PER_MULTISIG);
        CHECK_NONFATAL(IsP2MR(ms_ctx));
        if (fragment == Fragment::MULTI_SLHDSA) CHECK_NONFATAL(k == 1);
    } else {
    ...
    switch (fragment) {
        ...
        case Fragment::PK_MLDSA: return "Bonudemsk"_mst;
        case Fragment::PK_SLHDSA: return "Bonudemsk"_mst;
...
size_t ComputeScriptLen(Fragment fragment, Type sub0typ, size_t subsize, uint32_t k, size_t n_subs,
    ...
    switch (fragment) {
        ...
        case Fragment::PK_MLDSA: return 3 + MLDSA44_PUBKEY_SIZE + 1;
        case Fragment::PK_SLHDSA: return 1 + SLHDSA128S_PUBKEY_SIZE + 1;
        case Fragment::OLDER:
        ...
        case Fragment::MULTI_MLDSA: return (3 + MLDSA44_PUBKEY_SIZE + 1) * n_keys + BuildScript(k).size() + 1;
        case Fragment::MULTI_SLHDSA: return (1 + SLHDSA128S_PUBKEY_SIZE + 1) * n_keys + BuildScript(k).size() + 1;
