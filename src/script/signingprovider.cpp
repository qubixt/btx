...
const CPQKey* HidingSigningProvider::GetPQKey(Span<const unsigned char> pubkey) const
{
...
const CPQKey* FlatSigningProvider::GetPQKey(Span<const unsigned char> pubkey) const
{
    const auto it = pq_keys.find(std::vector<unsigned char>(pubkey.begin(), pubkey.end()));
...
const CPQKey* MultiSigningProvider::GetPQKey(Span<const unsigned char> pubkey) const
{
    for (const auto& provider: m_providers) {
...
bool MultiSigningProvider::GetP2MRSpendData(const WitnessV2P2MR& output, P2MRSpendData& spenddata) const
{
    ...
    for (const auto& provider : m_providers) {
        P2MRSpendData data;
        if (provider->GetP2MRSpendData(output, data)) {
            spenddata.Merge(std::move(data));
            found = true;
        }
    }
    return found;
...
{
    // TO-DO: figure out how to better deal with conflicting information
    // being merged.
