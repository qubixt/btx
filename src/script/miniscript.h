...
enum class Fragment {
    ...
    PK_MLDSA,  //!< [pqkey] OP_CHECKSIG_MLDSA (only within P2MR context)
    PK_SLHDSA, //!< [pqkey] OP_CHECKSIG_SLHDSA (only within P2MR context)
    ...
    MULTI_MLDSA, //!< [key_0] OP_CHECKSIG_MLDSA ([key_n] OP_CHECKSIGADD_MLDSA)* [k] OP_NUMEQUAL (only within P2MR ctx)
    MULTI_SLHDSA, //!< [key_0] OP_CHECKSIG_SLHDSA ([key_n] OP_CHECKSIGADD_SLHDSA)* [k] OP_NUMEQUAL (only within P2MR ctx; 1-of-N)
...
/** Whether the context is Tapscript. */
constexpr bool IsTapscript(MiniscriptContext ms_ctx)
{
    switch (ms_ctx) {
        case MiniscriptContext::P2WSH: return false;
        case MiniscriptContext::TAPSCRIPT: return true;
        case MiniscriptContext::P2MR: return false;
    }
    assert(false);
    return false;
}

/** Whether the context is P2MR. */
constexpr bool IsP2MR(MiniscriptContext ms_ctx)
{
    switch (ms_ctx) {
        case MiniscriptContext::P2WSH: return false;
        case MiniscriptContext::TAPSCRIPT: return false;
        case MiniscriptContext::P2MR: return true;
    }
    assert(false);
    return false;
}
...
constexpr uint32_t MaxScriptSize(MiniscriptContext ms_ctx)
{
    ...
    if (IsP2MR(ms_ctx)) return MAX_P2MR_SCRIPT_SIZE;
    ...
    CScript ToScript(const Ctx& ctx) const
    {
        ...
        auto upfn = [&ctx, is_tapscript](bool verify, const Node& node, Span<CScript> subs) -> CScript {
            switch (node.fragment) {
                ...
                case Fragment::PK_MLDSA:
                    return verify
                        ? BuildScript(ctx.ToPKBytes(node.keys[0]), OP_CHECKSIG_MLDSA, OP_VERIFY)
                        : BuildScript(ctx.ToPKBytes(node.keys[0]), OP_CHECKSIG_MLDSA);
                case Fragment::PK_SLHDSA:
                    return verify
                        ? BuildScript(ctx.ToPKBytes(node.keys[0]), OP_CHECKSIG_SLHDSA, OP_VERIFY)
                        : BuildScript(ctx.ToPKBytes(node.keys[0]), OP_CHECKSIG_SLHDSA);
                ...
                case Fragment::MULTI_MLDSA: {
                    CHECK_NONFATAL(IsP2MR(node.m_script_ctx));
                    CScript script = BuildScript(ctx.ToPKBytes(*node.keys.begin()), OP_CHECKSIG_MLDSA);
                    for (auto it = node.keys.begin() + 1; it != node.keys.end(); ++it) {
                        script = BuildScript(std::move(script), ctx.ToPKBytes(*it), OP_CHECKSIGADD_MLDSA);
                    }
                    return BuildScript(std::move(script), node.k, verify ? OP_NUMEQUALVERIFY : OP_NUMEQUAL);
                }
                case Fragment::MULTI_SLHDSA: {
                    CHECK_NONFATAL(IsP2MR(node.m_script_ctx));
                    CScript script = BuildScript(ctx.ToPKBytes(*node.keys.begin()), OP_CHECKSIG_SLHDSA);
                    for (auto it = node.keys.begin() + 1; it != node.keys.end(); ++it) {
                        script = BuildScript(std::move(script), ctx.ToPKBytes(*it), OP_CHECKSIGADD_SLHDSA);
                    }
                    return BuildScript(std::move(script), node.k, verify ? OP_NUMEQUALVERIFY : OP_NUMEQUAL);
                }
        ...
        auto upfn = [&ctx, is_tapscript](bool wrapped, const Node& node, Span<std::string> subs) -> std::optional<std::string> {
            ...
            switch (node.fragment) {
                ...
                case Fragment::PK_MLDSA: {
                    auto key_str = ctx.ToString(node.keys[0]);
                    if (!key_str) return {};
                    return std::move(ret) + "pk_mldsa(" + std::move(*key_str) + ")";
                }
                case Fragment::PK_SLHDSA: {
                    auto key_str = ctx.ToString(node.keys[0]);
                    if (!key_str) return {};
                    return std::move(ret) + "pk_slhdsa(" + std::move(*key_str) + ")";
                }
                ...
                case Fragment::MULTI_MLDSA: {
                    CHECK_NONFATAL(IsP2MR(node.m_script_ctx));
                    auto str = std::move(ret) + "multi_mldsa(" + util::ToString(node.k);
                    for (const auto& key : node.keys) {
                        auto key_str = ctx.ToString(key);
                        if (!key_str) return {};
                        str += "," + std::move(*key_str);
                ...
                case Fragment::MULTI_SLHDSA: {
                    CHECK_NONFATAL(IsP2MR(node.m_script_ctx));
                    auto str = std::move(ret) + "multi_slhdsa(" + util::ToString(node.k);
                    for (const auto& key : node.keys) {
                        auto key_str = ctx.ToString(key);
                        if (!key_str) return {};
                        str += "," + std::move(*key_str);
...
private:
    internal::Ops CalcOps() const {
        switch (fragment) {
            ...
            case Fragment::PK_MLDSA: return {1, 0, 0};
            case Fragment::PK_SLHDSA: return {1, 0, 0};
            case Fragment::OLDER:
            ...
            case Fragment::MULTI_MLDSA: return {(uint32_t)keys.size() + 1, 0, 0};
            case Fragment::MULTI_SLHDSA: return {(uint32_t)keys.size() + 1, 0, 0};
    ...
    internal::StackSize CalcStackSize() const {
        ...
        switch (fragment) {
            ...
            case Fragment::PK_MLDSA: return {
                SatInfo::Push() + SatInfo::OP_CHECKSIG(),
                SatInfo::Push() + SatInfo::OP_CHECKSIG()
            };
            case Fragment::PK_SLHDSA: return {
                SatInfo::Push() + SatInfo::OP_CHECKSIG(),
                SatInfo::Push() + SatInfo::OP_CHECKSIG()
            };
            case Fragment::SHA256:
            ...
            case Fragment::MULTI_MLDSA: return {SatInfo(keys.size() - 1, keys.size())};
            case Fragment::MULTI_SLHDSA: return {SatInfo(keys.size() - 1, keys.size())};
    ...
    internal::WitnessSize CalcWitnessSize() const {
        ...
        switch (fragment) {
            ...
            case Fragment::PK_MLDSA: return {1 + MLDSA44_SIGNATURE_SIZE, 1};
            case Fragment::PK_SLHDSA: return {1 + SLHDSA128S_SIGNATURE_SIZE, 1};
            case Fragment::SHA256:
            case Fragment::RIPEMD160:
            case Fragment::HASH256:
            ...
            case Fragment::MULTI_MLDSA: {
                ...
                const uint32_t sig_size_mldsa = 1U + static_cast<uint32_t>(MLDSA44_SIGNATURE_SIZE);
                return {k * sig_size_mldsa + num_keys - k, num_keys};
            }
            case Fragment::MULTI_SLHDSA: {
                const uint32_t num_keys = static_cast<uint32_t>(keys.size());
                const uint32_t sig_size_slhdsa = 1U + static_cast<uint32_t>(SLHDSA128S_SIGNATURE_SIZE);
                return {k * sig_size_slhdsa + num_keys - k, num_keys};
            }
            case Fragment::WRAP_A:
            case Fragment::WRAP_N:
            case Fragment::WRAP_S:
...
template<typename Key, typename Ctx>
size_t GetKeyByteSize(const Ctx& ctx, const Key& key)
{
    if constexpr (requires { ctx.ToPKBytes(key); }) {
        return ctx.ToPKBytes(key).size();
    }
...
inline NodeRef<Key> Parse(Span<const char> in, const Ctx& ctx)
{
    ...
    // Parses a multi*() fragment from its string representation. Returns false on parsing error.
    const auto parse_multi_exp = [&](Span<const char>& in, const ParsedMultiKind kind) -> bool {
        size_t max_keys{0};
        MiniscriptContext required_ctx{MiniscriptContext::P2WSH};
        Fragment fragment{Fragment::MULTI};
        size_t expected_key_size{0};
        switch (kind) {
        case ParsedMultiKind::LEGACY:
            max_keys = MAX_PUBKEYS_PER_MULTISIG;
            required_ctx = MiniscriptContext::P2WSH;
            fragment = Fragment::MULTI;
            break;
        case ParsedMultiKind::TAPSCRIPT_A:
            max_keys = MAX_PUBKEYS_PER_MULTI_A;
            required_ctx = MiniscriptContext::TAPSCRIPT;
            fragment = Fragment::MULTI_A;
            break;
        case ParsedMultiKind::P2MR_MLDSA:
            max_keys = MAX_PQ_PUBKEYS_PER_MULTISIG;
            required_ctx = MiniscriptContext::P2MR;
            fragment = Fragment::MULTI_MLDSA;
            expected_key_size = MLDSA44_PUBKEY_SIZE;
            break;
        case ParsedMultiKind::P2MR_SLHDSA:
            max_keys = MAX_PQ_PUBKEYS_PER_MULTISIG;
            required_ctx = MiniscriptContext::P2MR;
            fragment = Fragment::MULTI_SLHDSA;
            expected_key_size = SLHDSA128S_PUBKEY_SIZE;
            break;
        }
        ...
        while (next_comma != -1) {
            ...
            if (!key) return false;
            if (expected_key_size != 0 && GetKeyByteSize(ctx, *key) != expected_key_size) return false;
            keys.push_back(std::move(*key));
            in = in.subspan(key_length + 1);
        }
        if (keys.size() < 1 || keys.size() > max_keys) return false;
        if (k < 1 || k > (int64_t)keys.size()) return false;
        if (kind == ParsedMultiKind::P2MR_SLHDSA && k != 1) return false;
        if (kind == ParsedMultiKind::TAPSCRIPT_A) {
            // (push + xonly-key + CHECKSIG[ADD]) * n + k + OP_NUMEQUAL(VERIFY), minus one.
            script_size += (1 + 32 + 1) * keys.size() + BuildScript(k).size();
        } else if (kind == ParsedMultiKind::LEGACY) {
            script_size += 2 + (keys.size() > 16) + (k > 16) + 34 * keys.size();
        } else if (kind == ParsedMultiKind::P2MR_MLDSA) {
            // (push + pubkey + CHECKSIG[ADD]) * n + k + OP_NUMEQUAL(VERIFY), minus one.
            script_size += (3 + MLDSA44_PUBKEY_SIZE + 1) * keys.size() + BuildScript(k).size();
        } else {
            // (push + pubkey + CHECKSIG[ADD]) * n + k + OP_NUMEQUAL(VERIFY), minus one.
            script_size += (1 + SLHDSA128S_PUBKEY_SIZE + 1) * keys.size() + BuildScript(k).size();
        }
        constructed.push_back(MakeNodeRef<Key>(internal::NoDupCheck{}, ctx.MsContext(), fragment, std::move(keys), k));
        return true;
    };
    ...
    while (!to_parse.empty()) {
        ...
        case ParseContext::EXPR: {
            if (Const("0", in)) {
                ...
                in = in.subspan(key_size + 1);
                script_size += 23;
            } else if (Const("pk_mldsa(", in)) {
                if (!IsP2MR(ctx.MsContext())) return {};
                auto res = ParseKeyEnd<Key, Ctx>(in, ctx);
                if (!res) return {};
                auto& [key, key_size] = *res;
                if (GetKeyByteSize(ctx, key) != MLDSA44_PUBKEY_SIZE) return {};
                constructed.push_back(MakeNodeRef<Key>(internal::NoDupCheck{}, ctx.MsContext(), Fragment::PK_MLDSA, Vector(std::move(key))));
                in = in.subspan(key_size + 1);
                script_size += 3 + MLDSA44_PUBKEY_SIZE;
            } else if (Const("pk_slhdsa(", in)) {
                if (!IsP2MR(ctx.MsContext())) return {};
                auto res = ParseKeyEnd<Key, Ctx>(in, ctx);
                if (!res) return {};
                auto& [key, key_size] = *res;
                if (GetKeyByteSize(ctx, key) != SLHDSA128S_PUBKEY_SIZE) return {};
                constructed.push_back(MakeNodeRef<Key>(internal::NoDupCheck{}, ctx.MsContext(), Fragment::PK_SLHDSA, Vector(std::move(key))));
                in = in.subspan(key_size + 1);
                script_size += 1 + SLHDSA128S_PUBKEY_SIZE;
                ...
                if (!parse_multi_exp(in, ParsedMultiKind::LEGACY)) return {};
            } else if (Const("multi_a(", in)) {
                if (!parse_multi_exp(in, ParsedMultiKind::TAPSCRIPT_A)) return {};
            } else if (Const("multi_mldsa(", in)) {
                if (!parse_multi_exp(in, ParsedMultiKind::P2MR_MLDSA)) return {};
            } else if (Const("multi_slhdsa(", in)) {
                if (!parse_multi_exp(in, ParsedMultiKind::P2MR_SLHDSA)) return {};
...
inline NodeRef<Key> DecodeScript(I& in, I last, const Ctx& ctx)
{
    ...
    while (!to_parse.empty()) {
        ...
        case DecodeContext::SINGLE_BKV_EXPR: {
            ...
            // P2MR PQ single-sig fragments
            if (IsP2MR(ctx.MsContext()) && last - in >= 2 && in[0].first == OP_CHECKSIG_MLDSA && in[1].second.size() == MLDSA44_PUBKEY_SIZE) {
                auto key = ctx.FromPKBytes(in[1].second.begin(), in[1].second.end());
                if (!key) return {};
                in += 2;
                constructed.push_back(MakeNodeRef<Key>(internal::NoDupCheck{}, ctx.MsContext(), Fragment::PK_MLDSA, Vector(std::move(*key))));
                break;
            }
            if (IsP2MR(ctx.MsContext()) && last - in >= 2 && in[0].first == OP_CHECKSIG_SLHDSA && in[1].second.size() == SLHDSA128S_PUBKEY_SIZE) {
                auto key = ctx.FromPKBytes(in[1].second.begin(), in[1].second.end());
                if (!key) return {};
                in += 2;
                constructed.push_back(MakeNodeRef<Key>(internal::NoDupCheck{}, ctx.MsContext(), Fragment::PK_SLHDSA, Vector(std::move(*key))));
            ...
            // Tapscript's equivalent of multi, plus P2MR PQ multi variants.
            if (last - in >= 4 && in[0].first == OP_NUMEQUAL) {
                ...
                if (IsP2MR(ctx.MsContext())) {
                    enum class P2MRMultiMode { MLDSA, SLHDSA };
                    auto parse_p2mr_multi = [&](const P2MRMultiMode mode, Fragment fragment, const opcodetype checksig, const opcodetype checksigadd, const size_t key_size) -> bool {
                        std::vector<Key> mode_keys;
                        for (int pos = 2;; pos += 2) {
                            if (last - in < pos + 2) return false;
                            if (in[pos].first != checksigadd && in[pos].first != checksig) return false;
                            if (in[pos + 1].second.size() != key_size) return false;
                            auto key = ctx.FromPKBytes(in[pos + 1].second.begin(), in[pos + 1].second.end());
                            if (!key) return false;
                            mode_keys.push_back(std::move(*key));
                            if (mode_keys.size() > MAX_PQ_PUBKEYS_PER_MULTISIG) return false;
                            if (in[pos].first == checksig) break;
                        }
                        if (mode == P2MRMultiMode::SLHDSA && *k != 1) return false;
                        if (mode_keys.size() < (size_t)*k) return false;
                        in += 2 + mode_keys.size() * 2;
                        std::reverse(mode_keys.begin(), mode_keys.end());
                        constructed.push_back(MakeNodeRef<Key>(internal::NoDupCheck{}, ctx.MsContext(), fragment, std::move(mode_keys), *k));
                        return true;
                    };

                    if (parse_p2mr_multi(P2MRMultiMode::MLDSA, Fragment::MULTI_MLDSA, OP_CHECKSIG_MLDSA, OP_CHECKSIGADD_MLDSA, MLDSA44_PUBKEY_SIZE)) break;
                    if (parse_p2mr_multi(P2MRMultiMode::SLHDSA, Fragment::MULTI_SLHDSA, OP_CHECKSIG_SLHDSA, OP_CHECKSIGADD_SLHDSA, SLHDSA128S_PUBKEY_SIZE)) break;
                    return {};
                }
                return {};
            }
...
inline NodeRef<typename Ctx::Key> FromScript(const CScript& script, const Ctx& ctx) {
    try {
        ...
        // Parsing malformed scripts should fail closed, never terminate callers.
        return {};
    }
}
