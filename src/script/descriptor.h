...
using PQPubKeyMap = std::unordered_map<uint32_t, std::vector<unsigned char>>;
using PQPubKeyExprMap = std::unordered_map<uint32_t, PQPubKeyMap>;
...
private:
    ...
    /** Map key expression index -> map of (key derivation index -> PQ pubkey bytes). */
    PQPubKeyExprMap m_pq_pubkeys_mldsa;
    PQPubKeyExprMap m_pq_pubkeys_slh;
...
public:
    ...
    /** Cache a PQ public key derived at an index.
     *
     * This enables ExpandFromCache() to compute P2MR scriptPubKeys without access
     * to private key material.
     */
    void CacheDerivedPQPubKey(PQAlgorithm algo, uint32_t key_exp_pos, uint32_t der_index, Span<const unsigned char> pubkey);

    /** Retrieve a cached PQ public key derived at an index. */
    bool GetCachedDerivedPQPubKey(PQAlgorithm algo, uint32_t key_exp_pos, uint32_t der_index, std::vector<unsigned char>& pubkey) const;
    ...
    /** Retrieve all cached derived PQ pubkeys for the specified algorithm. */
    PQPubKeyExprMap GetCachedDerivedPQPubKeys(PQAlgorithm algo) const;
...
struct Descriptor {
    ...
    /** Inject a PQ master seed into any PQHDPubkeyProviders in this descriptor tree.
     *
     *  After deserialization from the wallet DB, PQ-native descriptors contain only
     *  fingerprint-form providers (zero seed). This method walks the pubkey args and
     *  subdescriptors, injecting the real seed so keys can be derived.
     *
     *  @param[in] seed 32-byte PQ master seed
     */
    virtual void InjectPQSeed(Span<const unsigned char> seed) {}

    /** Extract PQ master seed from this descriptor's PQHDPubkeyProviders, if present.
     *
     *  Returns the 32-byte seed if any PQ-native HD provider in the tree has
     *  a loaded seed. Used during descriptor import to persist the seed
     *  separately from the fingerprint-only serialized form.
     *
     *  @return The seed bytes, or empty if no PQ seed is present.
     */
    virtual std::optional<std::array<unsigned char, 32>> ExtractPQSeed() const { return std::nullopt; }

    /** Extract ALL PQ seeds from this descriptor, keyed by fingerprint.
     *
     *  For multisig descriptors with multiple pqhd() providers (potentially
     *  using different seeds), this collects every unique {fingerprint, seed}
     *  pair so all can be persisted and later re-injected.
     */
    virtual std::vector<std::pair<std::array<unsigned char, 4>, std::array<unsigned char, 32>>> ExtractAllPQSeeds() const { return {}; }

    /** Inject a PQ seed into only the provider whose fingerprint matches.
     *
     *  Unlike InjectPQSeed() which injects into ALL seedless providers,
     *  this targets a specific provider. Used when restoring multisig
     *  descriptors that hold providers from different seeds.
     */
    virtual void InjectPQSeedByFingerprint(const std::array<unsigned char, 4>& fingerprint, Span<const unsigned char> seed) {}
};
