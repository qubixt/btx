...
[[nodiscard]] UniValue GetShieldedRetentionInfo(const ChainstateManager& chainman)
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    AssertLockHeld(::cs_main);

    const int64_t target_spacing_seconds = chainman.GetConsensus().PowTargetSpacing().count();
    const int64_t snapshot_target_blocks = target_spacing_seconds > 0
        ? (SNAPSHOT_TARGET_DAYS * SECONDS_PER_DAY) / target_spacing_seconds
        : 0;

    UniValue info(UniValue::VOBJ);
    info.pushKV("profile", chainman.RetainShieldedCommitmentIndex() ? "full_commitment_index"
                                                                    : "externalized");
    info.pushKV("retain_shielded_commitment_index", chainman.RetainShieldedCommitmentIndex());
    info.pushKV("snapshot_target_bytes",
                static_cast<int64_t>(shielded::BridgeShieldedStateRetentionPolicy::WEEKLY_SNAPSHOT_TARGET_BYTES));
    info.pushKV("snapshot_target_days", SNAPSHOT_TARGET_DAYS);
    info.pushKV("snapshot_target_blocks", snapshot_target_blocks);
    info.pushKV("snapshot_target_spacing_seconds", target_spacing_seconds);
    return info;
}

[[nodiscard]] UniValue GetSnapshotSyncInfo(const ChainstateManager& chainman)
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    AssertLockHeld(::cs_main);

    const bool background_validation_in_progress = chainman.BackgroundSyncInProgress();
    const auto snapshot_blockhash = chainman.SnapshotBlockhash();
    const auto snapshot_base_height = chainman.GetSnapshotBaseHeight();

    UniValue info(UniValue::VOBJ);
    info.pushKV("active", snapshot_blockhash.has_value());
    info.pushKV("background_validation_in_progress", background_validation_in_progress);
    if (snapshot_blockhash.has_value()) {
        info.pushKV("base_blockhash", snapshot_blockhash->ToString());
    }
    if (snapshot_base_height.has_value()) {
        info.pushKV("base_height", *snapshot_base_height);
    }
    return info;
}
...
static int SaturatingIntFromInt64(const int64_t value)
{
...
{
    if (blockindex.nHeight == std::numeric_limits<int>::max()) {
        next = nullptr;
    ...
    if (next && next->pprev == &blockindex) {
        const int64_t confirmations64{
            static_cast<int64_t>(tip.nHeight) - static_cast<int64_t>(blockindex.nHeight) + 1};
        return confirmations64 > std::numeric_limits<int>::max()
            ? std::numeric_limits<int>::max()
            : static_cast<int>(confirmations64);
    }
    next = nullptr;
    return &blockindex == &tip ? 1 : -1;
}
...
UniValue blockToJSON(BlockManager& blockman, const CBlock& block, const CBlockIndex& tip, const CBlockIndex& blockindex, TxVerbosity verbosity, const uint256 pow_limit)
{
    ...
    if (block.matmul_dim != 0 || !block.matmul_digest.IsNull() || !block.seed_a.IsNull() || !block.seed_b.IsNull()) {
        ...
        if (!block.matrix_c_data.empty()) {
            result.pushKV("matrix_c_words", static_cast<uint64_t>(block.matrix_c_data.size()));
        }
    }

    result.pushKV("strippedsize", (int)::GetSerializeSize(TX_NO_WITNESS_WITH_SHIELDED(block)));
...
static RPCHelpMan sweepprivkeys()
{
    return RPCHelpMan{"sweepprivkeys",
        "\nSends BTX controlled by private key to specified destinations.\n",
        {
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::NO, "",
                {
                    ...
                    {"label", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "Label for received BTX"},
                },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    node.wallet_loader->assignContext(wallet_req.context);
                ...
                },
                {
                    RPCResult{"for verbose = true",
                        RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::STR_HEX, "nonce64", /*optional=*/true, "The 64-bit BTX nonce"},
                            {RPCResult::Type::STR_HEX, "mixhash", /*optional=*/true, "The KAWPOW mix hash"},
                            {RPCResult::Type::STR_HEX, "matmul_digest", /*optional=*/true, "The MatMul transcript digest"},
                            {RPCResult::Type::NUM, "matmul_dim", /*optional=*/true, "The MatMul matrix dimension (n)"},
                            {RPCResult::Type::STR_HEX, "seed_a", /*optional=*/true, "The MatMul matrix seed A"},
                            {RPCResult::Type::STR_HEX, "seed_b", /*optional=*/true, "The MatMul matrix seed B"},
        ...
        {RPCResult::Type::OBJ, "", "",
        {
            ...
            {RPCResult::Type::OBJ, "prevout", "(Only if undo information is available)",
            {
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                {
                    ...
                    {RPCResult::Type::STR, "address", /*optional=*/true, "The BTX address (only if a well-defined address exists)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (coin->nHeight == MEMPOOL_HEIGHT) {
        ...
        if (assumed_base_height && coin->nHeight < *assumed_base_height) {
            ret.pushKV("confirmations", 0);
            ret.pushKV("confirmations_assumed",
                static_cast<int64_t>(pindex->nHeight) - static_cast<int64_t>(coin->nHeight) + 1);
        } else {
...
static void SoftForkDescPushBack(const CBlockIndex* blockindex, UniValue& softforks, const ChainstateManager& chainman, Consensus::DeploymentPos id)
{
    ...
    if (has_signal) {
        ...
        statsUV.pushKV(
            "period_start",
            SaturatingIntFromInt64(
                static_cast<int64_t>(NextBlockHeightOrLimit(blockindex->nHeight)) -
                static_cast<int64_t>(statsStruct.elapsed)));
    ...
    return RPCHelpMan{"getblockchaininfo",
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::OBJ, "shielded_retention", "the active shielded retained-state posture and modeled weekly snapshot cadence", {
                    {RPCResult::Type::STR, "profile", "the shielded retained-state profile active on this node (`externalized` by default, `full_commitment_index` for the secondary dev/audit mode)"},
                    ...
                    {RPCResult::Type::NUM, "snapshot_target_bytes", "the retained-state snapshot size target used for the production weekly cadence"},
                    {RPCResult::Type::NUM, "snapshot_target_days", "the modeled retained-state snapshot cadence in days"},
                    {RPCResult::Type::NUM, "snapshot_target_blocks", "the modeled retained-state snapshot cadence in blocks at the current consensus target spacing"},
                    {RPCResult::Type::NUM, "snapshot_target_spacing_seconds", "the consensus target spacing used to derive the weekly snapshot cadence"},
                }},
                {RPCResult::Type::OBJ, "snapshot_sync", "snapshot-backed chainstate lifecycle status for assumeutxo recovery", {
                    {RPCResult::Type::BOOL, "active", "whether the active chainstate is currently backed by a UTXO snapshot"},
                    {RPCResult::Type::BOOL, "background_validation_in_progress", "whether the historical chainstate is still validating in the background"},
                    {RPCResult::Type::STR_HEX, "base_blockhash", /*optional=*/true, "the snapshot base block hash while a snapshot-backed chainstate is active"},
                    {RPCResult::Type::NUM, "base_height", /*optional=*/true, "the snapshot base height while a snapshot-backed chainstate is active"},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    obj.pushKV("shielded_retention", GetShieldedRetentionInfo(chainman));
    obj.pushKV("snapshot_sync", GetSnapshotSyncInfo(chainman));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (action == "status") {
        ...
        for (const auto& it : coins) {
            ...
            unspent.pushKV("confirmations",
                           static_cast<int64_t>(tip->nHeight) - static_cast<int64_t>(coin.nHeight) + 1);
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        RPCExamples{
            HelpExampleCli("scanblocks", "start '[\"addr(btxrt1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy48l2gcc)\"]' 300000") +
            HelpExampleCli("scanblocks", "start '[\"addr(btxrt1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy48l2gcc)\"]' 100 150 basic") +
            HelpExampleCli("scanblocks", "status") +
            HelpExampleRpc("scanblocks", "\"start\", [\"addr(btxrt1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy48l2gcc)\"], 300000") +
            HelpExampleRpc("scanblocks", "\"start\", [\"addr(btxrt1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy48l2gcc)\"], 100, 150, \"basic\"") +
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params[0].get_str() == "status") {
        ...
        do {
            ...
            const int start_block = !end_range ? start_index->nHeight : NextBlockHeightOrLimit(start_index->nHeight); // to not include the previous round 'end_range' block
            const int64_t chunk_end_height_64 = static_cast<int64_t>(start_block) + amount_per_chunk;
            const bool use_chunk_end = chunk_end_height_64 < static_cast<int64_t>(stop_block->nHeight);
            const int chunk_end_height = chunk_end_height_64 > std::numeric_limits<int>::max()
                ? std::numeric_limits<int>::max()
                : static_cast<int>(chunk_end_height_64);
            end_range = use_chunk_end
                    ? WITH_LOCK(::cs_main, return chainman.ActiveChain()[chunk_end_height])
                    : stop_block;
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "", {
                {RPCResult::Type::ARR, "activity", "events", {
                    ...
                    // NOTE is the skip_type_check avoidable with a heterogeneous ARR?
                }, /*skip_type_check=*/true},
            },
        },
        RPCExamples{
    ...
    const std::function<void()>& interruption_point)
{
    ...
    std::optional<ShieldedSnapshotSectionHeader> shielded_section;
    if (!is_human_readable) {
    {
        LOCK(::cs_main);
        if (!chainstate.m_chainman.EnsureShieldedStateInitialized()) {
            throw std::ios_base::failure("Failed to initialize BTX shielded state for snapshot");
        }
        shielded_section = chainstate.m_chainman.GetShieldedSnapshotSectionHeader(chainstate, tip);
    }
    ...
    result.pushKV("shielded_retention_profile",
                  chainstate.m_chainman.RetainShieldedCommitmentIndex() ? "full_commitment_index"
                                                                        : "externalized");
...
static RPCHelpMan loadtxoutset()
{
    return RPCHelpMan{
        ...
        "The result is a usable btxd instance that is current with the network tip in a "
            ...
            RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::STR, "shielded_retention_profile", "the shielded retained-state profile active during snapshot activation (`externalized` by default, `full_commitment_index` for the secondary dev/audit mode)"},
                    {RPCResult::Type::BOOL, "retain_shielded_commitment_index", "whether the shielded commitment-position index is being retained on disk"},
                }
        },
        RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    result.pushKV("shielded_retention_profile",
                  chainman.RetainShieldedCommitmentIndex() ? "full_commitment_index"
                                                           : "externalized");
    result.pushKV("retain_shielded_commitment_index", chainman.RetainShieldedCommitmentIndex());
    return result;
},
    };
}
