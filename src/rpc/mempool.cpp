        ...
        "\nRelated RPCs: createrawtransaction, signrawtransactionwithkey\n",
        {
            ...
            {"maxfeerate", RPCArg::Type::AMOUNT, RPCArg::Default{FormatMoney(DEFAULT_MAX_RAW_TX_FEE_RATE.GetFeePerK())},
             "Reject transactions whose fee rate is higher than the specified value, expressed in " + CURRENCY_UNIT +
                 "/kvB.\nFee rates larger than 1BTX/kvB are rejected.\nSet to 0 to accept any fee rate.",
...
UniValue MempoolToJSON(ChainstateManager &chainman, const CTxMemPool& pool, bool verbose, bool include_mempool_sequence)
{
    if (verbose) {
        ...
        const int next_block_height = NextBlockHeightOrLimit(active_chain.Height());
        LOCK(pool.cs);
        // NOTE: Release cs_main after mempool.cs acquired
