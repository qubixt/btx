...
bool DecodePQMultisigLeafScript(const CScript& script, PQMultisigScriptInfo& out)
{
    ...
    while (it != script.end()) {
        ...
        const bool is_pubkey_push = data.size() == MLDSA44_PUBKEY_SIZE || data.size() == SLHDSA128S_PUBKEY_SIZE;
        if (is_pubkey_push) {
            const bool is_mldsa = data.size() == MLDSA44_PUBKEY_SIZE;
            opcodetype checksig_opcode;
            if (!script.GetOp(it, checksig_opcode)) return false;
            if (first) {
                if ((is_mldsa && checksig_opcode != OP_CHECKSIG_MLDSA) ||
                    (!is_mldsa && checksig_opcode != OP_CHECKSIG_SLHDSA)) {
                    return false;
                }
            } else {
                if ((is_mldsa && checksig_opcode != OP_CHECKSIGADD_MLDSA) ||
                    (!is_mldsa && checksig_opcode != OP_CHECKSIGADD_SLHDSA)) {
                    return false;
                }
            }
            out.algorithms.push_back(is_mldsa ? "ml-dsa-44" : "slh-dsa-shake-128s");
            first = false;
            continue;
        }
        ...
        try {
            if (opcode == OP_0) {
                ...
                threshold = CScript::DecodeOP_N(opcode);
            } else {
                threshold = CScriptNum(data, /*fRequireMinimal=*/true).GetInt64();
            }
        } catch (const scriptnum_error&) {
            return false;
        }

        opcodetype numequal_opcode;
        if (!script.GetOp(it, numequal_opcode) || numequal_opcode != OP_NUMEQUAL) return false;
        if (it != script.end()) return false;
        if (out.algorithms.empty()) return false;
        if (threshold < 1 || threshold > static_cast<int64_t>(out.algorithms.size())) return false;
...
std::vector<RPCResult> DecodeTxDoc(const std::string& txid_field_doc)
{
    return {
        ...
        {RPCResult::Type::OBJ, "shielded", /*optional=*/true, "Decoded shielded bundle data for legacy or shielded_v2 transactions", {
            {RPCResult::Type::ELISION, "", "Includes bundle summary plus family-specific header, payload, shard, chunk, reserve, or settlement fields"},
        }},
    };
}
    ...
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::OBJ, "pq_multisig", /*optional=*/true, "Decoded PQ multisig leaf metadata (when script matches P2MR CHECKSIGADD multisig pattern)",
                {
                    {RPCResult::Type::NUM, "threshold", "The required number of signatures"},
                    {RPCResult::Type::NUM, "keys", "The number of pubkeys in the multisig leaf"},
                    {RPCResult::Type::ARR, "algorithms", "Algorithm per key in script order",
                    {
                        {RPCResult::Type::STR, "", "One of \"ml-dsa-44\" or \"slh-dsa-shake-128s\""},
                    }},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    PQMultisigScriptInfo pq_multisig;
    if (DecodePQMultisigLeafScript(script, pq_multisig)) {
        UniValue info(UniValue::VOBJ);
        info.pushKV("threshold", pq_multisig.threshold);
        info.pushKV("keys", static_cast<int>(pq_multisig.algorithms.size()));
        UniValue algos(UniValue::VARR);
        for (const auto& algo : pq_multisig.algorithms) {
            algos.push_back(algo);
        }
        info.pushKV("algorithms", std::move(algos));
        r.pushKV("pq_multisig", std::move(info));
    }
...
const RPCResult decodepsbt_inputs{
    RPCResult::Type::ARR, "inputs", "",
    {
        {RPCResult::Type::OBJ, "", "",
        {
            ...
            {RPCResult::Type::STR_HEX, "p2mr_leaf_script", /*optional=*/true, "The hex-encoded selected P2MR leaf script"},
            {RPCResult::Type::NUM, "p2mr_leaf_version", /*optional=*/true, "The selected P2MR leaf version"},
            {RPCResult::Type::STR_HEX, "p2mr_control_block", /*optional=*/true, "The hex-encoded selected P2MR control block"},
            {RPCResult::Type::STR_HEX, "p2mr_leaf_hash", /*optional=*/true, "The hex-encoded selected P2MR leaf hash"},
            {RPCResult::Type::ARR, "p2mr_partial_signatures", /*optional=*/true, "PQ partial signatures collected for the selected P2MR spend",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR_HEX, "leaf_hash", "The leaf hash associated with this signature"},
                    {RPCResult::Type::STR_HEX, "pubkey", "The PQ pubkey associated with this signature"},
                    {RPCResult::Type::STR_HEX, "signature", "The PQ signature bytes"},
                }},
            }},
            {RPCResult::Type::ARR, "p2mr_bip32_derivs", /*optional=*/true, "P2MR key origin data keyed by PQ pubkey",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR_HEX, "pubkey", "The PQ pubkey this path corresponds to"},
                    {RPCResult::Type::STR, "master_fingerprint", /*optional=*/true, "The fingerprint of the master key"},
                    {RPCResult::Type::STR, "path", /*optional=*/true, "The path"},
                    {RPCResult::Type::STR_HEX, "key_origin_bytes", "The raw serialized key-origin bytes"},
                }},
            }},
            {RPCResult::Type::ARR, "p2mr_csfs_messages", /*optional=*/true, "CSFS messages collected for the selected P2MR spend",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR_HEX, "leaf_hash", "The leaf hash associated with this message"},
                    {RPCResult::Type::STR_HEX, "pubkey", "The PQ pubkey that must produce the CSFS signature"},
                    {RPCResult::Type::STR_HEX, "message", "The CSFS message bytes"},
                }},
            }},
            {RPCResult::Type::ARR, "p2mr_csfs_signatures", /*optional=*/true, "CSFS signatures collected for the selected P2MR spend",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR_HEX, "leaf_hash", "The leaf hash associated with this signature"},
                    {RPCResult::Type::STR_HEX, "pubkey", "The PQ pubkey associated with this signature"},
                    {RPCResult::Type::STR_HEX, "signature", "The CSFS signature bytes"},
                }},
            }},
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed BTX transaction.",
                {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (unsigned int i = 0; i < psbtx.inputs.size(); ++i) {
        ...
        if (!input.m_p2mr_merkle_root.IsNull()) {
            in.pushKV("p2mr_merkle_root", input.m_p2mr_merkle_root.GetHex());
        }
        if (!input.m_p2mr_leaf_script.empty()) {
            in.pushKV("p2mr_leaf_script", HexStr(input.m_p2mr_leaf_script));
            in.pushKV("p2mr_leaf_version", static_cast<uint64_t>(input.m_p2mr_leaf_version));
            in.pushKV("p2mr_leaf_hash", ComputeP2MRLeafHash(input.m_p2mr_leaf_version, input.m_p2mr_leaf_script).GetHex());
        }
        if (!input.m_p2mr_control_block.empty()) {
            in.pushKV("p2mr_control_block", HexStr(input.m_p2mr_control_block));
        }
        if (!input.m_p2mr_pq_sigs.empty()) {
            UniValue p2mr_partial_sigs(UniValue::VARR);
            for (const auto& [leaf_pubkey, sig] : input.m_p2mr_pq_sigs) {
                const auto& [leaf_hash, pubkey] = leaf_pubkey;
                UniValue entry(UniValue::VOBJ);
                entry.pushKV("leaf_hash", leaf_hash.GetHex());
                entry.pushKV("pubkey", HexStr(pubkey));
                entry.pushKV("signature", HexStr(sig));
                p2mr_partial_sigs.push_back(std::move(entry));
            }
            in.pushKV("p2mr_partial_signatures", std::move(p2mr_partial_sigs));
        }
        if (!input.m_p2mr_bip32_paths.empty()) {
            UniValue p2mr_keypaths(UniValue::VARR);
            for (const auto& [pubkey, key_origin_bytes] : input.m_p2mr_bip32_paths) {
                UniValue entry(UniValue::VOBJ);
                entry.pushKV("pubkey", HexStr(pubkey));
                entry.pushKV("key_origin_bytes", HexStr(key_origin_bytes));
                try {
                    DataStream key_origin_stream{key_origin_bytes};
                    KeyOriginInfo key_origin = DeserializeKeyOrigin(key_origin_stream, key_origin_bytes.size());
                    entry.pushKV("master_fingerprint", strprintf("%08x", ReadBE32(key_origin.fingerprint)));
                    entry.pushKV("path", WriteHDKeypath(key_origin.path));
                } catch (const std::exception&) {
                }
                p2mr_keypaths.push_back(std::move(entry));
            }
            in.pushKV("p2mr_bip32_derivs", std::move(p2mr_keypaths));
        }
        if (!input.m_p2mr_csfs_msgs.empty()) {
            UniValue p2mr_csfs_messages(UniValue::VARR);
            for (const auto& [leaf_pubkey, msg] : input.m_p2mr_csfs_msgs) {
                ...
                p2mr_csfs_messages.push_back(std::move(entry));
            }
            in.pushKV("p2mr_csfs_messages", std::move(p2mr_csfs_messages));
        }
        if (!input.m_p2mr_csfs_sigs.empty()) {
            UniValue p2mr_csfs_sigs(UniValue::VARR);
            for (const auto& [leaf_pubkey, sig] : input.m_p2mr_csfs_sigs) {
                ...
                entry.pushKV("pubkey", HexStr(pubkey));
                entry.pushKV("signature", HexStr(sig));
                p2mr_csfs_sigs.push_back(std::move(entry));
            }
            in.pushKV("p2mr_csfs_signatures", std::move(p2mr_csfs_sigs));
        }
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed BTX transactions into one transaction.\n"
