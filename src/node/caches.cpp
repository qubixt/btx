...
//! Keep non-mainnet default memory footprint smaller for local/dev mining sessions.
static constexpr size_t DEFAULT_NONMAINNET_DB_CACHE{64_MiB};
...
CacheSizes CalculateCacheSizes(const ArgsManager& args, size_t n_indexes)
{
    ...
    } else if (args.GetChainType() != ChainType::MAIN) {
        total_cache = std::min(total_cache, DEFAULT_NONMAINNET_DB_CACHE);
    }
