...
bool FillBlock(const CBlockIndex* index, const FoundBlock& block, UniqueLock<RecursiveMutex>& lock, const CChain& active, const BlockManager& blockman)
{
    ...
    if (block.m_next_block) FillBlock(active.Next(index), *block.m_next_block, lock, active, blockman);
    if (block.m_data) {
        REVERSE_LOCK(lock);
...
public:
    ...
    std::optional<std::pair<uint256, uint64_t>> getShieldedTreeInfo() override
    {
        LOCK(::cs_main);
        if (!chainman().HasShieldedState()) return std::nullopt;
        const auto& tree = chainman().GetShieldedMerkleTree();
        return std::make_pair(tree.Root(), tree.Size());
    }
    std::optional<std::pair<uint256, shielded::registry::ShieldedAccountRegistrySpendWitness>>
    getShieldedAccountRegistryWitness(const uint256& account_leaf_commitment) override
    {
        LOCK(::cs_main);
        if (!chainman().HasShieldedState()) return std::nullopt;
        return chainman().GetShieldedAccountRegistryWitness(account_leaf_commitment);
    }
    ...
    {
        if (!old_tip.IsNull()) {
            const auto current_tip = WITH_LOCK(::cs_main, return chainman().ActiveChain().Tip());
            if (current_tip && old_tip == current_tip->GetBlockHash()) return;
        }
...
public:
    ...
    uint64_t getShieldedVerifyUnits() const override
    {
        return m_block_template->nShieldedVerifyUnits;
    }

    uint64_t getShieldedScanUnits() const override
    {
        return m_block_template->nShieldedScanUnits;
    }

    uint64_t getShieldedTreeUpdateUnits() const override
    {
    ...
    std::vector<uint256> getCoinbaseMerklePath() const override
    {
	    ...
	    bool submitSolution(uint32_t version,
	                        uint32_t timestamp,
	                        uint32_t nonce,
	                        ...
	                        std::optional<uint256> mix_hash) override
	    {
	        ...
	        block.nNonce64 = nonce64.value_or(nonce);
	        block.nNonce = static_cast<uint32_t>(block.nNonce64);
	        if (mix_hash.has_value()) {
	        ...
	        if (consensus.fMatMulPOW) {
	            ...
	            // Derive it deterministically for the provided nonce. Note: this
	            // re-derives the full MatMul proof for a single attempt. If the
	            // externally-provided nonce does not produce a valid digest in
...
public:
    ...
    {
        if (!WITH_LOCK(::cs_main, return chainman().ActiveChain().Tip())) {
            throw std::runtime_error("createNewBlock2: chain tip is not yet initialized");
        }
