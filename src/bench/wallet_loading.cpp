...
{
    static uint32_t next_lock_time{0};
    CMutableTransaction mtx;
    mtx.nLockTime = next_lock_time++; // Ensure unique txids for repeated AddToWallet calls.
    const std::vector<unsigned char> p2mr_program(32, 0x01);
    const CScript p2mr_script = CScript{} << OP_2 << p2mr_program;
    mtx.vout.emplace_back(COIN, p2mr_script);
...
BENCHMARK(WalletLoadingDescriptors, benchmark::PriorityLevel::LOW);
#endif
} // namespace wallet
