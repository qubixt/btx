...
public:
    ...
    //! Returns true if tracker is empty, false if timeout was reached.
    bool WaitUntilEmpty(std::chrono::seconds timeout = std::chrono::seconds{30}) const EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
    {
        WAIT_LOCK(m_mutex, lock);
        return m_cv.wait_for(lock, timeout, [this]() EXCLUSIVE_LOCKS_REQUIRED(m_mutex) { return m_tracker.empty(); });
    }
};
...
void StopHTTPServer()
{
    ...
    boundSockets.clear();
    {
        ...
        if (!g_requests.WaitUntilEmpty(std::chrono::seconds{30})) {
            LogPrintf("Warning: %d HTTP connections still active after 30s timeout, proceeding with shutdown\n",
