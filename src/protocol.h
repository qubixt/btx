...
/** Minimum protocol version that guarantees shielded relay support. */
static constexpr int SHIELDED_VERSION{800001};
/**
...
/** The shieldedtx message relays a shielded transaction payload. */
inline constexpr const char* SHIELDEDTX{"shieldedtx"};
/**
 * Request shielded bundle data for a specific block.
 * The command string is constrained by the 12-byte P2P command limit.
 */
inline constexpr const char* GETSHIELDEDDATA{"getshlddata"};
/** Response carrying shielded bundle data for a block. */
inline constexpr const char* SHIELDEDDATA{"shieldeddata"};
/**
...
/**
 * The dandelionacc message signals Dandelion++ support during handshake.
 * Empty payload.
 */
...
enum ServiceFlags : uint64_t {
    ...
    // witness data.
    NODE_WITNESS = (1 << 3),
    // NODE_SHIELDED indicates support for shielded transaction relay.
    NODE_SHIELDED = (1 << 8),
    ...
    // NODE_MATMUL_ECONOMIC indicates the node runs MatMul Phase 1-only
    // validation (Tier 2 economic behavior).
    NODE_MATMUL_ECONOMIC = (1 << 28),
