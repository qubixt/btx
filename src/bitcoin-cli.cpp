...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: btx-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to btxd). This changes the RPC endpoint used, e.g. http://127.0.0.1:19334/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-stdin", "Read extra arguments from standard input, one per line until EOF/Ctrl-D (recommended for sensitive information such as passphrases). When combined with -stdinrpcpass, the first line from standard input is used for the RPC password.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-stdinrpcpass", "Read RPC password from standard input as a single line. When combined with -stdin, the first line from standard input is used for the RPC password. When combined with -stdinwalletpassphrase or -stdinbundlepassphrase, -stdinrpcpass consumes the first line before those passphrases are read.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-stdinwalletpassphrase", "Read wallet passphrase from standard input as a single line for walletpassphrase(change), backupwalletbundle, or backupwalletbundlearchive. When combined with -stdin, the wallet passphrase line is consumed before any remaining stdin arguments.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-stdinbundlepassphrase", "Read wallet bundle archive passphrase from standard input as a single line for backupwalletbundlearchive or restorewalletbundlearchive. When combined with -stdin, the bundle passphrase line is consumed before any remaining stdin arguments.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
}
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                ...
                "\n"
                "Usage: btx-cli [options] <command> [params]\n"
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires btxd server to be running v0.21.1.knots or newer");
        }
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the btxd server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"btx-cli -help\" for more info.",
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    ...
    if (error.isObject()) {
        ...
        if (err_code.isNum()) code = err_code.getInt<int>();
        if (!err_code.isNull()) {
    ...
    // Avoid UB for abs(INT_MIN) if a malicious/non-standard RPC endpoint returns it.
    nRet = (code == std::numeric_limits<int>::min()) ? std::numeric_limits<int>::max() : std::abs(code);
}
...
static int CommandLineRPC(int argc, char *argv[])
{
    ...
    try {
        ...
        if (gArgs.GetBoolArg("-stdinwalletpassphrase", false)) {
            NO_STDIN_ECHO();
            const bool is_walletpassphrase = args.size() >= 1 && args[0].substr(0, 16) == "walletpassphrase";
            const bool is_backupwalletbundle = args.size() >= 1 && args[0] == "backupwalletbundle";
            const bool is_backupwalletbundlearchive = args.size() >= 1 && args[0] == "backupwalletbundlearchive";
            if (!is_walletpassphrase && !is_backupwalletbundle && !is_backupwalletbundlearchive) {
                throw std::runtime_error("-stdinwalletpassphrase is only applicable for walletpassphrase(change), backupwalletbundle, or backupwalletbundlearchive");
            }
            if (!StdinReady()) {
                fputs("Wallet passphrase> ", stderr);
                fflush(stderr);
            }
            wallet_passphrase.emplace();
            if (!std::getline(std::cin, *wallet_passphrase)) {
        ...
        std::optional<std::string> bundle_passphrase;
        if (gArgs.GetBoolArg("-stdinbundlepassphrase", false)) {
            NO_STDIN_ECHO();
            const bool is_backupwalletbundlearchive = args.size() >= 1 && args[0] == "backupwalletbundlearchive";
            const bool is_restorewalletbundlearchive = args.size() >= 1 && args[0] == "restorewalletbundlearchive";
            if (!is_backupwalletbundlearchive && !is_restorewalletbundlearchive) {
                throw std::runtime_error("-stdinbundlepassphrase is only applicable for backupwalletbundlearchive or restorewalletbundlearchive");
            }
            if (!StdinReady()) {
                fputs("Bundle passphrase> ", stderr);
                fflush(stderr);
            }
            bundle_passphrase.emplace();
            if (!std::getline(std::cin, *bundle_passphrase)) {
                throw std::runtime_error("-stdinbundlepassphrase specified but failed to read from standard input");
            }
            if (StdinTerminal()) {
                fputc('\n', stdout);
            }
        }
        if (bundle_passphrase.has_value()) {
            const bool is_backupwalletbundlearchive = args.size() >= 1 && args[0] == "backupwalletbundlearchive";
            const bool is_restorewalletbundlearchive = args.size() >= 1 && args[0] == "restorewalletbundlearchive";
            if (is_backupwalletbundlearchive) {
                if (args.size() < 2) {
                    throw std::runtime_error("backupwalletbundlearchive requires a destination argument before using -stdinbundlepassphrase");
                }
                args.insert(args.begin() + 2, *bundle_passphrase);
            } else if (is_restorewalletbundlearchive) {
                if (args.size() < 3) {
                    throw std::runtime_error("restorewalletbundlearchive requires wallet_name and archive_file arguments before using -stdinbundlepassphrase");
                }
                args.insert(args.begin() + 3, *bundle_passphrase);
            }
        }
        if (wallet_passphrase.has_value()) {
            const bool is_walletpassphrase = args.size() >= 1 && args[0].substr(0, 16) == "walletpassphrase";
            const bool is_backupwalletbundle = args.size() >= 1 && args[0] == "backupwalletbundle";
            const bool is_backupwalletbundlearchive = args.size() >= 1 && args[0] == "backupwalletbundlearchive";
            if (is_backupwalletbundle) {
                if (args.size() < 2) {
                    throw std::runtime_error("backupwalletbundle requires a destination argument before using -stdinwalletpassphrase");
                }
                args.insert(args.begin() + 2, *wallet_passphrase);
            } else if (is_backupwalletbundlearchive) {
                if (args.size() < 3) {
                    throw std::runtime_error("backupwalletbundlearchive requires destination and archive passphrase arguments before using -stdinwalletpassphrase");
                }
                args.insert(args.begin() + 3, *wallet_passphrase);
            } else if (is_walletpassphrase) {
                args.insert(args.begin() + 1, *wallet_passphrase);
            }
        }
...
#endif
    WarnAboutLegacyBinaryInvocation(argv[0] ? argv[0] : "", "bitcoin-cli", "btx-cli");
    SetupEnvironment();
    if (!SetupNetworking()) {
