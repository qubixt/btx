...
inline bool IsValidP2MRPubkeySize(const size_t size)
{
    return size == MLDSA44_PUBKEY_SIZE || size == SLHDSA128S_PUBKEY_SIZE;
}
...
struct PSBTInput
{
    ...
    // P2MR fields.
    std::vector<unsigned char> m_p2mr_leaf_script;
    std::vector<unsigned char> m_p2mr_control_block;
    uint8_t m_p2mr_leaf_version{P2MR_LEAF_VERSION};
    std::map<std::pair<uint256, std::vector<unsigned char>>, std::vector<unsigned char>> m_p2mr_pq_sigs;
    std::map<std::vector<unsigned char>, std::vector<unsigned char>> m_p2mr_bip32_paths;
    uint256 m_p2mr_merkle_root;
    std::map<std::pair<uint256, std::vector<unsigned char>>, std::vector<unsigned char>> m_p2mr_csfs_msgs;
    std::map<std::pair<uint256, std::vector<unsigned char>>, std::vector<unsigned char>> m_p2mr_csfs_sigs;
    ...
    inline void Serialize(Stream& s) const {
        ...
        if (non_witness_utxo) {
            ...
            SerializeToVector(s, TX_NO_WITNESS_WITH_SHIELDED(non_witness_utxo));
        }
        ...
        if (final_script_sig.empty() && final_script_witness.IsNull()) {
            ...
            // Write selected P2MR leaf script (single-leaf PSBT flow).
            if (!m_p2mr_leaf_script.empty() && !m_p2mr_control_block.empty()) {
                SerializeToVector(s, PSBT_IN_P2MR_LEAF_SCRIPT, Span{m_p2mr_control_block});
                std::vector<unsigned char> value{m_p2mr_leaf_script};
                value.push_back(m_p2mr_leaf_version);
                s << value;
            }

            // Write P2MR script-path signatures.
            for (const auto& [leaf_pubkey, sig] : m_p2mr_pq_sigs) {
                const auto& [leaf_hash, pubkey] = leaf_pubkey;
                SerializeToVector(s, PSBT_IN_P2MR_PQ_SIG, leaf_hash, Span{pubkey});
                s << sig;
            }

            // Write P2MR BIP32 derivations.
            for (const auto& [pubkey, value] : m_p2mr_bip32_paths) {
                SerializeToVector(s, PSBT_IN_P2MR_BIP32_DERIVATION, Span{pubkey});
                s << value;
            }
            ...
            // Write CSFS message/signature data keyed by (leaf_hash, pubkey).
            for (const auto& [leaf_pubkey, msg] : m_p2mr_csfs_msgs) {
                const auto& [leaf_hash, pubkey] = leaf_pubkey;
                SerializeToVector(s, PSBT_IN_CSFS_MESSAGE, leaf_hash, Span{pubkey});
                s << msg;
            }
            for (const auto& [leaf_pubkey, sig] : m_p2mr_csfs_sigs) {
                const auto& [leaf_hash, pubkey] = leaf_pubkey;
                SerializeToVector(s, PSBT_IN_CSFS_SIGNATURE, leaf_hash, Span{pubkey});
                s << sig;
            }
        }
            ...
            switch(type) {
                ...
                case PSBT_IN_P2MR_LEAF_SCRIPT:
                {
                    if (!key_lookup.emplace(key).second) {
                        ...
                        throw std::ios_base::failure("Input P2MR leaf script key is missing control block bytes");
                    }
                    ...
                    if (control_block.size() < P2MR_CONTROL_BASE_SIZE ||
                        ...
                        throw std::ios_base::failure("Input P2MR leaf script key has invalid control block size");
                    }

                    std::vector<unsigned char> script_v;
                    s >> script_v;
                    if (script_v.empty()) {
                        throw std::ios_base::failure("Input P2MR leaf script value must contain script and leaf version");
                    }
                    uint8_t leaf_ver = script_v.back();
                    script_v.pop_back();

                    if (!m_p2mr_control_block.empty() &&
                        (m_p2mr_control_block != control_block || m_p2mr_leaf_script != script_v || m_p2mr_leaf_version != leaf_ver)) {
                        throw std::ios_base::failure("Multiple selected P2MR leaves are not supported");
                    }

                    m_p2mr_control_block = std::move(control_block);
                    m_p2mr_leaf_script = std::move(script_v);
                    m_p2mr_leaf_version = leaf_ver;
                    break;
                }
                case PSBT_IN_P2MR_PQ_SIG:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input P2MR script signature already provided");
                    } else if (key.size() <= 1 + uint256::size()) {
                        throw std::ios_base::failure("Input P2MR script signature key is too short");
                    }
                    uint256 leaf_hash{Span<const unsigned char>(key).subspan(1, uint256::size())};
                    std::vector<unsigned char> pubkey(key.begin() + 1 + uint256::size(), key.end());
                    if (!IsValidP2MRPubkeySize(pubkey.size())) {
                        throw std::ios_base::failure("Input P2MR script signature key has invalid pubkey size");
                    }
                    std::vector<unsigned char> sig;
                    s >> sig;
                    m_p2mr_pq_sigs.emplace(std::make_pair(leaf_hash, std::move(pubkey)), std::move(sig));
                    break;
                }
                case PSBT_IN_P2MR_BIP32_DERIVATION:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input P2MR BIP32 keypath already provided");
                    } else if (key.size() < 2) {
                        throw std::ios_base::failure("Input P2MR BIP32 keypath key is missing pubkey bytes");
                    }
                    std::vector<unsigned char> value;
                    s >> value;
                    std::vector<unsigned char> pubkey(key.begin() + 1, key.end());
                    if (!IsValidP2MRPubkeySize(pubkey.size())) {
                        throw std::ios_base::failure("Input P2MR BIP32 keypath key has invalid pubkey size");
                    }
                    m_p2mr_bip32_paths.emplace(std::move(pubkey), std::move(value));
                    break;
                }
                ...
                case PSBT_IN_CSFS_MESSAGE:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input CSFS message already provided");
                    } else if (key.size() <= 1 + uint256::size()) {
                        throw std::ios_base::failure("Input CSFS message key is too short");
                    }
                    uint256 leaf_hash{Span<const unsigned char>(key).subspan(1, uint256::size())};
                    std::vector<unsigned char> pubkey(key.begin() + 1 + uint256::size(), key.end());
                    if (!IsValidP2MRPubkeySize(pubkey.size())) {
                        throw std::ios_base::failure("Input CSFS message key has invalid pubkey size");
                    }
                    std::vector<unsigned char> msg;
                    s >> msg;
                    m_p2mr_csfs_msgs.emplace(std::make_pair(leaf_hash, std::move(pubkey)), std::move(msg));
                    break;
                }
                case PSBT_IN_CSFS_SIGNATURE:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input CSFS signature already provided");
                    } else if (key.size() <= 1 + uint256::size()) {
                        throw std::ios_base::failure("Input CSFS signature key is too short");
                    }
                    uint256 leaf_hash{Span<const unsigned char>(key).subspan(1, uint256::size())};
                    std::vector<unsigned char> pubkey(key.begin() + 1 + uint256::size(), key.end());
                    if (!IsValidP2MRPubkeySize(pubkey.size())) {
                        throw std::ios_base::failure("Input CSFS signature key has invalid pubkey size");
                    }
                    std::vector<unsigned char> sig;
                    s >> sig;
                    m_p2mr_csfs_sigs.emplace(std::make_pair(leaf_hash, std::move(pubkey)), std::move(sig));
                    break;
                }
...
struct PSBTOutput
{
    ...
    // P2MR fields.
    std::vector<unsigned char> m_p2mr_tree;
    std::map<std::vector<unsigned char>, std::vector<unsigned char>> m_p2mr_bip32_paths;
    ...
    inline void Serialize(Stream& s) const {
        ...
        // Write P2MR BIP32 derivations.
        for (const auto& [pubkey, value] : m_p2mr_bip32_paths) {
            SerializeToVector(s, PSBT_OUT_P2MR_BIP32_DERIVATION, Span{pubkey});
            s << value;
        }
    ...
    inline void Unserialize(Stream& s) {
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSBT_OUT_P2MR_BIP32_DERIVATION:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, output P2MR BIP32 keypath already provided");
                    } else if (key.size() < 2) {
                        throw std::ios_base::failure("Output P2MR BIP32 keypath key is missing pubkey bytes");
                    }
                    std::vector<unsigned char> value;
                    s >> value;
                    std::vector<unsigned char> pubkey(key.begin() + 1, key.end());
                    if (!IsValidP2MRPubkeySize(pubkey.size())) {
                        throw std::ios_base::failure("Output P2MR BIP32 keypath key has invalid pubkey size");
                    }
                    m_p2mr_bip32_paths.emplace(std::move(pubkey), std::move(value));
                    break;
                }
...
struct PartiallySignedTransaction
{
    ...
    inline void Serialize(Stream& s) const {
        ...
        SerializeToVector(s, TX_NO_WITNESS_WITH_SHIELDED(*tx));
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                case PSBT_GLOBAL_UNSIGNED_TX:
                {
                    ...
                    UnserializeFromVector(s, TX_NO_WITNESS_WITH_SHIELDED(mtx));
                    tx = std::move(mtx);
