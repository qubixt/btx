...
bool IsWellFormedP2MRPartialSig(const std::vector<unsigned char>& sig, const size_t pubkey_size)
{
    const size_t expected_sig_size =
        pubkey_size == MLDSA44_PUBKEY_SIZE ? MLDSA44_SIGNATURE_SIZE :
        pubkey_size == SLHDSA128S_PUBKEY_SIZE ? SLHDSA128S_SIGNATURE_SIZE :
        0;
...
bool IsValidP2MRPartialSigForInput(const PartiallySignedTransaction& psbt,
                                   const size_t input_index,
                                   const std::pair<uint256, std::vector<unsigned char>>& leaf_pubkey,
                                   const std::vector<unsigned char>& sig,
                                   const PrecomputedTransactionData& txdata)
{
    const std::vector<unsigned char>& pubkey = leaf_pubkey.second;
    const PQAlgorithm algo =
        pubkey.size() == MLDSA44_PUBKEY_SIZE ? PQAlgorithm::ML_DSA_44 :
        pubkey.size() == SLHDSA128S_PUBKEY_SIZE ? PQAlgorithm::SLH_DSA_128S :
        PQAlgorithm::ML_DSA_44; // Placeholder, handled by size checks below.
    if (pubkey.size() != MLDSA44_PUBKEY_SIZE && pubkey.size() != SLHDSA128S_PUBKEY_SIZE) return false;

    const size_t expected_sig_size = algo == PQAlgorithm::ML_DSA_44 ? MLDSA44_SIGNATURE_SIZE : SLHDSA128S_SIGNATURE_SIZE;
    ...
    if (sig_to_check.size() == expected_sig_size + 1) {
        ...
        if (!IsDefinedP2MRSighashType(hash_type)) return false;
    ...
    return creator.Checker().CheckPQSignature(sig_to_check, pubkey, algo, hash_type, SigVersion::P2MR, execdata);
}

void CanonicalizeP2MRPartialSigs(PartiallySignedTransaction& out, const std::vector<PartiallySignedTransaction>& psbtxs)
{
    const PrecomputedTransactionData txdata = PrecomputePSBTData(out);
    for (size_t input_index = 0; input_index < out.inputs.size(); ++input_index) {
        auto& out_input = out.inputs.at(input_index);
        if (out_input.m_p2mr_pq_sigs.empty()) continue;

        // Prefer a deterministic, cryptographically-valid candidate for each (leaf_hash,pubkey),
        // independent of the order PSBTs are combined.
        for (auto& [leaf_pubkey, out_sig] : out_input.m_p2mr_pq_sigs) {
            std::vector<unsigned char> best_sig;
            bool have_best = false;

            for (const auto& psbt : psbtxs) {
                const auto& in = psbt.inputs.at(input_index);
                const auto it = in.m_p2mr_pq_sigs.find(leaf_pubkey);
                if (it == in.m_p2mr_pq_sigs.end()) continue;
                if (!IsValidP2MRPartialSigForInput(out, input_index, leaf_pubkey, it->second, txdata)) continue;

                if (!have_best || std::lexicographical_compare(it->second.begin(), it->second.end(),
                                                               best_sig.begin(), best_sig.end())) {
                    best_sig = it->second;
                    have_best = true;
                }
            }

            if (have_best) out_sig = std::move(best_sig);
        }
    }
}
} // namespace
...
{
    return !non_witness_utxo && witness_utxo.IsNull() && partial_sigs.empty() &&
           unknown.empty() && hd_keypaths.empty() && redeem_script.empty() &&
           witness_script.empty() && m_p2mr_leaf_script.empty() &&
           m_p2mr_control_block.empty() && m_p2mr_pq_sigs.empty() &&
           m_p2mr_bip32_paths.empty() && m_p2mr_merkle_root.IsNull() &&
           m_p2mr_csfs_msgs.empty() && m_p2mr_csfs_sigs.empty();
}
...
void PSBTInput::FillSignatureData(SignatureData& sigdata) const
{
    ...
    if (!m_p2mr_leaf_script.empty() && !m_p2mr_control_block.empty()) {
        ...
        sigdata.p2mr_spenddata.scripts[m_p2mr_leaf_script].insert(m_p2mr_control_block);
    }
    for (const auto& [key, sig] : m_p2mr_pq_sigs) {
        sigdata.p2mr_script_sigs.emplace(key, sig);
    }
    for (const auto& [key, msg] : m_p2mr_csfs_msgs) {
        sigdata.p2mr_csfs_msgs.emplace(key, msg);
    }
    for (const auto& [key, sig] : m_p2mr_csfs_sigs) {
        sigdata.p2mr_csfs_sigs.emplace(key, sig);
    }
...
void PSBTInput::FromSignatureData(const SignatureData& sigdata)
{
    ...
    if (!sigdata.p2mr_leaf_script.empty() && !sigdata.p2mr_control_block.empty()) {
        ...
        const auto& [script, controls] = *sigdata.p2mr_spenddata.scripts.begin();
        if (controls.size() == 1) {
            m_p2mr_leaf_script = script;
            m_p2mr_control_block = *controls.begin();
            m_p2mr_leaf_version = P2MR_LEAF_VERSION;
        }
    }

    for (const auto& [key, sig] : sigdata.p2mr_script_sigs) {
        // Prefer freshly-produced signer material over stale/coordinator-provided cache entries.
        m_p2mr_pq_sigs[key] = sig;
    }
    for (const auto& [key, msg] : sigdata.p2mr_csfs_msgs) {
        m_p2mr_csfs_msgs[key] = msg;
    }
    for (const auto& [key, sig] : sigdata.p2mr_csfs_sigs) {
        m_p2mr_csfs_sigs[key] = sig;
    }
}
...
void PSBTInput::Merge(const PSBTInput& input)
{
    ...
    for (const auto& [leaf_pubkey, sig] : input.m_p2mr_pq_sigs) {
        auto it = m_p2mr_pq_sigs.find(leaf_pubkey);
        if (it == m_p2mr_pq_sigs.end()) {
            m_p2mr_pq_sigs.emplace(leaf_pubkey, sig);
            continue;
        }
        // If an earlier combiner inserted malformed data for this key, allow a well-formed
        // signature from another signer to replace it.
        const bool existing_valid = IsWellFormedP2MRPartialSig(it->second, leaf_pubkey.second.size());
        const bool incoming_valid = IsWellFormedP2MRPartialSig(sig, leaf_pubkey.second.size());
        if (!existing_valid && incoming_valid) {
            it->second = sig;
        }
    }
    m_p2mr_bip32_paths.insert(input.m_p2mr_bip32_paths.begin(), input.m_p2mr_bip32_paths.end());
    m_p2mr_csfs_msgs.insert(input.m_p2mr_csfs_msgs.begin(), input.m_p2mr_csfs_msgs.end());
    m_p2mr_csfs_sigs.insert(input.m_p2mr_csfs_sigs.begin(), input.m_p2mr_csfs_sigs.end());
    ...
    if (!input.m_p2mr_leaf_script.empty() || !input.m_p2mr_control_block.empty()) {
        if (m_p2mr_leaf_script.empty() && m_p2mr_control_block.empty()) {
            m_p2mr_leaf_script = input.m_p2mr_leaf_script;
            m_p2mr_control_block = input.m_p2mr_control_block;
            m_p2mr_leaf_version = input.m_p2mr_leaf_version;
        } else if (m_p2mr_leaf_script != input.m_p2mr_leaf_script ||
                   m_p2mr_control_block != input.m_p2mr_control_block ||
                   m_p2mr_leaf_version != input.m_p2mr_leaf_version) {
            throw std::ios_base::failure("Conflicting selected P2MR leaves");
        }
    }
    if (!input.m_p2mr_merkle_root.IsNull()) {
        if (m_p2mr_merkle_root.IsNull()) {
            m_p2mr_merkle_root = input.m_p2mr_merkle_root;
        } else if (m_p2mr_merkle_root != input.m_p2mr_merkle_root) {
            throw std::ios_base::failure("Conflicting P2MR merkle roots");
        }
    }
}
...
{
    return redeem_script.empty() && witness_script.empty() && hd_keypaths.empty() &&
           unknown.empty() && m_p2mr_tree.empty() && m_p2mr_bip32_paths.empty();
}
...
void PSBTOutput::Merge(const PSBTOutput& output)
{
    ...
    if (m_p2mr_tree.empty() && !output.m_p2mr_tree.empty()) {
        m_p2mr_tree = output.m_p2mr_tree;
    } else if (!m_p2mr_tree.empty() && !output.m_p2mr_tree.empty() && m_p2mr_tree != output.m_p2mr_tree) {
        throw std::ios_base::failure("Conflicting P2MR trees");
    }
}
...
{
    CTxOut utxo;
    if (input_index >= psbt.inputs.size()) return false;
...
bool CombinePSBTs(PartiallySignedTransaction& out, const std::vector<PartiallySignedTransaction>& psbtxs)
{
    ...
    CanonicalizeP2MRPartialSigs(out, psbtxs);
    return true;
}
