...
void CTxMemPoolEntry::UpdateCachedPriority(unsigned int currentHeight, CAmount valueInCurrentBlock)
{
    ...
    // R5-508: Clamp instead of crashing on out-of-range value from corrupted data.
    if (!MoneyRange(inChainInputValue)) {
        inChainInputValue = std::clamp(inChainInputValue, CAmount{0}, MAX_MONEY);
    }
}
