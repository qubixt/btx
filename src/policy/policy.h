...
static constexpr int32_t MAX_STANDARD_TX_WEIGHT{1200000};
/** Shielded policy weight is normalized against the 24 MWU block budget across
 * serialized bytes, verify units, scan units, and tree updates. Give
 * shielded families a dedicated standardness ceiling so wallet-sized
 * `shielded_v2` transactions can relay without inheriting the tighter legacy
 * transparent cap. */
static constexpr int32_t MAX_STANDARD_SHIELDED_POLICY_WEIGHT{2400000};
/** `v2_ingress_batch` scales with both consumed notes and reserve outputs, so
 * keep a dedicated ingress ceiling that covers validated multishard ingress
 * batches without widening the rest of the shielded families to the same
 * limit. */
static constexpr int32_t MAX_STANDARD_INGRESS_SHIELDED_POLICY_WEIGHT{11000000};
/** The minimum non-witness size for transactions we're willing to relay/mine: one larger than 64  */
static constexpr unsigned int MIN_STANDARD_TX_NONWITNESS_SIZE{65};
/** Maximum number of signature check operations in an IsStandard() P2SH script */
static constexpr unsigned int MAX_P2SH_SIGOPS{15};
/** The maximum number of sigops we're willing to relay/mine in a single tx.
 * Raised from /5 to /3 to allow more PQ multisig inputs per transaction. */
static constexpr unsigned int MAX_STANDARD_TX_SIGOPS_COST{MAX_BLOCK_SIGOPS_COST/3};
...
static constexpr unsigned int DEFAULT_ANCESTOR_LIMIT{100};
/** Default for -limitancestorsize, maximum kilobytes of tx + all in-mempool ancestors */
static constexpr unsigned int DEFAULT_ANCESTOR_SIZE_LIMIT_KVB{1024};
