...
public:
    OpCodeParser()
    {
        for (unsigned int op = 0; op <= MAX_OPCODE; ++op) {
            ...
            if (op == OP_CHECKTEMPLATEVERIFY) {
                mapOpNames["OP_NOP4"] = static_cast<opcodetype>(op);
                mapOpNames["NOP4"] = static_cast<opcodetype>(op);
            }
        }
    }
...
bool DecodeHexBlk(CBlock& block, const std::string& strHexBlk)
{
    ...
    // Legacy 80-byte headers can deserialize "successfully" with shifted bytes when the
    // local header format includes extra fields (e.g. BTX extended headers). Validate
    // basic structure to avoid accepting an incorrect decode.
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) return false;
    if (BlockMerkleRoot(block) != block.hashMerkleRoot) return false;
    if (!ssBlock.empty()) return false;
