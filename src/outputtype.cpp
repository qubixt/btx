...
CTxDestination GetDestinationForKey(const CPubKey& key, OutputType type)
{
    ...
    case OutputType::P2MR:
        // P2MR destinations are derived from PQ public keys, not CPubKey.
        // Fall back to legacy key-hash here so legacy CPubKey-only callers remain non-fatal.
        return PKHash(key);
    case OutputType::UNKNOWN: {} // This function should never be used with BECH32M/P2MR/UNKNOWN, so let it assert
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
CTxDestination AddAndGetDestinationForScript(FlatSigningProvider& keystore, const CScript& script, OutputType type)
{
    ...
    case OutputType::BECH32M:
    case OutputType::P2MR:
        // P2MR script destinations are not built from legacy CScript templates.
        // Fall back to legacy script-hash for CPubKey-script callers.
        return ScriptHash(script);
    case OutputType::UNKNOWN: {} // This function should not be used for BECH32M/P2MR/UNKNOWN, so let it assert
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
