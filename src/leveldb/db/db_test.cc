...
TEST(DBTest, CompactionStatsTrackWrittenBytes) {
  Options options = CurrentOptions();
  options.write_buffer_size = 10000;
  Reopen(&options);
  ...
  const int64_t before = dbfull()->TEST_TotalCompactionBytesWritten();
  dbfull()->TEST_CompactRange(0, nullptr, nullptr);
  const int64_t after = dbfull()->TEST_TotalCompactionBytesWritten();
