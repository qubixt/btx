 ...
 *                number. (e.g. example.com:19335 or
...
/**
 * Detect ports that likely belong to a different Bitcoin-family network than
 * the currently selected chain. This is used to reduce cross-network addrman
 * pollution (for example, incoming 8333 gossip while operating on BTX mainnet
 * default port 19335).
 * @param[in] port Candidate peer port.
 * @param[in] default_port Active chain default port.
 * @returns whether the candidate should be treated as cross-network.
 */
bool IsLikelyCrossNetworkPort(uint16_t port, uint16_t default_port);
