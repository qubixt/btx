...
bool OptionsModel::setOption(OptionID option, const QVariant& value, const std::string& suffix)
{
    ...
    case peerblockfilters:
    {
        ...
        if (gArgs.GetBoolArg("-peerblockfilters", DEFAULT_PEERBLOCKFILTERS) != nv) {
            ...
            if (nv && !GetBlockFilterIndex(BlockFilterType::BASIC)) {
                // TO-DO: When other options are possible, we need to append a list!
                // TO-DO: Some way to unset/delete this...
    ...
    case dustdynamic:
        if (changed()) {
            const std::string newvalue_str = value.toString().toStdString();
            const util::Result<std::pair<int32_t, int>> parsed = ParseDustDynamicOpt(newvalue_str, 1008);
            if (!parsed) break; // Silently ignore unparseable values from the UI.
            update(newvalue_str);
