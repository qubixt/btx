    ...
    CTxMemPoolEntry(const CTransactionRef& tx, CAmount fee,
        ...
        : tx{tx},
          ...
          nSigOpCostWithAncestors{sigOpCost} {
            CAmount transparent_value_in = tx->GetValueOut() + nFee;
            if (tx->HasShieldedBundle()) {
                // For shielded transactions, nFee includes value_balance adjustment:
                // fee = transparent_in + value_balance - transparent_out.
                transparent_value_in -= GetShieldedTxValueBalance(tx->GetShieldedBundle());
            }
            // R5-509: Use Assume (non-fatal) instead of assert to avoid node crash
            // from corrupted mempool data or shielded value_balance edge cases.
            Assume(MoneyRangeSigned(transparent_value_in));
            Assume(inChainInputValue <= transparent_value_in);
        }
