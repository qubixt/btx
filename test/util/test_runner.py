...
def bctester(testDir, input_basename, buildenv, component_conf):
    ...
    if not os.path.exists(input_filename) and input_basename == "btx-util-test.json":
        # Backward compatibility for older checkouts/scripts that still use the legacy filename.
        input_filename = os.path.join(testDir, "bitcoin-util-test.json")
    ...
    for testObj in input_data:
        m = re.match(r'^\.\/btx-(\w+)$', testObj['exec'])
...
def bctest(testDir, testObj, buildenv):
    ...
    if testObj["exec"] == "./btx-util":
        execprog = os.getenv("BTXUTIL", default=execprog)
    elif testObj["exec"] == "./btx-tx":
        execprog = os.getenv("BTXTX", default=execprog)
    ...
    if "error_txt" in testObj:
        ...
        # Compare error text
        # TO-DO: ideally, we'd compare the strings exactly and also assert
        # That stderr is empty if no errors are expected. However, btx-tx
