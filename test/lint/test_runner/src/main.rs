...
fn lint_subtree() -> LintResult {
    ...
    for subtree in get_subtrees() {
        let status = Command::new("test/lint/git-subtree-check.sh")
            .arg(subtree)
            .status()
        ...
        // Exit code 2 means "not a subtree"; tolerate this for snapshot-based forks
        // that intentionally vendor these directories without git-subtree metadata.
        if status.code() == Some(2) {
            println!("Skipping subtree lint for {subtree}: no subtree metadata found.");
            continue;
        }
        good = false;
    }
