...
# Disabled warnings:
DISABLED = [
    'SC1091', # shellcheck cannot resolve host-specific sourced files (e.g. /etc/os-release) in all environments.
    'SC2162', # read without -r will mangle backslashes.
    'SC2317', # commands in functions invoked via trap or indirection may appear unreachable to shellcheck.
    'SC2329', # functions invoked via trap or indirection may appear unused to shellcheck.
]
...
def main():
    ...
    reg = re.compile(r'^src/(leveldb|secp256k1|minisketch|libbitcoinpqc)/')
