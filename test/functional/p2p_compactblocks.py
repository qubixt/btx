...
class CompactBlocksTest(BitcoinTestFramework):
    ...
    def test_compactblocks_not_at_tip(self, test_node):
        ...
        block = self.build_block_on_tip(node)
        block.hashPrevBlock = hashPrevBlock
        # BTX retargeting can vary bits across adjacent heights; when
        # constructing a fork header, match the historical child bits.
        historical_child = node.getblockhash(cur_height - 4)
        block.nBits = int(node.getblockheader(historical_child)["bits"], 16)
        block.solve()
    ...
    def test_end_to_end_block_relay(self, listeners):
        ...
        # NOTE: repeat this test with witness tx's to a segwit node.
