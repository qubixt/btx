...
class TaprootTest(BitcoinTestFramework):
    ...
    def block_submit(self, node, txs, msg, err_msg, cb_pubkey=None, fees=0, sigops_weight=0, witness=False, accept=False):
        ...
        # transactions.
        extra_sigops = (MAX_BLOCK_SIGOPS_WEIGHT - sigops_weight) // WITNESS_SCALE_FACTOR
        # BTX reduced-data limits cap non-OP_RETURN scriptPubKey length at 34 bytes.
        extra_sigops = min(extra_sigops, BTX_MAX_TXOUT_SCRIPT_SIZE)
        if extra_sigops > 0:
            coinbase_output_script = CScript(bytes([OP_CHECKSIG] * extra_sigops))
        elif cb_pubkey is not None:
            p2pk_script = key_to_p2pk_script(cb_pubkey)
            coinbase_output_script = p2pk_script if len(p2pk_script) <= BTX_MAX_TXOUT_SCRIPT_SIZE else key_to_p2pkh_script(cb_pubkey)
        else:
            coinbase_output_script = CScript([OP_1])

        # Solve in node code to avoid slow Python header grinding in KAWPOW tests.
        block_obj = node.generateblock(
            output=f"raw({bytes(coinbase_output_script).hex()})",
        ...
        block_response = node.submitblock(block_hex)
        if err_msg is not None:
            assert block_response is not None and err_msg in block_response, "Missing error message '%s' from block response '%s': %s" % (err_msg, "(None)" if block_response is None else block_response, msg)
        if accept:
            best_hash = node.getbestblockhash()
            assert best_hash == block_hash, "Failed to accept: %s (response: %s)" % (msg, block_response)
            header = node.getblockheader(best_hash)
            self.tip = int(best_hash, 16)
            self.lastblockhash = best_hash
            self.lastblocktime = header["time"]
            self.lastblockheight = header["height"]
        else:
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        if BTX_MAX_TXOUT_SCRIPT_SIZE < 10000:
            filtered = [spender for spender in spenders if len(spender.script) <= BTX_MAX_TXOUT_SCRIPT_SIZE]
            skipped = len(spenders) - len(filtered)
            if skipped:
