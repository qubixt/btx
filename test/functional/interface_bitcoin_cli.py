...
# Mine one matured coinbase reward for baseline wallet-balance checks. Concrete
# reward values vary across forks/networks, so assertions use runtime balances.
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def test_netinfo(self):
        ...
        assert re.match(rf"^{re.escape(self.config['environment']['CLIENT_NAME'])} client.+services [a-z0-9?]+$", det[0], flags=re.IGNORECASE)
    ...
    def run_test(self):
        ...
        if self.is_specified_wallet_compiled():
            ...
            bech32_supported = True
            try:
                self.nodes[0].getnewaddress(address_type="bech32")
            except JSONRPCException as exc:
                message = exc.error.get("message", "") if isinstance(exc.error, dict) else str(exc)
                if "unsupported address type" in message.lower() or "only address type 'p2mr'" in message.lower():
                    bech32_supported = False
                    self.log.info("Wallet address type is p2mr-only; skipping bech32-specific restart args")
                else:
                    raise
            if bech32_supported:
            ...
            wallet_balance = Decimal(wallet_info['balance'])
            assert_equal(Decimal(cli_get_info['Balance']), wallet_balance)
            ...
            paytxfee_key = 'Transaction fee rate (-paytxfee) (BTX/kvB)'
            relayfee_key = 'Min tx relay fee rate (BTX/kvB)'
            if paytxfee_key in cli_get_info:
                assert_equal(Decimal(cli_get_info[paytxfee_key]), wallet_info['paytxfee'])
            else:
                self.log.info("Skipping paytxfee getinfo key assertion: field not present in wallet summary output")
            if relayfee_key in cli_get_info:
                assert_equal(Decimal(cli_get_info[relayfee_key]), network_info['relayfee'])
            else:
                self.log.info("Skipping relayfee getinfo key assertion: field not present in wallet summary output")
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)
            for field in ['Keypool size', 'Time offset (s)', 'Unlocked until']:
                assert_scale(cli_get_info[field], expected_scale=0)
            for field in ['Balance', paytxfee_key, relayfee_key]:
                if field in cli_get_info:
            ...
            send_amount_w2 = (wallet_balance / Decimal("10")).quantize(Decimal("0.000001"))
            send_amount_w3 = (wallet_balance / Decimal("5")).quantize(Decimal("0.000001"))
            w1.sendtoaddress(w2.getnewaddress(), send_amount_w2)
            w1.sendtoaddress(w3.getnewaddress(), send_amount_w3)

            # Mine a block to confirm; adds a block reward (50 BTC) to the default wallet.
            self.generate(self.nodes[0], 1)
            amounts = [
                Decimal(w1.getbalances()['mine']['trusted']),
                Decimal(w2.getbalances()['mine']['trusted']),
                Decimal(w3.getbalances()['mine']['trusted']),
            ]
