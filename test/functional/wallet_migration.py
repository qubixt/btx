...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def clear_default_wallet(self, backup_path):
        old_wallet = self.old_node.wallets_path / "wallet.dat"
        if old_wallet.exists():
            old_wallet.unlink()
        master_wallet = self.master_node.wallets_path / "wallet.dat"
        if master_wallet.exists():
            master_wallet.unlink()
    ...
    def test_default_wallet_failure(self):
        ...
        # Force migration failure by occupying the derived watch-only wallet path.
        ...
        original_shasum = sha256sum_file(watch_only_dir / "wallet.dat")

        mocked_time = int(time.time())
        self.master_node.setmocktime(mocked_time)
        assert_raises_rpc_error(-4, "Wallet file creation failed", self.master_node.migratewallet, wallet_name="")
        self.master_node.setmocktime(0)

        # Migration failure must not delete unrelated wallet files or directories.
        assert self.master_node.wallets_path.exists()
        assert watch_only_dir.exists()
        assert_equal(original_shasum, sha256sum_file(watch_only_dir / "wallet.dat"))

        # The source unnamed wallet should remain a legacy wallet after rollback.
        with open(self.master_node.wallets_path / "wallet.dat", "rb") as f:
            _, _, magic = struct.unpack("QII", f.read(16))
    ...
    def run_test(self):
        ...
        # NOTE: Test the actual records in the wallet for these tests too. The behavior may be correct, but the data written may not be what we actually want
        self.test_basic()
        self.test_multisig()
