...
class BlockchainTest(BitcoinTestFramework):
    ...
    def _test_getblockchaininfo(self):
        ...
        assert_equal(res['matmulvalidationmode'], 'consensus')
        ...
        tip_header = self.nodes[0].getblockheader(res['bestblockhash'])
        assert_equal(res['bits'], tip_header['bits'])
        assert_equal(res['target'], tip_header['target'])
    ...
    def _test_gettxoutsetinfo(self):
        ...
        def coinbase_value(height):
            block = node.getblock(node.getblockhash(height), 2)
            return Decimal(str(block["tx"][0]["vout"][0]["value"]))

        # BTX may include the genesis coinbase in the UTXO set, unlike Bitcoin.
        # Derive expectations from chain data instead of fixed subsidy constants.
        expected_transactions = HEIGHT
        expected_total_amount = Decimal("0")
        for height in range(1, HEIGHT + 1):
            expected_total_amount += coinbase_value(height)

        genesis = node.getblock(node.getblockhash(0), 2)
        genesis_txid = genesis["tx"][0]["txid"]
        genesis_coinbase_utxo = node.gettxout(genesis_txid, 0)
        if genesis_coinbase_utxo is not None:
            expected_transactions += 1
            expected_total_amount += Decimal(str(genesis_coinbase_utxo["value"]))

        assert_equal(res['total_amount'], expected_total_amount)
        assert_equal(res['transactions'], expected_transactions)
        assert_equal(res['height'], HEIGHT)
        assert_equal(res['txouts'], expected_transactions)
        assert_greater_than(res['bogosize'], 0)
        ...
        expected_genesis_transactions = 1 if genesis_coinbase_utxo is not None else 0
        expected_genesis_amount = Decimal(str(genesis_coinbase_utxo["value"])) if genesis_coinbase_utxo is not None else Decimal('0')
        assert_equal(res2['transactions'], expected_genesis_transactions)
        assert_equal(res2['total_amount'], expected_genesis_amount)
        assert_equal(res2['height'], 0)
        assert_equal(res2['txouts'], expected_genesis_transactions)
        if expected_genesis_transactions == 0:
            assert_equal(res2['bogosize'], 0)
        else:
            assert_greater_than(res2['bogosize'], 0)
    ...
    def _test_gettxout(self):
        ...
        block_verbose = node.getblock(best_block_hash, 2)
        ...
        assert_equal(txout['value'], Decimal(str(block_verbose['tx'][0]['vout'][0]['value'])))
        ...
        assert_equal(txout['scriptPubKey']['address'], decoded_script['address'])
    ...
    def _test_getblockheader(self):
        ...
        assert_equal(header['bits'], chaininfo['bits'])
        assert_equal(header['target'], chaininfo['target'])
    ...
    def _test_getdifficulty(self):
        ...
        tip_header = self.nodes[0].getblockheader(self.nodes[0].getbestblockhash())
        assert_equal(difficulty, tip_header["difficulty"])
    ...
    def _test_getnetworkhashps(self):
        ...
        # For non-genesis heights, estimated hashrate should be positive.
        hashes_per_second = self.nodes[0].getnetworkhashps()
        assert_greater_than(hashes_per_second, 0)

        # nblocks=-1 should be accepted and report a positive estimate.
        hashes_since_diff_change = self.nodes[0].getnetworkhashps(-1)
        assert_greater_than(hashes_since_diff_change, 0)
        # Passing the current tip height explicitly should yield the same result.
        assert_equal(
            hashes_since_diff_change,
            self.nodes[0].getnetworkhashps(-1, self.nodes[0].getblockcount()),
        )
    ...
    def _test_stopatheight(self):
        ...
        try:
            self.generatetodescriptor(self.nodes[0], 1, self.wallet.get_descriptor(), sync_fun=self.no_op)
    ...
    def _test_getblock(self):
        ...
        # BTX policy only relays WITNESS_V2_P2MR/OP_RETURN outputs, while
        ...
        def assert_hexblock_hashes(verbosity):
            ...
            assert block.startswith(header_hex)
