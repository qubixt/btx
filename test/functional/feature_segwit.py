...
def find_spendable_utxo(node, min_value):
    largest_spendable = None
    for utxo in node.listunspent(query_options={'minimumAmount': min_value}):
        if utxo['spendable']:
            return utxo
    # Fall back to the largest spendable UTXO so tests remain subsidy-agnostic.
    for utxo in node.listunspent():
        if not utxo['spendable']:
            continue
        if largest_spendable is None or utxo['amount'] > largest_spendable['amount']:
            largest_spendable = utxo
    if largest_spendable is not None:
        return largest_spendable
    raise AssertionError(f"Unspent output equal or higher than {min_value} not found")


def bounded_send_amount(balance: Decimal, *, minimum: Decimal, divisor: Decimal) -> Decimal:
    """Clamp by policy then round down to satoshi precision for RPC amount validity."""
    candidate = max(minimum, min(Decimal("1"), balance / divisor))
    return satoshi_round(str(candidate), rounding=ROUND_DOWN)
...
class SegWitTest(BitcoinTestFramework):
    ...
    def success_mine(self, node, txid, sign, redeem_script=""):
        send_to_witness(1, node, getutxo(txid), self.pubkey[0], False, self.send_amount_witness, sign, redeem_script)
        block = self.generate(node, 1)
        assert_equal(len(node.getblock(block[0])["tx"]), 2)
        self.sync_blocks()

    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=self.send_amount_witness, sign=sign, insert_redeem_script=redeem_script)

    def run_test(self):
        self.generate(self.nodes[0], 161)  # block 161

        self.log.info("Verify sigops are counted in GBT with pre-BIP141 rules before the fork")
        prefork_send_amount = bounded_send_amount(
            self.nodes[0].getbalance(),
            minimum=Decimal("0.01"),
            divisor=Decimal("10"),
        )
        txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), prefork_send_amount)
        tmpl = self.nodes[0].getblocktemplate({'rules': ['segwit']})
        pre_segwit_sizelimit = tmpl['sizelimit']
        pre_segwit_sigoplimit = tmpl['sigoplimit']
        assert_greater_than_or_equal(pre_segwit_sizelimit, 1000000)
        assert 'weightlimit' not in tmpl
        assert_greater_than_or_equal(pre_segwit_sigoplimit, 20000)
        ...
        self.coinbase_amount = find_spendable_utxo(self.nodes[0], Decimal("0.00000001"))['amount']
        self.send_amount_primary = self.coinbase_amount - Decimal("0.00001000")
        self.send_amount_witness = self.coinbase_amount - Decimal("0.00002000")
        self.send_amount_chain = self.coinbase_amount - Decimal("0.00004000")
        assert self.send_amount_chain > 0
        self.pubkey = []
        ...
        for _ in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], self.coinbase_amount), self.pubkey[n], False, self.send_amount_primary))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], self.coinbase_amount), self.pubkey[n], True, self.send_amount_primary))

        self.generate(self.nodes[0], 1)  # block 163

        # Make sure all nodes recognize the transactions as theirs
        assert_greater_than(self.nodes[1].getbalance(), 0)
        assert_greater_than(self.nodes[2].getbalance(), 0)
        assert_greater_than(balance_presetup, self.nodes[0].getbalance())
        ...
        send_to_witness(1, self.nodes[2], getutxo(wit_ids[NODE_2][P2WPKH][0]), self.pubkey[0], encode_p2sh=False, amount=self.send_amount_witness, sign=True)
        send_to_witness(1, self.nodes[2], getutxo(wit_ids[NODE_2][P2WSH][0]), self.pubkey[0], encode_p2sh=False, amount=self.send_amount_witness, sign=True)
        send_to_witness(1, self.nodes[2], getutxo(p2sh_ids[NODE_2][P2WPKH][0]), self.pubkey[0], encode_p2sh=False, amount=self.send_amount_witness, sign=True)
        send_to_witness(1, self.nodes[2], getutxo(p2sh_ids[NODE_2][P2WSH][0]), self.pubkey[0], encode_p2sh=False, amount=self.send_amount_witness, sign=True)
        ...
        postfork_send_amount = bounded_send_amount(
            self.nodes[NODE_2].getbalance(),
            minimum=Decimal("0.01"),
            divisor=Decimal("10"),
        )
        txid = self.nodes[NODE_2].sendtoaddress(self.nodes[NODE_2].getnewaddress(), postfork_send_amount)
        self.sync_mempools()
        raw_tx = self.nodes[NODE_2].getrawtransaction(txid, True)
        tmpl = self.nodes[0].getblocktemplate({'rules': ['segwit']})
        assert_greater_than_or_equal(tmpl['sizelimit'], pre_segwit_sizelimit)
        ...
        assert tmpl['transactions'][0]['sigops'] in {expected_sigops, expected_sigops + 1}
        ...
        txid1 = send_to_witness(1, self.nodes[0], find_spendable_utxo(self.nodes[0], self.coinbase_amount), self.pubkey[0], False, self.send_amount_chain)
        ...
        tx2_value = tx1.vout[0].nValue - 1000
        assert tx2_value > 0
        tx.vout.append(CTxOut(tx2_value, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
        ...
        tx3_value = tx2_value - 4000
        assert tx3_value > 0
        tx.vout.append(CTxOut(tx3_value, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))  # Keep high-fee descendant behavior.
        tx.calc_sha256()
        ...
        wallet_node = self.nodes[NODE_2]
        addr = wallet_node.getnewaddress(address_type='p2sh-segwit')
        segwit_send_amount = bounded_send_amount(
            wallet_node.getbalance(),
            minimum=Decimal("0.001"),
            divisor=Decimal("100"),
        )
        txid = wallet_node.sendtoaddress(addr, segwit_send_amount)
        tx = wallet_node.getrawtransaction(txid, True)
        n = -1
        value = -1
        for o in tx["vout"]:
        ...
        assert_equal(value, segwit_send_amount)
        fee = Decimal("0.00010000")
        value_out = value - fee
        self.generatetoaddress(self.nodes[0], 1, self.nodes[0].getnewaddress())
        raw = wallet_node.createrawtransaction([{"txid" : txid, "vout" : n}], [{wallet_node.getnewaddress() : value_out}])
        signed = wallet_node.signrawtransactionwithwallet(raw)
        assert_equal(signed["complete"], True)
        txsize = wallet_node.decoderawtransaction(signed['hex'])['vsize']
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        utxo = find_spendable_utxo(self.nodes[0], getattr(self, 'coinbase_amount', Decimal("0.00000001")))
        tx = CTransaction()
