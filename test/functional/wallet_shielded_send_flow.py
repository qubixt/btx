...
class WalletShieldedSendFlowTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        # 2-input shielded send/unshield proofs.
    ...
    def run_test(self):
        ...
        def compact_to_live_direct_spend_envelope(shielded_wallet):
            while shielded_wallet.z_getbalance()["note_count"] > 2:
                merge = shielded_wallet.z_mergenotes(8)
                assert merge["txid"] in node.getrawmempool()
                assert_equal(merge["merged_notes"], 2)
        ...
        self.log.info("Surface a clear error once a direct shielded send would need more than two notes")
        overflow_addr = receiver.z_getnewaddress()
        for amount in [Decimal("0.12"), Decimal("0.13"), Decimal("0.14")]:
            funder.z_sendmany([{"address": overflow_addr, "amount": amount}])
            self.generatetoaddress(node, 1, mine_addr, sync_fun=self.no_op)
        overflow_dest = wallet.z_getnewaddress()
        assert_raises_rpc_error(
            -4,
            "direct shielded send currently supports at most 2 shielded inputs; merge notes first",
            receiver.z_sendmany,
            [{"address": overflow_dest, "amount": Decimal("0.30")}],
        )

        self.log.info("Pairwise note merges recover the receiver wallet into the live direct-send envelope")
        ...
        assert recovered_send["spends"] <= 2
        ...
        for iteration in range(2):
            ...
            assert stress_unshield["spends"] <= 2
