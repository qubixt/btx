...
def get_coinbase_reward(node, height=1):
    block = node.getblock(node.getblockhash(height), 2)
    return Decimal(str(block["tx"][0]["vout"][0]["value"]))
...
def assert_transparent_balance_views(wallet, expected_trusted, *, expected_untrusted=Decimal("0"), expected_immature=Decimal("0"), expected_z_total=None):
    ...
    assert_equal(wallet.getbalance(), expected_trusted)

    trusted_utxo_total = sum((Decimal(str(utxo["amount"])) for utxo in wallet.listunspent(minconf=0, include_unsafe=False) if utxo.get("spendable", False)), Decimal("0"))
    assert_equal(trusted_utxo_total, expected_trusted)

    spendable_utxo_total = sum((Decimal(str(utxo["amount"])) for utxo in wallet.listunspent(0) if utxo.get("spendable", False)), Decimal("0"))
    assert_equal(spendable_utxo_total, expected_trusted + expected_untrusted)

    if expected_z_total is None:
        expected_z_total = expected_trusted
    totals = wallet.z_gettotalbalance()
...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        coinbase_reward = get_coinbase_reward(self.nodes[0])
        send_amount_0 = (coinbase_reward * Decimal("0.8")).quantize(Decimal("0.00000001"))
        send_amount_1 = (coinbase_reward * Decimal("1.2")).quantize(Decimal("0.00000001"))
        ...
        assert_transparent_balance_views(self.nodes[0], coinbase_reward)
        assert_transparent_balance_views(self.nodes[1], coinbase_reward)
        assert_equal(self.nodes[0].getbalance("*"), coinbase_reward)
        assert_equal(self.nodes[0].getbalance("*", 1), coinbase_reward)
        ...
        if not self.options.descriptors:
            assert_equal(self.nodes[0].getbalance("*", 1, True), coinbase_reward * 2)
        else:
            assert_equal(self.nodes[0].getbalance("*", 1, True), coinbase_reward)
        ...
        # Send 80% of one subsidy from 0 to 1 and 120% of one subsidy back from 1 to 0.
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), send_amount_0, [Decimal('0.01')])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), send_amount_1, [Decimal('0.01'), Decimal('0.02')])
        ...
        # Before `test_balance()`, we have had two nodes with one confirmed subsidy
        # each and then we:
        #
        # 1) Sent send_amount_0 from node A to node B with fee 0.01
        # 2) Sent send_amount_1 from node B to node A with fee 0.01
        #
        ...
        #
        # Then our node would report a confirmed balance of send_amount_0 +
        # coinbase_reward - 10, which is more than would be available if transaction 1 were
        # replaced.


        def test_balances(*, fee_node_1=0):
            change_node0 = coinbase_reward - send_amount_0 - Decimal("0.01")
            change_node1 = coinbase_reward + send_amount_0 - send_amount_1 - fee_node_1
            # getbalances
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           change_node0,  # change from node 0's send
                                                 'untrusted_pending': send_amount_1},
                                   'watchonly': {'immature':          coinbase_reward * COINBASE_MATURITY,
                                                 'trusted':           coinbase_reward,
            ...
            assert_transparent_balance_views(self.nodes[1], Decimal("0"), expected_untrusted=change_node1, expected_z_total=Decimal("0"))
            # getbalance without any arguments includes unconfirmed transactions, but not untrusted transactions
            assert_equal(self.nodes[0].getbalance(), change_node0)  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # getbalance with '*' and minconf=0 includes unconfirmed transactions, AND untrusted transactions
            assert_equal(self.nodes[0].getbalance('*', 0), change_node0 + send_amount_1)
            assert_equal(self.nodes[1].getbalance('*', 0), change_node1)
            # getbalance with '*' and minconf=1 includes only confirmed and sent transactions
            assert_equal(self.nodes[0].getbalance('*', 1), change_node0)
            assert_equal(self.nodes[1].getbalance('*', 1), coinbase_reward - send_amount_1 - fee_node_1)
            # getunconfirmedbalance
            assert_equal(self.nodes[0].getunconfirmedbalance(), send_amount_1)  # output of node 1's spend
            assert_equal(self.nodes[1].getunconfirmedbalance(), change_node1)  # Doesn't include output of node 0's send since it was spent
            # getwalletinfo.unconfirmed_balance
            assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], send_amount_1)
            assert_equal(self.nodes[1].getwalletinfo()["unconfirmed_balance"], change_node1)
        ...
        balance_node0 = coinbase_reward - send_amount_0 - Decimal("0.01") + send_amount_1  # node 1's send plus change from node 0's send
        balance_node1 = coinbase_reward + send_amount_0 - send_amount_1 - Decimal("0.02")  # change from node 0's send
        assert_transparent_balance_views(self.nodes[0], balance_node0)
        assert_transparent_balance_views(self.nodes[1], balance_node1)
        ...
        descendant_amount = (self.nodes[0].getbalance() - Decimal("1")).quantize(Decimal("0.00000001"))
        for _ in range(3):
            # Set amount high enough such that all coins are spent by each tx
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), descendant_amount)
        ...
        tx_replace = tx_orig.replace(
            int(descendant_amount * Decimal(10**8)).to_bytes(8, "little", signed=True).hex(),
            int((descendant_amount - Decimal("1")) * Decimal(10**8)).to_bytes(8, "little", signed=True).hex(),
            1,
        )
