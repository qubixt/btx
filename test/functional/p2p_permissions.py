...
class P2PPermissionsTests(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.checkpermission(
            ...
            ["addr", "forcerelay", "relay", "mempool", "download"])
    ...
    def check_tx_relay(self):
        ...
        self.restart_node(1, extra_args=self.extra_args[1] + ["-whitelist=forcerelay@127.0.0.1"])
        ...
        # Make the transaction consensus-invalid irrespective of policy mode.
        tx.vout[0].nValue = -1
        txid = tx.rehash()
        ...
        p2p_rebroadcast_wallet.send_txs_and_test(
            ...
            self.nodes[1],
            success=False,
            reject_reason='{} (wtxid={}) from peer=0 was not accepted: bad-txns-vout-negative'.format(txid, tx.getwtxid())
        )
    ...
    def checkpermission(self, args, expectedPermissions):
        self.restart_node(1, self.extra_args[1] + ['-peerbloomfilters=0'] + args)
        self.connect_nodes(0, 1)
        peerinfo = self.nodes[1].getpeerinfo()[0]
        assert_equal(sorted(expectedPermissions), sorted(peerinfo['permissions']))
