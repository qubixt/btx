...
class FeatureRemovePrunedFilesOnStartupTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        files_before_reindex = ls_files()
        # BTX pruning can leave more than two blk/rev pairs before reindex.
        # The invariant we care about is that pruned data uses matching blk/rev files.
        assert len(files_before_reindex) >= 4
        blk_files = [f for f in files_before_reindex if f.startswith("blk")]
        rev_files = [f for f in files_before_reindex if f.startswith("rev")]
        assert_equal(len(blk_files), len(rev_files))
