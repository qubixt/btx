...
class WalletMultisigDescriptorPSBTTest(BitcoinTestFramework):
    ...
    def _get_xpub(self, wallet, internal):
        """Extract the wallet's xpub from descriptor metadata for the requested branch.

        BTX descriptor wallets may be PQ-only (`mr(pqhd(...),pk_slh(...))`) and not expose
        xpub-based descriptors. In that mode this upstream legacy multisig flow is not applicable.
        """
        xpub_pattern = re.compile(r"([tx]pub[1-9A-HJ-NP-Za-km-z]+(?:/[0-9h'/*]+)?)")
        for descriptor in wallet.listdescriptors()["descriptors"]:
            if descriptor["internal"] != internal:
                continue
            # Strip checksum suffix before extraction.
            desc = descriptor["desc"].split("#", 1)[0]
            match = xpub_pattern.search(desc)
            if match:
                return match.group(1)

        branch = "internal" if internal else "external"
        raise SkipTest(
            f"{wallet.getwalletinfo()['walletname']} does not expose xpub descriptors for {branch} branch "
            "(BTX PQ descriptor policy active)"
        )
