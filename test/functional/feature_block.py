...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.kawpow_strict = any("matmulstrict" in arg for arg in self.extra_args[0])
        ...
        b15_sigops = get_legacy_sigopcount_block(b15)
        b15_extra = self.create_sigops_tx(b15.vtx[1], (MAX_BLOCK_SIGOPS - 1) - b15_sigops)
        b15 = self.update_block(15, [b15_extra])
        self.save_spendable_output()
        self.send_blocks([b15], True)

        self.log.info("Reject a block with too many checksigs")
        b16 = self.next_block(16, spend=out[6])
        b16_sigops = get_legacy_sigopcount_block(b16)
        b16_extra = self.create_sigops_tx(b16.vtx[1], (MAX_BLOCK_SIGOPS + 1) - b16_sigops)
        b16 = self.update_block(16, [b16_extra])
        ...
        tx = self.create_weight_padding_tx(b23.vtx[1], MAX_BLOCK_WEIGHT - b23.get_weight())
        ...
        b31_sigops = get_legacy_sigopcount_block(b31)
        b31_extra = self.create_sigops_tx(
            b31.vtx[1],
            MAX_BLOCK_SIGOPS - b31_sigops,
            opcode=OP_CHECKMULTISIG,
            sigops_per_opcode=20,
        )
        b31 = self.update_block(31, [b31_extra])
        assert_equal(get_legacy_sigopcount_block(b31), MAX_BLOCK_SIGOPS)
        self.send_blocks([b31], True)
        self.save_spendable_output()

        # Go over the limit by one sigop.
        self.log.info("Reject a block with too many OP_CHECKMULTISIG sigops")
        b32 = self.next_block(32, spend=out[9])
        b32_sigops = get_legacy_sigopcount_block(b32)
        b32_extra = self.create_sigops_tx(
            b32.vtx[1],
            (MAX_BLOCK_SIGOPS + 1) - b32_sigops,
            opcode=OP_CHECKMULTISIG,
            sigops_per_opcode=20,
        )
        b32 = self.update_block(32, [b32_extra])
        ...
        b33_sigops = get_legacy_sigopcount_block(b33)
        b33_extra = self.create_sigops_tx(
            b33.vtx[1],
            MAX_BLOCK_SIGOPS - b33_sigops,
            opcode=OP_CHECKMULTISIGVERIFY,
            sigops_per_opcode=20,
        )
        b33 = self.update_block(33, [b33_extra])
        assert_equal(get_legacy_sigopcount_block(b33), MAX_BLOCK_SIGOPS)
        self.send_blocks([b33], True)
        self.save_spendable_output()

        self.log.info("Reject a block with too many OP_CHECKMULTISIGVERIFY sigops")
        b34 = self.next_block(34, spend=out[10])
        b34_sigops = get_legacy_sigopcount_block(b34)
        b34_extra = self.create_sigops_tx(
            b34.vtx[1],
            (MAX_BLOCK_SIGOPS + 1) - b34_sigops,
            opcode=OP_CHECKMULTISIGVERIFY,
            sigops_per_opcode=20,
        )
        b34 = self.update_block(34, [b34_extra])
        assert_equal(get_legacy_sigopcount_block(b34), MAX_BLOCK_SIGOPS + 1)
        ...
        b35_sigops = get_legacy_sigopcount_block(b35)
        b35_extra = self.create_sigops_tx(
            b35.vtx[1],
            MAX_BLOCK_SIGOPS - b35_sigops,
            opcode=OP_CHECKSIGVERIFY,
            sigops_per_opcode=1,
        )
        b35 = self.update_block(35, [b35_extra])
        assert_equal(get_legacy_sigopcount_block(b35), MAX_BLOCK_SIGOPS)
        self.send_blocks([b35], True)
        self.save_spendable_output()

        self.log.info("Reject a block with too many OP_CHECKSIGVERIFY sigops")
        b36 = self.next_block(36, spend=out[11])
        b36_sigops = get_legacy_sigopcount_block(b36)
        b36_extra = self.create_sigops_tx(
            b36.vtx[1],
            (MAX_BLOCK_SIGOPS + 1) - b36_sigops,
            opcode=OP_CHECKSIGVERIFY,
            sigops_per_opcode=1,
        )
        b36 = self.update_block(36, [b36_extra])
        assert_equal(get_legacy_sigopcount_block(b36), MAX_BLOCK_SIGOPS + 1)
        ...
        tx = self.create_sigops_tx(sigops_source_tx, b40_sigops_to_fill)
        new_txs.append(tx)
        ...
        tx = self.create_sigops_tx(sigops_source_tx, b41_sigops_to_fill)
        ...
        b44 = create_block(self.tip.sha256, coinbase, self.tip.nTime + 1)
        ...
        b45 = create_block(self.tip.sha256, coinbase, self.tip.nTime + 1)
        b45.vtx = [non_coinbase]
        ...
        self.move_tip(44)
        b46 = create_block(b44.sha256, create_coinbase(self.block_heights[b44.sha256] + 1, self.coinbase_pubkey), b44.nTime + 1)
        b46.vtx = []
        ...
        # Mutate the solved nonce to make proof-of-work invalid deterministically.
        b47.nNonce64 = (b47.nNonce64 + 1) & 0xFFFFFFFFFFFFFFFF
        b47.rehash()
        if self.kawpow_strict:
            self.send_blocks([b47], False, force_send=True, reject_reason='high-hash', reconnect=True)
        else:
            # Default BTX regtest mode skips KAWPOW validation for functional-test speed.
            self.send_blocks([b47], True, force_send=True)
            # Keep subsequent branch-length assumptions stable by rewinding the
            # accepted invalid-work block in non-strict mode.
            node.invalidateblock(b47.hash)
            assert_equal(node.getbestblockhash(), b44.hash)
        ...
        self.send_blocks([b53], False, force_send=True)
        self.save_spendable_output()

        self.log.info("Reject a block with timestamp before MedianTimePast")
        mtp_b53 = self.median_time_past(b53)
        ...
        # b73 - tx with excessive sigops where one sigop is placed after a push-data element.
        #       BTX constrains txout scripts to 34 bytes, so this case is composed from many
        #       small outputs plus a tail script that exercises "sigop-after-pushdata" counting.
        self.log.info("Reject a block containing too many sigops after a large script element")
        self.move_tip(72)
        self.next_block(73, spend=out[22])
        b73_base_sigops = get_legacy_sigopcount_block(self.blocks[73])
        tx = self.create_sigops_tx(self.blocks[73].vtx[1], MAX_BLOCK_SIGOPS - b73_base_sigops)
        tx.vout.append(CTxOut(0, CScript(bytes([0x4c, 0x01, 0x00, OP_CHECKSIG]))))  # OP_PUSHDATA1(1) <00> OP_CHECKSIG
        tx.rehash()
        ...
        #
        #       The invalid script element is that push_data indicates there will be a
        #       large amount of data (0xffffffff bytes), but the script ends right
        ...
        self.move_tip(72)
        self.next_block(74, spend=out[22])
        invalid_push_tail = CScript(bytes([OP_CHECKSIG, 0x4e, 0xff, 0xff, 0xff, 0xff]))
        b74_base_sigops = get_legacy_sigopcount_block(self.blocks[74])
        tx = self.create_sigops_tx(self.blocks[74].vtx[1], MAX_BLOCK_SIGOPS - b74_base_sigops)
        tx.vout.append(CTxOut(0, invalid_push_tail))
        tx.rehash()
        b74 = self.update_block(74, [tx])
        self.send_blocks([b74], success=False, reject_reason='bad-blk-sigops', reconnect=True)

        self.move_tip(72)
        self.next_block(75, spend=out[22])
        b75_base_sigops = get_legacy_sigopcount_block(self.blocks[75])
        tx = self.create_sigops_tx(self.blocks[75].vtx[1], MAX_BLOCK_SIGOPS - b75_base_sigops - 1)
        tx.vout.append(CTxOut(0, invalid_push_tail))
        tx.rehash()
        ...
        self.move_tip(75)
        self.next_block(76, spend=out[23])
        b76_base_sigops = get_legacy_sigopcount_block(self.blocks[76])
        tx = self.create_sigops_tx(self.blocks[76].vtx[1], MAX_BLOCK_SIGOPS - b76_base_sigops - 1)
        tx.vout.append(CTxOut(0, CScript(bytes([0x4c, 0x04, OP_CHECKSIG, OP_CHECKSIG, OP_CHECKSIG, OP_CHECKSIG]))))
        tx.rehash()
        b76 = self.update_block(76, [tx])
        assert_equal(get_legacy_sigopcount_block(b76), MAX_BLOCK_SIGOPS - 1)
        ...
        self.move_tip(88)
        reorg_size_raw = os.getenv("BTX_FEATURE_BLOCK_LARGE_REORG_SIZE", "32")
        try:
            LARGE_REORG_SIZE = int(reorg_size_raw)
        except ValueError as exc:
            raise AssertionError("BTX_FEATURE_BLOCK_LARGE_REORG_SIZE must be an integer") from exc
        # Keep this deep enough to exercise long-reorg logic while avoiding
        # timeout-prone test tails on BTX/KAWPOW CI lanes.
        assert_greater_than(LARGE_REORG_SIZE, 15)
        # Keep timestamps bounded during large synthetic reorgs so headers don't
        # get rejected as too-far-in-the-future under KAWPOW target spacing.
        # Also avoid ultra-low deltas that make DGW over-tighten difficulty and
        # turn block.solve() into a CPU-heavy tail in Python.
        MAX_REORG_SPAN_SECONDS = (2 * 60 * 60) - 120
        LARGE_REORG_TIME_DELTA = max(2, min(REGTEST_KAWPOW_TARGET_SPACING, MAX_REORG_SPAN_SECONDS // (LARGE_REORG_SIZE + 2)))
        self.log.info(
            f"Test a large re-org ({LARGE_REORG_SIZE} blocks, dt={LARGE_REORG_TIME_DELTA}s)"
        )
        LARGE_REORG_TIMEOUT = max(300, LARGE_REORG_SIZE * 2)
        blocks = []
        spend = out[32]
        for i in range(89, LARGE_REORG_SIZE + 89):
            b = self.next_block(i, spend, block_time_delta=LARGE_REORG_TIME_DELTA)
            blocks.append(b)
            self.save_spendable_output()
            spend = self.get_spendable_output()

        self.send_blocks(blocks, True, timeout=LARGE_REORG_TIMEOUT)
        chain1_tip = i
        ...
        blocks2 = []
        for i in range(89, LARGE_REORG_SIZE + 89):
            blocks2.append(self.next_block("alt" + str(i), block_time_delta=LARGE_REORG_TIME_DELTA))
        self.send_blocks(blocks2, False, force_send=False)

        # extend alt chain to trigger re-org
        block = self.next_block("alt" + str(chain1_tip + 1), block_time_delta=LARGE_REORG_TIME_DELTA)
        self.send_blocks([block], True, timeout=LARGE_REORG_TIMEOUT)

        # ... and re-org back to the first chain
        self.move_tip(chain1_tip)
        block = self.next_block(chain1_tip + 1, block_time_delta=LARGE_REORG_TIME_DELTA)
        self.send_blocks([block], False, force_send=True)
        block = self.next_block(chain1_tip + 2, block_time_delta=LARGE_REORG_TIME_DELTA)
        self.send_blocks([block], True, timeout=LARGE_REORG_TIMEOUT)
    ...
    def get_block_by_hash(self, block_hash):
        for block in self.blocks.values():
            if block.sha256 == block_hash:
                return block
        return None

    def median_time_past(self, block):
        times = []
        cursor = block
        for _ in range(11):
            times.append(cursor.nTime)
            prev = self.get_block_by_hash(cursor.hashPrevBlock)
            if prev is None:
                break
            cursor = prev
        times.sort()
    ...
    def create_weight_padding_tx(self, spend_tx, target_weight):
        ...
        # with script_len constrained to [1, 34] by BTX reduced-data rules.
        min_outputs = (target_bytes + 43 - 1) // 43
        max_outputs = target_bytes // 10

        for output_count in range(min_outputs, max_outputs + 1):
            base_size = 50 + self._compact_size_len(output_count)
            remaining = target_bytes - base_size
            min_payload = 10 * output_count
            max_payload = 43 * output_count
        ...
        raise AssertionError(f"unable to build tx with target weight {target_weight}")

    def create_sigops_tx(self, spend_tx, target_sigops, *, opcode=OP_CHECKSIG, sigops_per_opcode=1):
        """Create tx whose outputs contribute exactly target_sigops legacy sigops."""
        assert target_sigops > 0
        assert sigops_per_opcode > 0
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(spend_tx.sha256, 0)))
        tx.vout = []

        if sigops_per_opcode == 1:
            full_outputs, remainder = divmod(target_sigops, 34)
            tx.vout.extend(CTxOut(0, CScript([opcode] * 34)) for _ in range(full_outputs))
            if remainder:
                tx.vout.append(CTxOut(0, CScript([opcode] * remainder)))
        else:
            full_outputs, remainder = divmod(target_sigops, sigops_per_opcode)
            tx.vout.extend(CTxOut(0, CScript([opcode])) for _ in range(full_outputs))
            if remainder:
                checksig_full, checksig_remainder = divmod(remainder, 34)
                tx.vout.extend(CTxOut(0, CScript([OP_CHECKSIG] * 34)) for _ in range(checksig_full))
                if checksig_remainder:
                    tx.vout.append(CTxOut(0, CScript([OP_CHECKSIG] * checksig_remainder)))

        # Sign after outputs are finalized so signature checks stay valid.
        self.sign_tx(tx, spend_tx)
        tx.rehash()
        assert_equal(get_legacy_sigopcount_tx(tx), target_sigops)
        return tx
    ...
    def sign_tx(self, tx, spend_tx):
        ...
        if (scriptPubKey[0] == OP_TRUE):  # an anyone-can-spend
            ...
            return
        if len(scriptPubKey) == 25 and scriptPubKey[0] == OP_DUP and scriptPubKey[1] == OP_HASH160 and scriptPubKey[2] == 20 and scriptPubKey[23] == OP_EQUALVERIFY and scriptPubKey[24] == OP_CHECKSIG:
            # Preserve any pre-populated scriptSig bytes (some invalid-tx templates
            # rely on those) and append the pubkey needed for P2PKH checks.
            tx.vin[0].scriptSig = bytes(tx.vin[0].scriptSig) + bytes(CScript([self.coinbase_pubkey]))
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, *, script=None, version=4, block_time_delta=1):
        if script is None:
            script = CScript([OP_TRUE])
        assert block_time_delta > 0
        if self.tip is None:
            base_block_hash = self.genesis_hash
            block_time = int(time.time()) + 1
        else:
            base_block_hash = self.tip.sha256
            block_time = self.tip.nTime + block_time_delta
