...
class ValidateAddressMainTest(BitcoinTestFramework):
    ...
    def test_validateaddress(self):
        # BTX mainnet uses a custom Bech32 HRP ("btx"), so Bitcoin mainnet
        # vectors with "bc1..." are treated as unsupported HRP here.
        mainnet_bech32_hrp = self.nodes[0].decodescript(VALID_DATA[0][1])["address"].split("1", 1)[0].lower()
        custom_mainnet_hrp = mainnet_bech32_hrp != "bc"

        for (addr, error, locs) in INVALID_DATA:
            if custom_mainnet_hrp and addr.lower().startswith("bc1"):
                res = self.nodes[0].validateaddress(addr)
                assert_equal(res["isvalid"], False)
                assert "error" in res
            else:
        ...
        if custom_mainnet_hrp:
            ...
            # Ensure checksum corruption and mixed-case violations are still
            # detected with the BTX-specific mainnet HRP.
            bad_checksum = sample_valid[:-1] + ("q" if sample_valid[-1] != "q" else "p")
            bad_checksum_res = self.nodes[0].validateaddress(bad_checksum)
            assert_equal(bad_checksum_res["isvalid"], False)
            assert bad_checksum_res["error"] in {"Invalid checksum", "Invalid Bech32 checksum"}

            mixed_case = sample_valid[:4].upper() + sample_valid[4:]
            mixed_case_res = self.nodes[0].validateaddress(mixed_case)
            assert_equal(mixed_case_res["isvalid"], False)
            assert_equal(mixed_case_res["error"], "Invalid character or mixed case")
