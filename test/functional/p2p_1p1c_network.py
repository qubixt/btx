...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def raise_network_minfee(self):
        ...
        # Keep the highest network floor for package construction that is
        # expected to propagate without package-only acceptance semantics.
        self.network_mempool_minfee = max(node.getmempoolinfo()['mempoolminfee'] for node in self.nodes)
    ...
    def create_package_2p1c(self, wallet):
        # Use dynamic above-floor rates so both parents relay individually even
        # when mempoolminfee is raised significantly by fill_mempool().
        parent_floor = max(self.network_mempool_minfee * Decimal("1.25"), Decimal(DEFAULT_MIN_RELAY_TX_FEE) / COIN * 20)
        parent1 = wallet.create_self_transfer(fee_rate=parent_floor, confirmed_only=True)
        parent2 = wallet.create_self_transfer(fee_rate=parent_floor * Decimal("1.5"), confirmed_only=True)
    ...
    def run_test(self):
        ...
        self.wallet_nonsegwit = MiniWallet(self.nodes[2], mode=MiniWalletMode.RAW_P2PKH)
