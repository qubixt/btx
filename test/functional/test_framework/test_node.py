...
# BTX regtest DGW uses a 180-block window; keep one extra parent for continuity.
BTX_POW_HISTORY_LOOKBACK = 181
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        ...
        # Keep tests pinned to their generated bitcoin.conf even though the
        # product default config filename is btx.conf.
    ...
    @staticmethod
    def _extract_tip_hash_from_generate_result(result):
        if isinstance(result, str):
            return result
        if isinstance(result, list):
            return result[-1] if result and isinstance(result[-1], str) else None
        if isinstance(result, dict):
            block_hash = result.get("hash")
            if isinstance(block_hash, str):
                return block_hash
            hashes = result.get("hashes")
            if isinstance(hashes, list) and hashes and isinstance(hashes[-1], str):
                return hashes[-1]
        return None

    def _seed_pow_history_from_tip(self, tip_hash):
        """Cache recent chain headers so handcrafted block builders derive BTX DGW bits correctly."""
        if not isinstance(tip_hash, str) or not tip_hash:
            return

        cursor = tip_hash
        for _ in range(BTX_POW_HISTORY_LOOKBACK):
            if not cursor:
                break
            try:
                cursor_int = int(cursor, 16)
            except ValueError:
                break

            # If this block is known, ancestors should already be cached as well.
            if lookup_solved_block_index(cursor_int) is not None:
                break

            try:
                header = self.getblockheader(cursor)
                prev_hash_hex = header.get("previousblockhash")
                seed_solved_block_index(
                    block_hash=cursor_int,
                    prev_hash=int(prev_hash_hex, 16) if prev_hash_hex else 0,
                    height=int(header["height"]),
                    n_time=int(header["time"]),
                    n_bits=int(header["bits"], 16),
                )
            except (JSONRPCException, KeyError, TypeError, ValueError):
                break

            cursor = prev_hash_hex

    def _cache_generate_result_pow_history(self, result):
        tip_hash = self._extract_tip_hash_from_generate_result(result)
        if tip_hash is not None:
            self._seed_pow_history_from_tip(tip_hash)
    ...
    def generateblock(self, *args, called_by_framework, **kwargs):
        ...
        result = self.__getattr__('generateblock')(*args, **kwargs)
        self._cache_generate_result_pow_history(result)
        return result

    def generatetoaddress(self, *args, called_by_framework, **kwargs):
        assert called_by_framework, "Direct call of this mining RPC is discouraged. Please use one of the self.generate* methods on the test framework, which sync the nodes to avoid intermittent test issues. You may use sync_fun=self.no_op to disable the sync explicitly."
        result = self.__getattr__('generatetoaddress')(*args, **kwargs)
        self._cache_generate_result_pow_history(result)
        return result

    def generatetodescriptor(self, *args, called_by_framework, **kwargs):
        assert called_by_framework, "Direct call of this mining RPC is discouraged. Please use one of the self.generate* methods on the test framework, which sync the nodes to avoid intermittent test issues. You may use sync_fun=self.no_op to disable the sync explicitly."
        result = self.__getattr__('generatetodescriptor')(*args, **kwargs)
        self._cache_generate_result_pow_history(result)
        return result
...
class TestNodeCLI():
    ...
    def __init__(self, binary, datadir, rpc_timeout):
        ...
        self.bitcoinconf = self.datadir / "bitcoin.conf"
