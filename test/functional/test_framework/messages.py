...
DEFAULT_REGTEST_MATMUL_DIM = 64

MAGIC_BYTES = {
    "mainnet": b"\xb7\x54\x58\x01",   # mainnet (BTX)
    "testnet3": b"\xb7\x54\x58\x02",  # testnet3 (BTX)
    "testnet4": b"\x1c\x16\x3f\x28",  # testnet4 (BTX)
    "regtest": b"\xfa\xbf\xb5\xda",   # regtest
    "signet": b"\x0a\x03\xcf\x40",    # signet
    "shieldedv2dev": b"\xe2\xb7\xdaz", # shieldedv2dev
}
...
def uint256_to_compact(v):
    if v == 0:
        return 0
    if v < 0:
        raise ValueError("uint256_to_compact only supports non-negative integers")

    nbytes = (v.bit_length() + 7) // 8
    if nbytes <= 3:
        compact = v << (8 * (3 - nbytes))
    else:
...
# Tracks solved block headers by hash so blocktools can derive BTX-specific
# next-work requirements when tests build chains with handcrafted blocks.
_SOLVED_BLOCK_INDEX = {}


def seed_solved_block_index(*, block_hash, height, n_time, n_bits, prev_hash=0):
    _SOLVED_BLOCK_INDEX[block_hash] = {
        "prev_hash": prev_hash,
        "height": int(height),
        "n_time": int(n_time),
        "n_bits": int(n_bits),
...
def lookup_solved_block_index(block_hash):
    return _SOLVED_BLOCK_INDEX.get(block_hash)


def register_solved_block(*, block_hash, prev_hash, n_time, n_bits):
    prev = lookup_solved_block_index(prev_hash)
    if prev is None or prev.get("height") is None:
        height = None
    else:
...
def deterministic_matmul_seed(prev_hash, height, which):
    payload = ser_uint256(prev_hash) + int(height).to_bytes(4, "little") + bytes([which & 0xFF])
    return uint256_from_str(sha256(payload))


def _default_matmul_dim():
    try:
        dim = int(os.getenv("BTX_PY_MATMUL_DIM", str(DEFAULT_REGTEST_MATMUL_DIM)))
    except ValueError:
        dim = DEFAULT_REGTEST_MATMUL_DIM
    if dim <= 0 or dim > 0xFFFF:
        return DEFAULT_REGTEST_MATMUL_DIM
    return dim


def _next_header_height(prev_hash):
    prev = lookup_solved_block_index(prev_hash)
    if prev is None or prev.get("height") is None:
        return 0
...
class CBlockHeader:
    __slots__ = ("hash", "hashMerkleRoot", "hashPrevBlock", "mixHash", "nBits", "nNonce",
                 "nNonce64", "nTime", "nVersion", "sha256", "matmul_digest", "matmul_dim",
                 "seed_a", "seed_b")
    ...
    def __init__(self, header=None):
        if header is None:
            ...
            self.mixHash = getattr(header, "mixHash", 0)
            self.matmul_digest = getattr(header, "matmul_digest", self.mixHash)
            self.matmul_dim = getattr(header, "matmul_dim", 0)
            self.seed_a = getattr(header, "seed_a", 0)
            self.seed_b = getattr(header, "seed_b", 0)
    ...
    def _effective_nonce64(self):
        # Keep legacy tests that still set nNonce working by mirroring it
        # into the serialized 64-bit nonce field when nNonce64 is unset.
        return self.nNonce64 if self.nNonce64 != 0 else self.nNonce

    def _effective_matmul_digest(self):
        # Legacy KAWPOW-style tests may set mixHash; map it when explicit
        # MatMul digest is not provided.
        return self.matmul_digest if self.matmul_digest != 0 else self.mixHash
    ...
    def deserialize(self, f):
        ...
        self.nNonce64 = int.from_bytes(f.read(8), "little")
        self.matmul_digest = deser_uint256(f)
        self.matmul_dim = int.from_bytes(f.read(2), "little")
        self.seed_a = deser_uint256(f)
        self.seed_b = deser_uint256(f)
        # Mirror fields kept for transitional test compatibility.
    ...
    def serialize(self):
        ...
        r += self._effective_nonce64().to_bytes(8, "little")
        r += ser_uint256(self._effective_matmul_digest())
        r += self.matmul_dim.to_bytes(2, "little")
        r += ser_uint256(self.seed_a)
        r += ser_uint256(self.seed_b)
        return r
    ...
    def __repr__(self):
        return "CBlockHeader(nVersion=%i hashPrevBlock=%064x hashMerkleRoot=%064x nTime=%s nBits=%08x nNonce=%08x nNonce64=%016x matmul_digest=%064x matmul_dim=%u seed_a=%064x seed_b=%064x)" \
            % (self.nVersion, self.hashPrevBlock, self.hashMerkleRoot,
               time.ctime(self.nTime), self.nBits, self.nNonce, self.nNonce64,
               self.matmul_digest, self.matmul_dim, self.seed_a, self.seed_b)
...
class CBlock(CBlockHeader):
    ...
    def solve(self):
        strict = os.getenv("BTX_PY_BLOCK_SOLVE_STRICT", "").lower() in {"1", "true", "yes", "on"}
        default_max_tries = "300000" if strict else "5000"
        try:
            max_tries = int(os.getenv("BTX_PY_BLOCK_SOLVE_MAX_TRIES", default_max_tries))
        except ValueError:
            max_tries = int(default_max_tries)
        if max_tries <= 0:
            max_tries = 1

        # Keep handcrafted block headers phase1-valid on BTX regtest by
        # default: set deterministic non-null seeds, expected dimension, and a
        # digest that satisfies the compact target check.
        ...
        target = uint256_from_compact(self.nBits)
        tries = 0
        while self.sha256 > target and tries < max_tries:
        ...
        if self.sha256 > target:
            if strict:
                raise RuntimeError(f"Unable to solve block header in {max_tries} tries")
            # Many BTX regtest functional lanes skip full PoW checks. Keep the
            # header deterministic while preserving phase1-valid MatMul fields.
            self.nNonce = 0
            self.nNonce64 = 0
            if self.hashPrevBlock != 0:
                self.matmul_digest = 0
        ...
        register_solved_block(
            block_hash=self.sha256,
            prev_hash=self.hashPrevBlock,
            n_time=self.nTime,
            n_bits=self.nBits,
        )
