...
def fill_mempool(test_framework, node, *, tx_sync_fun=None):
    ...
    global _FILL_MEMPOOL_CALL_COUNTER
    _FILL_MEMPOOL_CALL_COUNTER += 1
    ephemeral_miniwallet = MiniWallet(node, tag_name=f"fill_mempool_ephemeral_wallet_{_FILL_MEMPOOL_CALL_COUNTER}")
    ...
    mempoolminfee_before_tail = node.getmempoolinfo()["mempoolminfee"]
    # BTX large-tx construction can trigger earlier trimming than upstream,
    # so only require that mempoolminfee never drops below minrelaytxfee.
    assert mempoolminfee_before_tail >= minrelayfee
    for fee in batch_fees[-3:]:
        send_batch(fee)
