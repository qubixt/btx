...
def create_lots_of_big_transactions(mini_wallet, node, fee, tx_batch_size, txouts, utxos=None):
    ...
    def build_large_fanout_tx(*, utxo_to_spend):
        # BTX consensus caps OP_RETURN payloads, so keep large-tx generation
        # independent of nulldata by fanning out to many standard outputs.
        min_output_value_sat = 1000
        utxo_value_sat = int(utxo_to_spend["value"] * COIN)
        output_count = min(1500, max(1, utxo_value_sat // (min_output_value_sat + 1)))
        tx_result = mini_wallet.create_self_transfer_multi(
            utxos_to_spend=[utxo_to_spend],
            num_outputs=output_count,
            fee_per_output=1,
        )
        tx = tx_result["tx"]
        target_fee_sat = int(fee * COIN)
        current_fee_sat = int(tx_result["fee"] * COIN)
        fee_delta_sat = target_fee_sat - current_fee_sat
        if fee_delta_sat > 0:
            # As block subsidy falls on regtest, a single output may no longer
            # be large enough to absorb the desired fee delta.
            remaining_fee_delta = fee_delta_sat
            for tx_out in tx.vout:
                if remaining_fee_delta == 0:
                    break
                max_deduct = tx_out.nValue - min_output_value_sat
                if max_deduct <= 0:
                    continue
                deduct = min(max_deduct, remaining_fee_delta)
                tx_out.nValue -= deduct
                remaining_fee_delta -= deduct
            assert_equal(remaining_fee_delta, 0)
        tx.rehash()
        return tx

    txids = []
    use_internal_utxos = utxos is None
    # BTX enforces a strict output script size cap, so oversized padding
    # outputs must never enter the legacy OP_RETURN construction path.
    max_scriptpubkey_size = 83
    txouts_oversized = txouts is not None and any(len(txout.scriptPubKey) > max_scriptpubkey_size for txout in txouts)
    use_legacy_padding = txouts is not None and not txouts_oversized
    for _ in range(tx_batch_size):
        utxo_to_spend = mini_wallet.get_utxo() if use_internal_utxos else utxos.pop()

        if use_legacy_padding:
            ...
            # BTX enforces stricter output-size consensus limits than upstream.
            reject_reason = res.get("reject-reason", "")
            if "opreturn-size" not in reject_reason and "scriptpubkey-size" not in reject_reason:
                raise AssertionError(f"unexpected rejection while building large tx: {reject_reason}")
            use_legacy_padding = False

        tx = build_large_fanout_tx(utxo_to_spend=utxo_to_spend)
        res = node.testmempoolaccept([tx.serialize().hex()])[0]
        if not res.get("allowed", False):
            raise AssertionError(f"large tx generation rejected by mempool policy: {res}")
        assert_equal(res['fees']['base'], fee)
        txids.append(node.sendrawtransaction(tx.serialize().hex()))
    return txids
