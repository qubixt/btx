...
class LoadblockTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # Keep blk/rev files plain (no XOR obfuscation) so linearization
        # utilities can process short test chains deterministically.
        self.extra_args = [['-blocksxor=0'], ['-blocksxor=0']]

    def run_test(self):
        linearize_timeout_seconds = 120
        ...
        block_header_size = len(bytes.fromhex(self.nodes[0].getblockheader(genesis_block, False)))
        ...
        with open(cfg_file, "a", encoding="utf-8") as cfg:
            ...
            cfg.write(f"block_header_sz={block_header_size}\n")
        ...
        subprocess.run([sys.executable, linearize_hashes_file, cfg_file],
                       stdout=hash_list,
                       timeout=linearize_timeout_seconds,
                       check=True)
        hash_list.flush()
        os.fsync(hash_list.fileno())
        hash_list.close()
