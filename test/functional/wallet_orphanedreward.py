...
def get_coinbase_reward(node, block_hash):
    block = node.getblock(block_hash, 2)
    return Decimal(str(block["tx"][0]["vout"][0]["value"]))


def assert_transparent_balance_views(wallet, expected_trusted, *, expected_immature=Decimal("0")):
    mine_balances = wallet.getbalances()["mine"]
    assert_equal(mine_balances["trusted"], expected_trusted)
    assert_equal(mine_balances["untrusted_pending"], Decimal("0"))
    ...
    assert_equal(spendable_utxo_total, expected_trusted)

    totals = wallet.z_gettotalbalance()
    assert_equal(Decimal(str(totals["transparent"])), expected_trusted)
    assert_equal(Decimal(str(totals["shielded"])), Decimal("0"))
    assert_equal(Decimal(str(totals["total"])), expected_trusted)
...
class OrphanedBlockRewardTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        coinbase_reward = get_coinbase_reward(self.nodes[1], blk)

        # Let the block reward mature and send coins including both
        # the existing balance and the block reward.
        self.generate(self.nodes[0], 150)
        assert_transparent_balance_views(self.nodes[1], Decimal("10") + coinbase_reward)
        pre_reorg_conf_bals = self.nodes[1].getbalances()
        txid = self.nodes[1].sendtoaddress(self.nodes[0].getnewaddress(), coinbase_reward + Decimal("5"))
        ...
        assert_transparent_balance_views(self.nodes[1], Decimal("10"))
