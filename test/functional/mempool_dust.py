...
from test_framework.blocktools import BTX_MAX_TXOUT_SCRIPT_SIZE
    ...
    def run_test(self):
        ...
        for dustfee_sat_kvb in (DUST_RELAY_TX_FEE, 0, 1, 66, 500, 1337, 12345, 21212, 333333):
            ...
            for output_script, description in output_scripts:
                if len(output_script) > BTX_MAX_TXOUT_SCRIPT_SIZE:
                    self.log.info(
                        f"-> Skip {description} output (size {len(output_script)} exceeds consensus max {BTX_MAX_TXOUT_SCRIPT_SIZE})"
                    )
                    continue
