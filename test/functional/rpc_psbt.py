...
class PSBTTest(BitcoinTestFramework):
    ...
    def _supports_wallet_address_type(self, address_type):
        try:
            self.nodes[0].getnewaddress(address_type=address_type)
            return True
        except JSONRPCException as exc:
            message = exc.error.get("message", "") if isinstance(exc.error, dict) else str(exc)
            lowered = message.lower()
            if "unsupported address type" in lowered or "only address type 'p2mr'" in lowered or "no wallet is loaded" in lowered:
                return False
            raise
    ...
    def run_btx_p2mr_psbt_smoke(self):
        node = self.nodes[0]
        self.log.info("BTX p2mr-only policy detected; running PSBT smoke coverage.")

        assert_raises_rpc_error(-8, "Only address type 'p2mr' is supported", node.getnewaddress, address_type="bech32")
        assert_raises_rpc_error(-8, "Only address type 'p2mr' is supported", node.getnewaddress, address_type="bech32m")

        # Ensure core PSBT funding/signing flow works with default p2mr outputs.
        recipient = node.getnewaddress()
        psbt = node.walletcreatefundedpsbt([], {recipient: Decimal("1.0")})["psbt"]
        processed = node.walletprocesspsbt(psbt)
        assert processed["complete"]
        txid = node.sendrawtransaction(processed["hex"])
        [block_hash] = self.generate(node, 1)
    ...
    def run_test(self):
        ...
        self.log.info("Test walletcreatefundedpsbt fee rate of 10000 sat/vB and 0.1 BTX/kvB produces a total fee at or slightly below -maxtxfee (~0.05290000)")
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (BTX/kvB)",
