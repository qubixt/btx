...
class NodeNetworkLimitedTest(BitcoinTestFramework):
    ...
    def test_avoid_requesting_historical_blocks(self):
        ...
        # Lastly, ensure the full_node is not sync without permission, then restart it
        # with outbound download permission and verify it can historical-sync from the
        # NODE_NETWORK_LIMITED peer.
        assert_equal(full_node.getblockcount(), start_height_full_node)

        self.log.info("Restart pruned node with download permission to serve historical blocks")
        self.restart_node(0, self.extra_args[0] + ["-whitelist=noban,download@127.0.0.1"])
        pruned_node = self.nodes[0]

        self.log.info("Restart lagging full node with outbound download permission")
        self.restart_node(2, self.extra_args[2] + ["-whitelist=noban,download,out@127.0.0.1"])
        full_node = self.nodes[2]
        self.connect_nodes(2, 0)
        self.sync_blocks([pruned_node, full_node])
    ...
    def run_test(self):
        ...
        assert_equal(node.nServices & expected_services, expected_services)

        self.log.info("Check that the localservices is as expected.")
        assert_equal(int(self.nodes[0].getnetworkinfo()['localservices'], 16) & expected_services, expected_services)
