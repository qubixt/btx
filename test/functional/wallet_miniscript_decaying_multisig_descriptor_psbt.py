...
class WalletMiniscriptDecayingMultisigDescriptorPSBTTest(BitcoinTestFramework):
    ...
    def _get_xpub(wallet):
        """Extract the active HD xpub for the signer wallet.

        BTX descriptor wallets may default to PQ-only receive/change descriptors, so
        relying on a legacy `pkh(...)` descriptor is not robust here. The HD master
        xpub remains the stable source for building imported legacy miniscript flows.
        """
        hdkeys = wallet.gethdkeys(active_only=True)
        if not hdkeys:
            hdkeys = wallet.gethdkeys()
        if not hdkeys:
            raise SkipTest(
                f"{wallet.getwalletinfo()['walletname']} does not expose any HD xpub "
                "(BTX PQ descriptor policy active without secp256k1 HD metadata)"
            )
        return hdkeys[0]["xpub"]
    ...
    def run_test(self):
        ...
        signer_xpubs = [self._get_xpub(signer) for signer in signers]
        external_xpubs = [f"{xpub}/0/*" for xpub in signer_xpubs]
        internal_xpubs = [f"{xpub}/1/*" for xpub in signer_xpubs]
