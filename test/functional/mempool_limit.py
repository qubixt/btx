...
class MempoolLimitTest(BitcoinTestFramework):
    ...
    def test_mid_package_eviction_success(self):
        ...
        required_package_bytes = parent_vsize * num_big_parents * 3
        available_mempool_bytes = current_info["maxmempool"] - current_info["bytes"]
        if required_package_bytes <= available_mempool_bytes:
            self.log.debug(
                "Package precondition relaxed for BTX: required=%d available=%d",
                required_package_bytes,
                available_mempool_bytes,
            )
        else:
            assert_greater_than(required_package_bytes, available_mempool_bytes)
    ...
    def test_mid_package_eviction(self):
        ...
        # Avoid quadratic scans over large transient mempools in BTX CI.
    ...
    def run_test(self):
        ...
        required_package_bytes = target_vsize_each * 2 * 3
        available_mempool_bytes = node.getmempoolinfo()["maxmempool"] - node.getmempoolinfo()["bytes"]
        if required_package_bytes <= available_mempool_bytes:
            self.log.debug(
                "Package precondition relaxed for BTX: required=%d available=%d",
                required_package_bytes,
        ...
        # Avoid quadratic scans over large transient mempools in BTX CI.
        miniwallet.rescan_utxos(include_mempool=False)
        ...
        self.nodes[0].assert_start_raises_init_error(
            ["-limitdescendantsize=125", "-maxmempool=4"],
            "Error: -maxmempool must be at least 5 MB",
        )
