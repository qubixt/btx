...
class ZMQTest (BitcoinTestFramework):
    ...
    def test_basic(self, unix = False):
        ...
        for x in range(num_blocks):
            ...
            # KAWPOW headers cannot be validated with the legacy SHA256 is_valid() helper.
            assert_equal(block.hashMerkleRoot, block.calc_merkle_root())
            assert_equal(block.vtx[0].hash, tx.hash)
            assert_equal(len(block.vtx), 1)
            assert_equal(genhashes[x], self.nodes[0].getblock(genhashes[x], 1)["hash"])
            assert_equal(hex.hex(), self.nodes[0].getblock(genhashes[x], 0))
