...
Sanity check:
  Sum(1,2,3,4 balances) == 114 * initial mature coinbase reward
...
class WalletBackupTest(BitcoinTestFramework):
    ...
    def test_restore_existent_dir(self):
        ...
        wallet_file = wallet_dir / "wallet.dat"
        os.remove(wallet_file)
        extra_file = wallet_dir / "not_a_wallet.txt"
        extra_file.touch()
        res = node.restorewallet(wallet_name, backup_file)
        assert_equal(res['name'], wallet_name)
    ...
    def test_restore_into_unnamed_wallet(self):
        ...
        node = self.nodes[3]
        backup_file = self.nodes[0].datadir_path / 'wallet.bak'
        res = node.restorewallet("", backup_file)
    ...
    def run_test(self):
        ...
        initial_coinbase_reward = self.nodes[0].getbalance()
        assert_equal(self.nodes[1].getbalance(), initial_coinbase_reward)
        assert_equal(self.nodes[2].getbalance(), initial_coinbase_reward)
        ...
        # 114 are mature, so the sum of all wallets should track the live mature
        # coinbase reward instead of Bitcoin's historical 50-unit assumption.
        assert_equal(total, initial_coinbase_reward * 114)
