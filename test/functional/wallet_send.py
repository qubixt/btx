...
class WalletSendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            import_res = w2.importdescriptors([{
            ...
            if not all(entry.get("success", False) for entry in import_res):
                raise SkipTest(
                    "w2 descriptor import for wpkh xpriv failed "
        ...
        for _ in range(3):
            try:
                ...
                if self.options.descriptors and "no available keys" in str(e).lower():
                    raise SkipTest(
                        "descriptor wallet has no compatible keypool for upstream wpkh flow "
                        "(BTX PQ descriptor policy active)"
                    ) from e
                raise
        ...
        # NOTE: Return hex if fee rate is below -maxmempool
