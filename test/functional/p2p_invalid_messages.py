...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_size(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        oversized_str_data = VALID_DATA_LIMIT + 1
        oversized_message_size = MAX_PROTOCOL_MESSAGE_LENGTH + 1
        # For v2 transport, logs include packet framing overhead.
        oversized_packet = oversized_message_size + 13
        error_msg = (
            [f'V2 transport error: packet too large ({oversized_packet} bytes)'] if self.options.v2transport
            else [f'Header error: Size too large (badmsg, {oversized_message_size} bytes)']
        )
        with self.nodes[0].assert_debug_log(error_msg):
            msg = msg_unrecognized(str_data="d" * oversized_str_data)
    ...
    def test_addrv2(self, label, required_log_messages, raw_addrv2, *, expect_disconnect=False):
        node = self.nodes[0]
    ...
    def test_invalid_pow_headers_msg(self):
        self.log.info("Test headers message with invalid proof-of-work is rejected and scored")
        blockheader_tip_hash = self.nodes[0].getbestblockhash()
        headers_before = self.nodes[0].getblockchaininfo()['headers']

        # send valid headers message first
        block_template = self.generateblock(self.nodes[0], output='raw(51)', transactions=[], submit=False, sync_fun=self.no_op)
        block = from_hex(CBlock(), block_template["hex"])
        blockheader = CBlockHeader(block)
        blockheader.rehash()
        peer = self.nodes[0].add_p2p_connection(P2PInterface())
        peer.send_and_ping(msg_headers([blockheader]))
        assert_equal(self.nodes[0].getblockchaininfo()['headers'], headers_before + 1)
        chaintips = self.nodes[0].getchaintips()
        assert_equal(chaintips[0]['status'], 'headers-only')
        assert_equal(chaintips[0]['hash'], blockheader.hash)

        # invalidate PoW
        blockheader.nBits ^= 0x1
        blockheader.rehash()
        with self.nodes[0].assert_debug_log(['invalid claimed header work transition']):
        ...
        with self.nodes[0].assert_debug_log(['bad-diffbits, incorrect proof of work', 'invalid block header; tolerating']):
