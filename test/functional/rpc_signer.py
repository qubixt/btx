...
class RPCSignerTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        signer = signers[0]
        assert_equal(signer['fingerprint'], '00000001')
        assert_equal(signer['name'], 'trezor_t')
        assert_equal(signer['capabilities']['p2mr'], True)
        assert_equal(signer['capabilities']['pq_algorithms'], ['ml_dsa_44', 'slh_dsa_128s'])
