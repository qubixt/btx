...
class MinimumChainWorkTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Determine per-block chainwork increment dynamically so the test works
        # with both legacy PoW and KAWPOW chainwork semantics.
        probe_hash = self.generate(self.nodes[0], 1, sync_fun=self.no_op)[0]
        probe_chain_work = int(self.nodes[0].getblockheader(probe_hash)['chainwork'], 16)
        work_per_block = probe_chain_work - starting_chain_work
        assert work_per_block > 0

        target_below_threshold = self.node_min_work[1] - 1
        remaining_work = max(target_below_threshold - probe_chain_work, 0)
        num_blocks_to_generate = remaining_work // work_per_block
        self.log.info(f"Generating {1 + num_blocks_to_generate} blocks on node0 before threshold-cross trigger")
        if num_blocks_to_generate:
            self.generate(self.nodes[0], num_blocks_to_generate, sync_fun=self.no_op)

        current_chain_work = int(self.nodes[0].getblockheader(self.nodes[0].getbestblockhash())['chainwork'], 16)
        self.log.info(f"Node0 current chain work: {current_chain_work:064x}")
        ...
        blocks_to_exceed = ((self.node_min_work[1] - current_chain_work) // work_per_block) + 1
        self.generate(self.nodes[0], blocks_to_exceed)
