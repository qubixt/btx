...
class MempoolAcceptanceTest(BitcoinTestFramework):
    ...
    def _miniwallet_probe(self, node):
        ...
        return {
            "tx_hex": probe_tx_hex,
            "accept": node.testmempoolaccept([probe_tx_hex])[0],
        }

    def _run_btx_policy_mode(self, node, probe_result):
        self.log.info("BTX policy mode: MiniWallet default script type is non-standard; running policy-specific mempool checks.")
        probe_accept = probe_result["accept"]
        assert_equal(probe_accept["allowed"], False)
        assert_equal(probe_accept["reject-reason"], "scriptpubkey")
        assert_raises_rpc_error(-26, "scriptpubkey", node.sendrawtransaction, probe_result["tx_hex"])
    ...
    def run_test(self):
        ...
        if not probe["accept"].get("allowed", False) and probe["accept"].get("reject-reason") == "scriptpubkey":
            self._run_btx_policy_mode(node, probe)
            return
        ...
        # Settings are listed in BTX/kvB
        ...
        # Use a specific UTXO so split amounts stay valid across non-Bitcoin subsidy schedules.
        utxo_in_block = self.wallet.get_utxo(mark_as_spent=False)
        tx = self.wallet.create_self_transfer(utxo_to_spend=utxo_in_block)['tx']
        tx.vout.append(deepcopy(tx.vout[0]))
        split_amount = Decimal("0.3")
        split_fee = Decimal("0.01")
        ...
        self.mempool_size = 0
        # Also check feerate. 1BTX/kvB fails
        assert_raises_rpc_error(-8, "Fee rates larger than or equal to 1BTX/kvB are not accepted", lambda: self.check_mempool_result(
        ...
        utxo_final = self.wallet.get_utxo(mark_as_spent=False)
        ...
        # Depending on reject-ordering, this may fail as oversize or duplicate-inputs.
        result = node.testmempoolaccept(rawtxs=[tx.serialize().hex()])
        assert_equal(len(result), 1)
        assert_equal(result[0]["txid"], tx.rehash())
        assert_equal(result[0]["allowed"], False)
        assert result[0]["reject-reason"] in {"bad-txns-oversize", "bad-txns-inputs-duplicate"}
        assert_equal(node.getmempoolinfo()['size'], self.mempool_size)  # Must not change mempool state
        ...
        tx.rehash()
        if len(tx.vout[0].scriptPubKey) > 34:
            self.log.info('Bare multisig scriptPubKey exceeds BTX max size and is consensus-invalid')
            try:
                self.generateblock(node, address, [tx.serialize().hex()])
            except JSONRPCException as e:
                assert e.error["code"] in (-25, -32603)
                if e.error["code"] == -25:
                    assert "bad-txns-scriptpubkey-size" in e.error["message"]
                else:
                    assert "block not accepted" in e.error["message"]
            else:
                raise AssertionError("Expected generateblock to reject consensus-invalid oversized scriptPubKey")
            return
