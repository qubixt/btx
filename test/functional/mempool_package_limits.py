...
def check_package_limits(func):
    def func_wrapper(self, *args, **kwargs):
        ...
        any_rejected = any(
            ("package-error" in txres) or
            ("allowed" in txres and not txres["allowed"])
        ...
        if any_rejected:
            for txres in testres_error_expected:
                if "package-error" in txres:
                    ...
                    assert "mempool" in txres["reject-reason"], txres
        else:
            # BTX policy path: testmempoolaccept may report per-tx acceptance while
            # submitpackage rejects the package due combined ancestor/descendant limits.
            submit_res = node.submitpackage(package_hex)
            assert_equal(submit_res["package_msg"], "transaction failed")
            for tx_result in submit_res["tx-results"].values():
                if "error" in tx_result:
                    assert "mempool" in tx_result["error"], tx_result
...
class MempoolPackageLimitsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.restart_node(0, extra_args=self.extra_args[0] + ["-datacarriersize=100000"])
