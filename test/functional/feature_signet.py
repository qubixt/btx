...
class SignetBasicTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        first_submit = None
        try:
            first_submit = self.nodes[2].submitblock(signet_blocks[0])
        except JSONRPCException as e:
            if e.error["code"] == -22 and e.error["message"] == "Block decode failed":
                first_submit = "decode-failed"
            else:
                raise
        ...
        if first_submit is None:
            ...
            for block in signet_blocks[1:]:
                assert_equal(self.nodes[2].submitblock(block), None)
                height += 1
                assert_equal(self.nodes[2].getblockcount(), height)
            incompatible_solution_block = signet_blocks[0]
        else:
            # Under BTX/KAWPOW the legacy pregenerated signet fixtures are not valid
            # PoW data; keep exercising signet challenge isolation with live blocks.
            assert first_submit in {"decode-failed", "high-hash", "bad-signet-blksig"}
            assert self.nodes[2].submitblock(op_true_block) in {'bad-signet-blksig', 'duplicate'}
            incompatible_solution_block = op_true_block

        self.log.info("pregenerated signet blocks check (incompatible solution)")
        assert self.nodes[4].submitblock(incompatible_solution_block) in {'bad-signet-blksig', 'duplicate'}
