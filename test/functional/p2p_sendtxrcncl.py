...
class SendTxRcnclTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info('SENDTXRCNCL with trailing bytes is punished even when txreconciliation is disabled')
        malformed_sendtxrcncl = create_sendtxrcncl_msg().serialize() + b"\x00"
        peer = self.nodes[0].add_p2p_connection(PeerNoVerack(), send_version=True, wait_for_verack=False)
        with self.nodes[0].assert_debug_log(["Misbehaving", "sendtxrcncl payload size = 13"]):
            peer.send_message(msg_generic(b"sendtxrcncl", malformed_sendtxrcncl))
            peer.wait_for_disconnect()

        self.log.info('truncated SENDTXRCNCL payload is punished')
        peer = self.nodes[0].add_p2p_connection(PeerNoVerack(), send_version=True, wait_for_verack=False)
        with self.nodes[0].assert_debug_log(["Misbehaving", "sendtxrcncl payload size = 2"]):
            peer.send_message(msg_generic(b"sendtxrcncl", b"\x01\x02"))
            peer.wait_for_disconnect()
