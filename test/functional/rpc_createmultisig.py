...
class RpcCreateMultiSigTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        try:
            node0.createmultisig(2, probe_pubkeys)
        except JSONRPCException as err:
            if err.error["code"] == -8 and LEGACY_MULTISIG_DISABLED_ERROR in err.error["message"]:
                if self.is_wallet_compiled():
                    wallet_multi = self.create_wallet(node1, 'wmulti')
                    assert_raises_rpc_error(
                        -8,
                        LEGACY_MULTISIG_DISABLED_ERROR,
                ...
                self.log.info("BTX post-quantum policy disables legacy multisig RPCs; skipping upstream multisig matrix.")
                return
            raise
