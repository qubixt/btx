...
def read_logs(tmp_dir):
    ...
    # Some tests (e.g. mainnet-only) can log directly under node0/debug.log.
    debug_logs = list(pathlib.Path(tmp_dir).glob('node0/**/debug.log'))
    match len(debug_logs):
        case 0:
            chain = 'regtest'  # fallback to regtest
        case 1:
            relative = debug_logs[0].relative_to(pathlib.Path(tmp_dir) / 'node0')
            chain = relative.parent.as_posix()
            if chain == '.':
                chain = ''
        case _:
    ...
    for i in itertools.count():
        ...
        logfile = f"{tmp_dir}/node{i}/{chain_path}debug.log"
