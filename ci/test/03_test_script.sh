...
(
  ...
  tee >(patch --batch --forward -r /dev/null -p1) <<'EOF'
...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  # Host-mode CI can leave partial qa-assets directories behind on retries.
  # Recreate deterministically to avoid repeated clone failures/backoff.
  if [ ! -d "$DIR_FUZZ_IN" ] || [ ! -d "${DIR_QA_ASSETS}/.git" ]; then
    rm -rf "${DIR_QA_ASSETS}"
...
# cross-contaminating matrix lanes (e.g. fuzz-only cache in non-fuzz jobs).
...
BUILD_GOAL="${GOAL}"
if [[ -n "${BUILD_GOAL}" ]]; then
  if ! cmake --build . --target help | grep -Eq "(^|[[:space:]])${BUILD_GOAL}([[:space:]]|$)"; then
    echo "Requested goal '${BUILD_GOAL}' is unavailable for this CMake configuration; building only 'all'."
    BUILD_GOAL=""
  fi
fi
BUILD_GOAL_ARGS=""
if [[ -n "${BUILD_GOAL}" ]]; then
  BUILD_GOAL_ARGS=" ${BUILD_GOAL}"
fi

bash -c "cmake --build . $MAKEJOBS --target all${BUILD_GOAL_ARGS}" || ( echo "Build failure. Verbose build follows." && bash -c "cmake --build . --target all${BUILD_GOAL_ARGS} --verbose" ; false )
...
if [ "$RUN_UNIT_TESTS" = "true" ]; then
  ...
  if [ -n "${CTEST_REGEX:-}" ]; then
    CTEST_RUN_ARGS+=(-R "${CTEST_REGEX}")
  fi
  if [ -n "${CTEST_EXCLUDE_REGEX:-}" ]; then
    CTEST_RUN_ARGS+=(-E "${CTEST_EXCLUDE_REGEX}")
  fi
  DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" \
    LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" \
    CTEST_OUTPUT_ON_FAILURE=ON \
    ctest --stop-on-failure --output-on-failure "${MAKEJOBS}" \
      --timeout $(( TEST_RUNNER_TIMEOUT_FACTOR * 60 )) \
      "${CTEST_RUN_ARGS[@]}"
fi

if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${BASE_OUTDIR}"/bin/test_btx --catch_system_errors=no -l test_suite
fi
