...
if [[ "${HAS_GIT}" == "true" && "$(git config --global "${CFG_DONE}" 2>/dev/null || true)" == "true" ]]; then
...
if [[ "${RUN_TIDY}" == "true" ]]; then
  # Tidy lanes may re-run base install in a container already containing iwyu
  # from image build time. Reset paths to make retries deterministic.
