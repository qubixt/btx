...
class BlockDataCopier:
    ...
    def is_preallocated_padding(self, inhdr, file_offset):
        """Return True if bytes are an unwritten preallocated tail.

        Block files may be sparse-preallocated with zero bytes on disk. When
        per-block-file XOR is enabled, zero bytes decode to the rolling XOR key
        stream, not literal zeros. Detect both cases to avoid scanning large
        preallocated tails byte-by-byte.
        """
        if not inhdr:
            return True
        if inhdr[0] == 0:
        ...
        if not key:
            return False
        pad = bytes(key[(file_offset + i) % len(key)] for i in range(len(inhdr)))
        return inhdr == pad
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            ...
            hdr_offset = self.inF.tell()
            inhdr = self.read_xored(self.inF, 8)
            if self.is_preallocated_padding(inhdr, hdr_offset):
                self.inF.close()
                self.inF = None
            ...
            if su[0] < self.settings['block_header_sz']:
                # Corrupted length field; rewind similarly to the bad-magic path
                # and continue scanning.
                self.inF.seek(-7, os.SEEK_CUR)
                continue
            inLen = su[0] - self.settings['block_header_sz'] # length without header
            blk_hdr = self.read_xored(self.inF, self.settings['block_header_sz'])
...
if __name__ == '__main__':
    ...
    if 'block_header_sz' not in settings:
        settings['block_header_sz'] = 80
    ...
    settings['block_header_sz'] = int(settings['block_header_sz'])
