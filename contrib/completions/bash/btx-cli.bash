# Dynamic bash programmable completion for btx-cli(1)
# Copyright (c) 2012-2024 The Bitcoin Core developers
# Copyright (c) 2026 The BTX developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $btx_cli for RPC
_btx_rpc() {
    # determine already specified args necessary for RPC
    local rpcargs=()
    for i in ${COMP_LINE}; do
        case "$i" in
            -conf=*|-datadir=*|-regtest|-rpc*|-testnet|-testnet4)
                rpcargs=( "${rpcargs[@]}" "$i" )
                ;;
        esac
    done
    $btx_cli "${rpcargs[@]}" "$@"
}

_btx_cli() {
    local cur prev words=() cword
    local btx_cli

    # save and use original argument to invoke btx-cli for -help, help and RPC
    # as btx-cli might not be in $PATH
    btx_cli="$1"

    COMPREPLY=()
    _get_comp_words_by_ref -n = cur prev words cword

    if ((cword > 5)); then
        case ${words[cword-5]} in
            descriptorprocesspsbt)
                COMPREPLY=( $( compgen -W "false true" -- "$cur" ) )
                return 0
                ;;
        esac
    fi

    if ((cword > 4)); then
        case ${words[cword-4]} in
            createpsbt|createrawtransaction|descriptorprocesspsbt|setban)
                COMPREPLY=( $( compgen -W "false true" -- "$cur" ) )
                return 0
                ;;
            signrawtransactionwithkey)
                COMPREPLY=( $( compgen -W "ALL ALL|ANYONECANPAY NONE NONE|ANYONECANPAY SINGLE SINGLE|ANYONECANPAY" -- "$cur" ) )
                return 0
                ;;
        esac
    fi

    if ((cword > 3)); then
        case ${words[cword-3]} in
            addnode|converttopsbt|getdescriptoractivity|gettxout|gettxoutsetinfo)
                COMPREPLY=( $( compgen -W "false true" -- "$cur" ) )
                return 0
                ;;
        esac
    fi

    if ((cword > 2)); then
        case ${words[cword-2]} in
            converttopsbt|decoderawtransaction|finalizepsbt|getblockheader|getmempoolancestors|getmempooldescendants|getrawmempool|listmempooltransactions)
                COMPREPLY=( $( compgen -W "false true" -- "$cur" ) )
                return 0
                ;;
            addnode)
                COMPREPLY=( $( compgen -W "add onetry remove" -- "$cur" ) )
                return 0
                ;;
            setban)
                COMPREPLY=( $( compgen -W "add remove" -- "$cur" ) )
                return 0
                ;;
            estimatesmartfee)
                COMPREPLY=( $( compgen -W "CONSERVATIVE ECONOMICAL UNSET" -- "$cur" ) )
                return 0
                ;;
        esac
    fi

    case "$prev" in
        dumptxoutset|importmempool|loadtxoutset)
            _filedir
            return 0
            ;;
        getrawmempool|setnetworkactive|setscriptthreadsenabled)
            COMPREPLY=( $( compgen -W "false true" -- "$cur" ) )
            return 0
            ;;
    esac

    case "$cur" in
        -conf=*)
            cur="${cur#*=}"
            _filedir
            return 0
            ;;
        -datadir=*)
            cur="${cur#*=}"
            _filedir -d
            return 0
            ;;
        -*=*)	# prevent nonsense completions
            return 0
            ;;
        *)
            local helpopts commands

            # only parse -help if senseful
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($btx_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_btx_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi

            COMPREPLY=( $( compgen -W "$helpopts $commands" -- "$cur" ) )

            # Prevent space if an argument is desired
            if [[ $COMPREPLY == *= ]]; then
                compopt -o nospace
            fi
            return 0
            ;;
    esac
} &&
complete -F _btx_cli btx-cli

# Local variables:
# mode: shell-script
# sh-basic-offset: 4
# sh-indent-comment: t
# indent-tabs-mode: nil
# End:
# ex: ts=4 sw=4 et filetype=sh
