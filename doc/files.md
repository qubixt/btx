...
3. All content of the data directory, except for `btx.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `btx.conf`            | User-defined [configuration settings](btx-conf.md) for `btxd` or `btx-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `btx_rw.conf`         | Contains [configuration settings](btx-conf.md) modified by `btxd` or `btx-qt`; can be specified by `-confrw` option
`./`               | `btxd.pid`            | Stores the process ID (PID) of `btxd` or `btx-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `btxd` or `btx-qt`; can be specified by `-debuglogfile` option
