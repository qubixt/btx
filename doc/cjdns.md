...
CJDNS peers than Tor or IP ones. You can use `btx-cli -addrinfo` to see the
...
To see which CJDNS peers your node is connected to, use `btx-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `btx-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `btx-cli help getnodeaddresses` for details.
