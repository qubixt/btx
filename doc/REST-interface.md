...
The interface runs on the same port as the JSON-RPC interface.
BTX deployment runbooks typically set `rpcport=19334` for mainnet operation.
If `rpcport` is omitted, the default RPC ports are 19334 (mainnet),
29334 (testnet), 48332 (testnet4), 38332 (signet), and 18443 (regtest).
...
Example:
```
$ curl localhost:29334/rest/getutxos/checkmempool/b2cdfd7b89def827ff8af7cd9bff7627ff72e5e8b0f71210f92ea7a4000c5d75-0.json 2>/dev/null | json_pp
{
   "chainHeight" : 325347,
...
Risks
-------------
Running a web browser on the same node with REST enabled `btxd` can be a risk.
Accessing prepared XSS websites could read out tx/block data of your node by
placing links like
`<script src="http://127.0.0.1:19334/rest/tx/1234567890.json">`, which might
break node privacy.
