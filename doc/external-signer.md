# Support for signing transactions outside of BTX

For BTX production custody patterns, pair this document with the
[BTX Key Management Guide](btx-key-management-guide.md). The guide covers when
to use external signers, how to keep the coordinator watch-only, and why
address verification on the signer should be mandatory for high-value flows.

BTX can be launched with `-signer=<cmd>` where `<cmd>` is an external tool
which can sign transactions and perform other functions. For example, it can be
used to communicate with a hardware wallet.

## Example usage

The following example is based on the
[HWI](https://github.com/bitcoin-core/HWI) tool. Version 2.0 or newer is
required. Although this tool is hosted under the Bitcoin Core GitHub
organization and maintained by Bitcoin Core developers, it should be used with
caution. It is considered experimental and has far less review than BTX itself.
Be particularly careful when running tools such as these on a computer with
private keys on it.

When using a hardware wallet, consult the manufacturer website for
(alternative) software they recommend. As long as their software conforms to
the standard below, it should be able to work with BTX.

Start BTX:

```sh
$ btxd -signer=../HWI/hwi.py
```
...
```sh
$ btx-cli createwallet "hww" true true "" true true true
```
...
```sh
$ btx-cli -rpcwallet=<wallet> getnewaddress
$ btx-cli -rpcwallet=<wallet> walletdisplayaddress <address>
```
...
```sh
$ btx-cli -rpcwallet=<wallet> sendtoaddress <address> <amount>
```
...
In order to be compatible with BTX, any signer command should conform to the
specification below. This specification is subject to change. Ideally a BIP
should propose a standard so that other wallets can also make use of it.
