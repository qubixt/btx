...
The BTX daemon (`btxd`, legacy alias: `bitcoind`) can be configured to act as
a trusted "border
...
For instance:
    ...
    $ btxd -zmqpubhashtx=tcp://127.0.0.1:28332 \
           ...
           -zmqpubrawtx=unix:/tmp/btxd.tx.raw \
...
From the perspective of `btxd`, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into `btxd` directly. Furthermore, no information is
...
using. `btxd` appends an up-counting sequence number to each
