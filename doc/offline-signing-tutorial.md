...
> [!NOTE]
> This tutorial explains the generic offline-signing pattern. BTX production
> operators should pair it with the
> [BTX Key Management Guide](btx-key-management-guide.md) and use P2MR
> descriptor wallets, watch-only coordinators, and signer separation instead of
> treating legacy Bitcoin address types as the preferred BTX model.
...
```sh
[offline]$ ./build/bin/btx-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/bin/btx-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/bin/btx-cli -signet -named createwallet \
...
```sh
[online]$ ./build/bin/btx-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/bin/btx-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./build/bin/btx-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/bin/btx-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/bin/btx-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/bin/btx-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/bin/btx-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/bin/btx-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/bin/btx-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/bin/btx-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/bin/btx-cli -signet -rpcwallet="watch_only_wallet" listtransactions
