...
```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_btx
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_btx --log_level=test_suite
...
execution. When `lcov` and `genhtml` are not available, the coverage scripts
fall back to `gcovr` automatically. This is useful on macOS/Clang hosts where
LCOV is not packaged by default.

To enable coverage report generation during test runs:
...
# A coverage report will now be accessible at `./build/test_btx.coverage/index.html`,
...
To use the `gcovr` fallback explicitly, install `gcovr` and pass its path if it
is not on `PATH`:

```shell
python3 -m pip install --user gcovr
cmake -DGCOVR_EXECUTABLE="$(command -v gcovr)" -P build/Coverage.cmake
```
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 19335.
